...
хода клампится по персональному deny-list пользователя (`user_llm_access`); при подмене модель получает явное предупреждение в ответе.
