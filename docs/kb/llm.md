...
Старые ответы переписаны разово при старте (`runOnce('codex_thread_usage_v1')`
→ `chat.migrateCodexThreadUsage()`, оба бэкенда): по порядку сообщений беседы,
идемпотентно — ответы с `codexThreadUsage` не трогаются. В CI тот же расчёт
...
`stop-loading`, `frames`, `styles`, `reset-session`, `dialogs`, `handle-dialog`, `downloads`, `read-download`, `cancel-download` и `delete-download` также входят в allow-list Claude при подключённом preview MCP,
