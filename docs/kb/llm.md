...
- ` ```image ` + JSON `{path, agentId?, caption?}` → созданная моделью картинка
  ...
  `MessageImage` при отсутствии явного `agentId` **сначала спрашивает сервер**
  (`GET /api/files/read`, мост `window.files`), и только если тот ответил 404 —
  читает с машины через `fs.read`. Байты в обоих случаях приходят base64 и
  показываются как data-URL.
