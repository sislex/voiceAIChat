...
читает локальный `HOME`: при настроенном исполнителе он проксирует запрос в
`GET /v1/auth/status?userId=...` и получает статус профиля пользователя на
стороне `apps/llm-runner`. UI по-прежнему опрашивает этот роут раз в 30 с.
...
`/api/cx/*` больше не читают диск напрямую: они проксируют в файловые роуты
исполнителя `/v1/fs/cc/*` и `/v1/fs/cx/*`, сохраняя наружу прежние формы
ответов. Live-tail больше идёт не через локальный `fs.watch`, а через SSE
`/v1/fs/cc/watch` и `/v1/fs/cx/watch`; клиентский контракт `cc.tail` /
`cx.tail` не меняется, а reconnect сервера к исполнителю продолжается с
`Last-Event-ID`, чтобы не терять хвост между переподключениями. Важные детали
формата и грабли — в `docs/plans/CC_OBSERVER.md`: файлы бывают до ~10 МБ,
поэтому для списка читается только «голова» файла (cwd + первый промпт), полный
разбор — при открытии транскрипта; реальный путь проекта берётся из `cwd`
события, а не из слага каталога.
