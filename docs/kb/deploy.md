...
- `VC_BROWSER_EXTRA_CA_FILE` (по умолчанию
  `/certs-src/caddy/pki/authorities/local/root.crt`) или `VC_BROWSER_EXTRA_CA_PEM`
  содержимым. Том Caddy `vc-caddy` смонтирован в раннер как `/certs-src:ro`.
...
- Сертификат читается **один раз при старте**. На совсем свежем стенде Caddy
  может ещё не создать свой CA к этому моменту — тогда раннер пишет
  предупреждение и поднимается без дополнительного доверия; помогает
  `docker compose restart browser-runner`.
