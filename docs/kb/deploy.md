...
Штатные `scripts/prod/deploy.sh` и `scripts/prod/rebuild-when-idle.sh` после
определения собираемой ревизии экспортируют `VC_RELEASE_COMMIT` с 12-символьным
SHA. Из темы последнего коммита они извлекают первое обозначение задачи вида
`chat-149` или `chatai-149` (разделитель может быть пробелом) и нормализуют его до
`chat-149` в `VC_RELEASE_TASK`; если обозначения нет, значение остаётся пустым.
`docker-compose.yml` передаёт обе переменные серверу, где пустые значения становятся
`null` в ответе health.
