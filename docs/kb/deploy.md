...
решает это задержкой `PROD_REBUILD_DELAY=1600`: до шага пересборки после
модельного хода ещё проходят тесты (`npm test` на этом репозитории — около 7
минут), коммит, пуш и мерж. Запас покрывает весь остаток рана, включая cleanup и
резюме, поэтому ран успевает штатно закрыться и отправить ветку в origin.

Разложение на две фазы делает окно рестарта секундным и проверяемым. Обе фазы
уходят в отложенные сеансы: в лимит bash-моста (300 с) не укладывается даже
`build`.
...
файл с кодом выхода: он единственный отличает «собралось» от «канал оборвался».
...
## Версия и время выпуска в интерфейсе

`GET /api/health` отдаёт release metadata: `version` и ISO-время `releasedAt`.
...
дата и время выпуска находятся в нативной подсказке и доступном имени элемента.
