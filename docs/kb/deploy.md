...
`VC_MAIL_FROM=ChatAI <логин>`, `VC_PUBLIC_URL=https://45.135.182.251`. Рядом
...
`VC_PUBLIC_URL` тут не обязателен: `baseUrl(req)` собирает адрес из
`X-Forwarded-Proto`/`X-Forwarded-Host`, которые Caddy проставляет, и получается
то же самое. Задан явно, чтобы ссылка не зависела от конфигурации прокси.
...
**Для https алиас не работает и не может.** Подмена меняет и SNI: Caddy
получает `caddy` вместо `89.125.68.35`, а у него блок сайта и сертификат заведены
на IP — ответ `ERR_SSL_PROTOCOL_ERROR`. Поэтому в проде задан только
`89.125.68.35:8787=voicechat:8787`. Чтобы открывать стенд по https изнутри,
нужен либо узкий ufw-приём (`ufw allow from 172.16.0.0/12 to any port 443 proto
tcp` — внешнюю доступность он не меняет), либо внутреннее имя в Caddyfile и в
SAN сертификата.
