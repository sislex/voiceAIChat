...
Git-теги и ищут на собираемом `HEAD` строгий тег `vX.Y.Z`. Защищённая публикация ставит этот тег на итоговый merge-commit основной ветки до запуска production deploy. Найденный номер без `v`
экспортируется как `VC_RELEASE_VERSION`; для нетегированного обычного деплоя остаётся
настроенное значение или compose-default `0.1.0`. Эти же скрипты экспортируют
