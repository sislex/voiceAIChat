...
Персональные профили пользователей лежат внутри
...
профиль пользователя внутри исполнителя. Именно исполнитель теперь читает
`/v1/auth/status`, `/v1/files/read`, `/v1/fs/cc/*` и `/v1/fs/cx/*`; bind-mount
