...
/api/health` вместе с версией и ISO-временем выпуска отдаёт короткий Git SHA и
связанную задачу как nullable-значения. Номер версии задаётся контейнеру через
`VC_RELEASE_VERSION` (в compose по умолчанию `0.1.0`), а `VC_RELEASED_AT` позволяет
передать точное время сборки. Если дата не передана, сервер фиксирует время старта
нового контейнера — для обычного прод-деплоя это и есть время обновления.

Штатные `scripts/prod/deploy.sh` и `scripts/prod/rebuild-when-idle.sh` обновляют
Git-теги и ищут на собираемом `HEAD` строгий тег `vX.Y.Z`. Защищённая публикация release-ветки передаёт её проверенную версию непосредственно
в production deploy как `VC_RELEASE_VERSION`, поэтому опубликованная версия не зависит
от наличия Git-тега на выбранном SHA. Для обычного production deploy скрипты по-прежнему
ищут строгий тег `vX.Y.Z` на собираемом `HEAD`: найденный номер без `v` экспортируется
как `VC_RELEASE_VERSION`, а для нетегированного deploy остаётся настроенное значение
или compose-default `0.1.0`. Эти же скрипты экспортируют
`VC_RELEASE_COMMIT` с 12-символьным SHA. Из темы последнего коммита они извлекают
первое обозначение задачи вида `chat-149` или `chatai-149` (разделитель может быть
пробелом) и нормализуют его до `chat-149` в `VC_RELEASE_TASK`; если обозначения нет,
значение остаётся пустым. `docker-compose.yml` передаёт переменные серверу, где
пустые commit/task становятся `null` в ответе health.

Web-клиент получает эти данные через `app:ping` и сохраняет прежнее отображение
`v<version>` в правом нижнем углу. Нативная подсказка и доступное имя элемента
включают локальные дату и время выпуска, а также имеющиеся коммит и задачу; строка
задачи полностью пропускается, когда определить её не удалось. Реализация находится
в `packages/ui/src/App.tsx`, граничные случаи закреплены в соседнем DOM-тесте.
