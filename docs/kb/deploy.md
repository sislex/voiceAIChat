...
- **Своя база образа.** Цель `browser-runner-runtime` собирается из
  `mcr.microsoft.com/playwright:v1.54.2-noble`, а не из общего `runtime-base`:
  Chromium тянет десятки системных библиотек, и класть их в общий образ ради
  одного сервиса значит раздуть все остальные. **Версия образа обязана совпадать
  с `playwright` в `apps/browser-runner/package.json`** — иначе запустится не тот
  браузер.
...
Проверено `docker build --check --target browser-runner-runtime` и
`docker compose config`. Первая сборка тянет образ Playwright (~2 ГБ) — на проде
стоит следить за местом: инцидент с переполнением диска уже был.
