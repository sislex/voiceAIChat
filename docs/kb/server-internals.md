...
`UploadStore` выдаёт вложению непрозрачный id и хранит его местоположение. UI передаёт в `POST /api/uploads` id разговора и, когда цель сообщения выбрана явно, `agentId`; при отсутствии явного id сервер разрешает машину по настройке разговора тем же resolver, который использует ход. Сервер проверяет принадлежность машины пользователю и сразу пишет исходник через `AgentRegistry.fsWrite` в `<root>/.voicechat_uploads/<random-id>.<ext>`; постоянная серверная копия в этом режиме не создаётся. Без выбранной машины остаётся совместимый режим хранения в `VC_DATA_DIR/uploads`. В prompt передаётся фактический путь, а не клиентское имя; ограничения размера и нормализация пути применяются до записи.
