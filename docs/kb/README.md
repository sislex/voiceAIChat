...
| [data-auth.md](data-auth.md) | Данные и доступ: SQLite, пользователи, роли | 2026-07-30 | ⚠ 10 коммит(ов) в areas после сверки: ed5425f CI: гейт гоняет только воркфлоу — цикл «тест → правка модели → тест» … |
...
| [features/kb-usage.md](features/kb-usage.md) | Использование базы знаний (телеметрия и панель) | 2026-07-31 | ⚠ 6 коммит(ов) в areas после сверки: ed5425f CI: гейт гоняет только воркфлоу — цикл «тест → правка модели → тест» … |
...
| [projects.md](projects.md) | Проекты и канбан-доска | 2026-08-01 | ⚠ 10 коммит(ов) в areas после сверки: ed5425f CI: гейт гоняет только воркфлоу — цикл «тест → правка модели → тест» … |
...
| [server-internals.md](server-internals.md) | Backend изнутри: сборка, маршруты, сессии и сервисы | 2026-07-27 | ⚠ 47 коммит(ов) в areas после сверки: ed5425f CI: гейт гоняет только воркфлоу — цикл «тест → правка модели → тест» … |
| [shared.md](shared.md) | Общий пакет: типы, контракты и чистая логика | 2026-07-27 | ⚠ 26 коммит(ов) в areas после сверки: ed5425f CI: гейт гоняет только воркфлоу — цикл «тест → правка модели → тест» … |
...
| [ui.md](ui.md) | Интерфейс: React, store, remote-мосты и голосовой UX | 2026-07-31 | ⚠ 8 коммит(ов) в areas после сверки: ed5425f CI: гейт гоняет только воркфлоу — цикл «тест → правка модели → тест» … |
