...
**Доверенные устройства и второй фактор.** `PATCH /api/session/:sid { trusted: true }` ставит `trusted_at`; при следующем входе `findTrustedDevice` (доверие живёт 30 дней) пропускает шаг TOTP — 2FA защищает от входа с чужого устройства, а на своём превращается в ежедневный налог. Ключ сравнения — `device_key`, поэтому обновление браузера или соседний адрес провайдера доверие не теряют. Он же решает, считать ли вход новым устройством (`isNewDevice`) — прежнее точное сравнение UA+IP слало предупреждение на смену версии браузера.
