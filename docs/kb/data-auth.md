...
`UserRole = 'admin' | 'developer' | 'tester' | 'observer'` (`packages/shared/src/types.ts`). Проектные полномочия централизованы в `apps/server/src/users/auth.ts`: admin разрешены все текущие и будущие операции; developer — просмотр, создание/редактирование задач, запуск workflow и merge; tester/observer пока не имеют отдельных разрешений. Классификатор `projectPermissionForRequest` ставит административные и опасные HTTP-операции под deny-by-default и возвращает 403 до обращения к данным. Legacy-роль `user` мигрирует в `developer`, а конкретные существующие аккаунты ChatAI `admin` и `admin1` назначаются admin без повышения будущих пользователей.
