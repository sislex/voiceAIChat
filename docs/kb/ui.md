...
В единой строке постоянно доступны вложение, textarea, dropdown режима, микрофон и круглая кнопка отправки; наличие текста не скрывает микрофон. Send и stop используют общий размер UI Kit `sm`: интерактивная область остаётся 44×44, а согласованная графика занимает 16×16; варианты `primary`/`danger`, loader, disabled, hover и focus-visible используют токены UI Kit в обеих темах. На десктопе элементы обычной строки центрируются по общей горизонтальной оси; однострочный текст и placeholder прижаты к нижней части поля перераспределением вертикальных отступов при неизменной высоте строки 40 px. Центральный VoiceBar позиционируется относительно `.main`, поэтому его центр совпадает с фактической областью сообщений и автоматически смещается при открытии или закрытии сайдбара. Мобильное и полноэкранное состояния сохраняют собственные отступы и выравнивание по нижнему краю. Источники поведения — `packages/ui/src/components/ChatColumn.tsx`, `packages/ui/src/components/VoiceBar.tsx`, `packages/ui/src/lib/autoGrow.ts` и `packages/ui/src/styles/app.css`.
