...
Слои по z-index (чтобы панели не перекрывали друг друга): `.side-backdrop` 50 < выдвижной sidebar `.side` 60 < модалки (`.ovl`, `.util-embed--fs`) 70 < `.convsettings` 90 < `.ci-console` 1000 < `.pb-overlay` 1200. Плюс любой переход по маршруту закрывает выдвижной sidebar (эффект в `App.tsx` по `path` из `useHashRoute`) — иначе забытая открытой панель висит поверх страницы.

На телефоне (`max-width: 768px`) полноэкранны не только `.modal`, но и модальные тулы `ToolFrame` (`.ccobs`): высота — `100dvh`, потому что `88vh` уезжает под адресную строку браузера. Кнопка «на весь экран» там скрыта.
