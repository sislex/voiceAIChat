...
`submitText()` валидирует draft/attachments, фиксирует цель выполнения, сохраняет пользовательское сообщение и отправляет LLM-запрос. Композер остаётся доступен в состояниях ожидания, транскрибации, стриминга и озвучки (кроме записи с микрофона): текст, вложения и preview можно отправить, не дожидаясь `idle`. В активном состоянии стор не начинает второй локальный ход и не выполняет utility-команду, а передаёт сохранённую реплику через `sendClaudePrompt`; кнопка и Enter помечены для пользователя как добавление в очередь. Повторный `keydown` Enter с `event.repeat` игнорируется в `VoiceBar`, а `chatStore` допускает только одну наблюдаемую отправку: до начала асинхронной работы он синхронно записывает `pendingSubmit`, и следующие клики или Enter возвращают `false` до авторитетного подтверждения. Источник контракта и переходов — `packages/chat-app/src/store/chatStore.ts`, проводка и блокировка композера — `packages/ui/src/App.tsx` и `packages/ui/src/components/VoiceBar.tsx`.
...
Поле начинается с одной строки, `useAutoGrow` увеличивает и уменьшает его по фактическому содержимому до четырёх строк, после чего высота фиксируется и прокрутка остаётся внутри textarea. Независимый переключатель размера появляется при фактическом переполнении textarea, визуальном переносе длинной строки или явном переводе строки. Он расположен внутри правого верхнего угла `.composer-input`; textarea получает правый отступ, поэтому кнопка не перекрывает текст и соседние действия. Кнопка использует `DiagonalResizeIcon` из трёх синих квадратов, сообщает состояние через `aria-expanded`, доступное имя меняется между разворачиванием и сворачиванием длинного текста, а активация мышью или клавиатурой сохраняет тот же DOM-узел, фокус и выделение.

В единой строке постоянно доступны вложение, textarea, dropdown режима, микрофон и круглая кнопка отправки; наличие текста не скрывает микрофон. На десктопе элементы обычной строки центрируются по общей горизонтальной оси; однострочный текст и placeholder прижаты к нижней части поля перераспределением вертикальных отступов при неизменной высоте строки 40 px. Центральный VoiceBar позиционируется относительно `.main`, поэтому его центр совпадает с фактической областью сообщений и автоматически смещается при открытии или закрытии сайдбара. Мобильное и полноэкранное состояния сохраняют собственные отступы и выравнивание по нижнему краю. Источники поведения — `packages/ui/src/components/ChatColumn.tsx`, `packages/ui/src/components/VoiceBar.tsx`, `packages/ui/src/lib/autoGrow.ts` и `packages/ui/src/styles/app.css`.
