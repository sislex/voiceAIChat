...
Плюс `TaskModal` (хост рендерит им черновик задачи, предложенной моделью в чате),
`ProjectSettings` и `BrowserSessionPane`.
Итог: `index-` **775 КБ** против 1 336 КБ до выноса (−562 КБ, −42%).

**Состав чанка мерян по sourcemap, без новых зависимостей:**
`cd apps/web && npx vite build --sourcemap --outDir dist-map` — в `.js.map`
входного чанка лежат `sources` и `sourcesContent`, то есть точная атрибуция по
модулям. Именно так нашлись 85 КБ `TaskModal` и 130 КБ админки с профилем в
главном чанке, которые не были видны по одному размеру бандла.
...
**Достаточно одного значения из индекса пакета.** `parseAdminRoute`, взятый
статически из `@voicechat/admin-app`, возвращал в главный чанк всю админку
(70 КБ) и профиль (60 КБ), который она тянет: динамический
`import('@voicechat/admin-app')` обращается к тому же модулю через namespace, и
все его экспорты приходится сохранить. Поэтому чистая маршрутная логика переехала
в `@shared/adminRoute` (там же, где `issueKey`/`projectKey`), а пакет оставил
ре-экспорт для своего API. Правило сторожит тот же
`packages/ui/src/lazyScreens.test.ts` — список `LAZY_PACKAGES`.
