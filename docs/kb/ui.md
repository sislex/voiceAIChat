...
Проектный фильтр чатов — мультивыбор. `chatStore.sidebarProjectIds` содержит выбранные актуальные id, а `syncSidebarProjects` сверяет их с текущим списком проектов: удалённые id исключаются, каждый новый id автоматически добавляется. Полный актуальный набор означает «Все чаты» и включает разговоры без `projectId`; частичный набор включает только разговоры выбранных проектов, пустой — ни одного разговора. Закрытый контрол показывает «Все чаты», имя единственного проекта, количество нескольких проектов либо «Ничего не выбрано». В открытом меню проекты представлены независимыми checkbox; общая кнопка снимает полный выбор или выбирает все при частичном и пустом выборе, изменения применяются без закрытия меню.

Под `SIDEBAR_PROJECT_KEY` сохраняется JSON-снимок версии 1 с `selectedProjectIds` и `knownProjectIds`: второй массив позволяет при следующей синхронизации отличить новый проект от удалённого. Отсутствующее и повреждённое значение восстанавливает полный режим; прежняя простая строка мигрирует в одиночный выбор, если такой проект существует. Неизвестные id не попадают в нормализованное состояние и следующую запись.

`chatStore` получает полный результат `conversations:list`, `conversations:search` и `messages:search`, затем применяет один выбранный набор на клиенте; IPC-контракты не расширены. Reader-списки строятся из полного ответа до фильтрации. Смена набора немедленно обновляет состояние, перезапрашивает список и обесценивает устаревший поиск сообщений. Проектный фильтр больше не задаёт `projectId` нового черновика или разговора: принадлежность выбирается отдельным продуктовым действием.
