...
Storybook в общий гейт не входит, но `*.stories.tsx` проверяются `tsc`. Команды: `npm run -w @voicechat/ui storybook` (dev, порт 6006) и `npm run -w @voicechat/ui build-storybook` (смоук-сборка; чисто, если не считать двух предупреждений про `eval` внутри `@storybook/core`).
