...
**Адрес, пароль и просмотры публикации (п.25).** В диалоге публикации поля «Адрес» (slug → `/s/<slug>/`, ссылка и «Копировать» предпочитают его токену) и «Пароль» (пустое поле — не менять; «Снять пароль» шлёт `password: null`), счётчик «Просмотров». Всё уходит одним `make:publish` вместе с выбранным снимком по «Обновить публикацию».
...
**Шапка и диалоги Make после ревизии стилей.** В шапке остались только первичные действия режима (превью: ширина, выбор элемента, обновить, 💬; код: Проверить, + Файл, Сохранить; история: + Снимок, Место; компоненты: работать/📸/снимки) плюс стоимость, «Опубликовать», меню «⋯» (`aria-label="Ещё"`, `.make-more-menu` на базе `.jcard-menu`, закрывается кликом вне и Esc) и полный экран. В «⋯» ушли тема/язык превью, ♿, скриншот, «открыть в новой вкладке», загрузка/ассеты/токены/библиотека, галерея и ссылка на стори, идеи, шаблоны, импорт, скачать — их accessible-имена не менялись, поэтому в тестах перед кликом вызывается `openMore()`. Все диалоги Make получают `className="make-dialog"`: `Dialog` ui-kit не оборачивает содержимое, отступы 22px и читаемый `.fsub` (12.5px) даёт именно этот класс, поля `.tin` в диалогах — с рамкой. На ≤720px в шапке скрыты группа ширины превью и чип стоимости; превью обёрнуто в `.make-preview-body` (без него iframe терял высоту при закрытой панели комментариев).
