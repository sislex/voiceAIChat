...
Проектный фильтр чатов имеет три состояния, заданные `SidebarProjectFilter` в `packages/chat-app/src/contracts.ts`: `undefined` — «Все», `null` — «Без проекта», строка — конкретный проект. В селекте `Sidebar` пункты идут как «Все», доступные проекты и «Без проекта». Отсутствие `SIDEBAR_PROJECT_KEY` в preferences восстанавливает начальный режим «Все»; `SIDEBAR_NO_PROJECT_VALUE` (`__none__`) кодирует сохранённый режим «Без проекта», а прочая строка считается id проекта. При выборе «Все» ключ удаляется.

`chatStore` всегда получает полный результат `conversations:list` либо поиска по названиям `conversations:search`, затем применяет фильтр на клиенте: `undefined` оставляет все разговоры, `null` сопоставляется с отсутствующим или `null` `projectId`, строка требует точного совпадения. Reader-списки строятся из полного ответа до этой фильтрации. Смена значения немедленно обновляет состояние и перезапрашивает список; в режиме поиска по сообщениям она также запускает новый `messages:search` с тем же трёхзначным `projectId`.
...
`chatStore` допускает только одну неподтверждённую отправку: `pendingSubmit` ставится синхронно до первого `await`, поэтому повторный вызов не дублирует сообщение. При активном ходе новая реплика сохраняется сервером, но в клиенте до realtime-подтверждения видна только как оптимистичный элемент `queuedTurns`, без второго локального хода и без преждевременного появления в timeline. Подтверждение `claude.queue` снимает pending и оставляет серверный FIFO-порядок, после чего можно отправить следующую реплику. Успех очищает композер, ошибка отправки убирает оптимистичный элемент и возвращает исходный текст и вложения. Серверная очередь идемпотентна по `messageId`: завершение, ручная остановка или ошибка активного хода автоматически запускают следующий ожидающий элемент. Неуспешный элемент остаётся видимым со статусом `failed`, но не выбирается автоматическим FIFO-диспетчером и не блокирует следующие элементы. Источники — store пакета `packages/chat-app`, `apps/server/src/turns.ts` и методы очереди в `apps/server/src/db/database.ts`; регрессии закреплены в `packages/ui/src/store/appRuntime.chat.test.ts` и `apps/server/src/turns.test.ts`.
