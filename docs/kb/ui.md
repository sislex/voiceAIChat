...
`cssTokens.test.ts` эту ошибку не ловит по построению: он сторожит `var(--x)` **без** фолбэка (браузер отбрасывает такое свойство молча), а `var(--x, #fff)` для него валиден. Второй сторож — `styles/kanbanTokens.test.ts`: он падает на любом упоминании `--card-bg` и на `rgba(128, 128, 128, …)` в правилах канбана и карточки задачи.
