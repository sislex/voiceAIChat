...
выбирается независимо для каждой строки чата в сайдбаре: id машины, `null`
(сервер) или `'none'` (команды запрещены). Мост
...
или ответа. UI показывает его текстовой подписью и не даёт редактировать. Поле
