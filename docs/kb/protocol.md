...
Руками статус больше не меняют: селектор из карточки чата убран, и мост зовёт
только автозавершение хода (карточка подписана режимом чата, см.
[ui.md](ui.md#компоненты-и-поверхности)).
