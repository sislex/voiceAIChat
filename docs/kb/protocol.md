...
велика сама по себе». `CONTEXT_GROWTH_RATIO_NOTICE` (1,5 от среднего по
`turnSizes`, минимум `CONTEXT_GROWTH_MIN_TURNS` = 3 хода) — относительный: он
ловит сегодняшнюю правку, из-за которой каждый ход стал дороже, задолго до
абсолютного порога. Меньше трёх ходов среднее не считается: это была бы
случайная величина. Тест на относительный порог обязан задавать прошлым ходам
реалистичный размер — на фоне ходов по сорок символов базовая постоянная часть
«вырастает» всегда, и проверка «предупреждения ещё нет» падает. Третий —
`CONTEXT_HISTORY_TOKENS_NOTICE` (1000 токенов) плюс условие «история длиннее
постоянной части»: в таком разговоре совет «выключите источники» вреден, потому
что место занимают сообщения, и предупреждение зовёт в новый разговор или к
продолжению сессии движка.
...
Тот же контент по человекочитаемому адресу `GET /s/<slug>/*` (`MAKE_SLUG_PREFIX`, п.25): slug задаётся в `POST /api/make/:id/publish` (`{ snapshotId, slug, password }`; `slug`/`password` — `undefined` не трогать, `null` снять, строка установить; slug — `isValidMakeSlug`, занятый другим проектом → 409 `exists`). Публикация с паролем отдаёт на HTML-запросы форму (401), на остальное — 401 text; `POST /p/<token>/__auth__` и `/s/<slug>/__auth__` с urlencoded `password` ставят cookie `vc_pub_<token>` (sha256 от токена и хэша пароля — смена пароля разлогинивает всех) и редиректят на `?next`. Открытие `index.html` увеличивает `views` в `.publish.json`; там же (roadmap-3 п.3) ведутся `days` (просмотры по дням UTC, последние 90) и `referers` (хост из заголовка `Referer`, топ-20; заходы с хоста самого сервера и без реферера — не считаются) — наружу это `MakePublication.stats { days[], referers[] }`. Форма пароля ограничена `SlidingWindowLimiter` (10 попыток за 10 минут на `ip:token`, roadmap-2 п.3): сверх лимита `__auth__` отвечает 429 той же формой с текстом «подождите N с» и `Retry-After`; лимитер подменяется через `MakeRoutesDeps.passwordLimiter`. `MakePublication` содержит `slug`, `slugUrl`, `passwordProtected`, `views` и `history` (roadmap-2 п.11: записи `{ at, snapshotId, snapshotLabel }` добавляются, когда меняется, что отдаётся — снимок или «живая»; смена slug/пароля историю не пополняет; хранится до 30 последних); хэш пароля наружу не отдаётся.

**Дизайн карточки ↔ Make (2026-08-31).** REST: `REST.taskDesigns` / `REST.taskDesign`
(`GET/POST /api/projects/:id/tasks/:taskId/designs`, `DELETE …/designs/:linkId` → `TaskDesignLink[]`),
`REST.projectDesignSources` (`GET /api/projects/:id/design-sources` → `ProjectDesignSource[]`),
`REST.makeTaskLinks` (`GET/POST /api/make/:id/task-links`, `GET …/task-links/tasks`,
`DELETE …/task-links/:linkId`). Мосты: `tasks:designs/linkDesign/unlinkDesign`,
`projects:designSources`, `make:taskLinks/linkTask/linkableTasks`. Адрес превью страницы
...
**Persist-коллекции (roadmap-2 п.12).** Файл `mock/<база>.json` вида `{ "$collection": true, "$body": [ … ] }` превращает `api/<база>` в CRUD: `GET` — список, `POST` — добавить (id генерируется, если не передан) → 201; `api/<база>/<id>`: `GET`/`PUT`/`PATCH`/`DELETE` (204). Каждая мутация переписывает файл через `workspaces.write` (→ `make.changed`, снимок за ход не делается). Разбор пути — `collectionCandidates` (`@shared/makeMock`): сначала сам путь как коллекция, затем родитель + последний сегмент как id. На публикации (`/p/`, `/s/`) коллекции только читаются — не-GET отвечают 405: анонимные посетители не пишут в проект. Проверяется раньше обычных `mock/<путь>[.METHOD].json`; тело запроса передаётся из превью-маршрутов (`resolveMock(..., body)`).

**Квота и очистка (п.30).** `MAKE_LIMITS.maxProjectBytes` (64 МБ) считается по файлам + `.snapshots` + `.shots`; превышение при записи — `MakeError('quota')` → 413 с подсказкой открыть «Место». `GET /api/make/:id/usage` → `MakeUsage` (байты/счётчики по составляющим, `unusedAssets` — бинарники, имя которых не встречается ни в одном текстовом файле ≤ 512 КБ). `POST /api/make/:id/cleanup` `{ keepSnapshots?, shots?, unusedAssets? }` → `MakeCleanupResult { freedBytes, removed, usage, state }`; снимок, закреплённый в публикации, не удаляется; после очистки рассылается `make.changed`. Мосты `make:usage` / `make:cleanup`. Квота на пользователя (roadmap-2 п.15): `MAKE_LIMITS.maxUserBytes` (512 МБ) — сумма `usage.totalBytes` по всем Make-проектам владельца (`setProjectsOfOwner` в `server.ts` через `db.listConversations`), считается перед `writeBuffer`/`importFiles` с кэшем 60 с и сбрасывается после записи; превышение — тот же `MakeError('quota')` → 413. `AdminMakeStats.userLimitBytes` — для предупреждения в админке (строка пользователя ≥ 80 % подсвечивается).

**Комментарии к элементам превью (п.32).** `GET/POST /api/make/:id/comments`, `PATCH/DELETE /api/make/:id/comments/:commentId` → `{ comments: MakeComment[] }` (`id, selector, elementLabel, text, author = uid, createdAt, resolved`); хранятся в `.comments.json` проекта (переживает `reset`, как `.publish.json`), лимит 500. Любая мутация комментариев рассылает `make.changed` с `paths: ['.comments.json']` (`MAKE_COMMENTS_SYNC_PATH`) и текущим `rev` — панель в других вкладках перечитывает список, не перезагружая превью (roadmap-2 п.7). Мосты `make:comments` / `make:commentAdd` / `make:commentUpdate` / `make:commentRemove`. **Presence вкладок (roadmap-2 п.14).** `POST /api/make/:id/presence { clientId, path, editing, leave? }` — heartbeat вкладки (панель шлёт раз в 15 с и при смене файла/грязности, `leave: true` при размонтировании); `MakeHub.heartbeat` держит живые вкладки в памяти (TTL 45 с) и рассылает всем сокетам владельца WS-кадр `make.presence { conversationId, clients: MakePresenceClient[] }` (`clientId, user, path, editing, at`). Мост `make:presence`, событие — `window.make.onPresence` (необязательное у старых хостов). Инспектор превью понимает два новых сообщения родителя: `vc-make.pins { items: [{ selector, n, text, resolved }] }` — рисует нумерованные метки поверх элементов (fixed-слой, пересчёт на scroll/resize) и `vc-make.highlight { selector }` — прокрутка к элементу и красная обводка на 1,6 с.
