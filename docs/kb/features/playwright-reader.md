...
Это важнее деталей: **сквозного режима ещё нет**. Реализованы тип разговора,
маршрут с левой панелью чата, shared-контракты браузерной сессии и отдельный
сервис `apps/browser-runner`. Правая панель — статичная заглушка, а сервис ни
откуда не вызывается: в `apps/server` нет ни одного упоминания browser-runner, в
`packages/ui` нет моста к нему, экранов сессии нет. Не реализованы: серверная
оркестрация сессий (REST/WS, проверка владения разговором, service-токен
раннера), screencast и передача кадров, пользовательский ввод в Chromium, DOM/
accessibility snapshot, highlight, отдельный MCP-namespace и его подключение к
ходу, confirmation gates опасных действий, метрики и настоящий health-probe,
idle-timeout и retention профилей, тесты (ни одного файла с `playwright-reader` в
тестах нет).
