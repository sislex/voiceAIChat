...
сценария и экспорт в Playwright. В быстром режиме его действия исполняются в проксированной странице
пользовательской панели; в полном режиме и Playwright Reader они передаются отдельному Chromium через
...
Screenshot берёт page.url/title из метаданных самого кадра; для старого раннера используется status. Состояние управления дополнительно читается через status; status/screenshot
...
поллингом (400 мс после действия, затем 1200 мс). Профиль Reader сохраняется; потоковый транспорт кадров остаётся отдельной работой.
...
`stale_element_ref`. После навигации/замены узла нужен новый find. По умолчанию find исключает скрытые
копии до лимита; visibleOnly:false включает их явно, total/truncated сохраняются. Для upload
скрытый file input допустим, но он также должен быть единственным. scrollTo,
read/a11y, hover, set и drag используют ту же проверку однозначности.
Обычный wait видимой цели тоже строгий; явные count/hidden/detached сохраняют
смысл ожидания набора элементов или отсутствия цели.
...
чтение журналов/каталога скачиваний и ответы на JavaScript-диалог обходят очередь:
ожидающий prompt не должен блокировать собственный ответ. Выбор другой вкладки
сериализуется, но не блокируется диалогом прежней вкладки. Команды control/cancel разрешены только actor=user: модель не
...
Кнопки прерывания и отмены постоянно занимают место в панели: изменение busy
...
Сочетания клавиш, paste, диалоги, адресация frame, popup и скачивания уже
поддерживаются в объединённой реализации; их ограничения и регрессии описаны
в тематических разделах ниже. `reset-session` для Chromium вызывает очистку
хранилища раннера, затем сбрасывает cookie прокси. Stop сохраняет профиль Reader.
...
и удаляет только одноразовые QA-профили; профиль Reader сохраняется.
Screenshot/status также обновляют lastUsedAt.
...
### Скачивания и вложения: цикл проверки14 (2026-09-10)

`BrowserDownloads` слушает download каждой вкладки до навигации. Chromium
запускается с acceptDownloads:true; ранее HTTP и Blob-файлы отклонялись настройкой
контекста. downloads возвращает id, filename, публичный URL, tabId, startedAt,
state (downloading/completed/failed/canceled), completedAt, размер готового файла
и ошибку. В метаданных сессии — ограниченный каталог и downloadCount. Полный
перечень можно дочитать через offset/nextOffset, в том числе для закрытой вкладки,
если её файл ещё хранится. Произвольный чужой или устаревший id недоступен;
авторизация разговора и incarnation проверяются обычным путём Reader.

MCP downloads/read-download/cancel-download/delete-download входят в allow-list
Claude. read-download отдаёт UTF-8 текст частями до12000 символов (offset в UTF-16
позициях) или исходные байты в base64 до8192 байт для модели. Текстовые порции
не разрывают surrogate pair и уменьшаются по JSON-бюджету; продолжение задаёт
nextOffset. Текстовое чтение ограничено8 МиБ, бинарный/не-UTF-8 файл можно получать
base64. Пустой готовый файл подтверждается пустой порцией; незавершённый файл не
выдаётся за пустой. Старое ready не считается подтверждением каталога, чтения,
отмены или удаления. Файловый путь раннера не передаётся ни модели, ни панели.

Панель «Скачивания» показывает состояния и ошибки, сохраняет файл на устройство,
отменяет поток и удаляет временные данные. Сохранение собирает подтверждённые
непрерывные base64-порции по512 КиБ и только затем создаёт Blob/ссылку скачивания;
неверный offset, чужой id, потерянная порция, ошибка или уход с панели не сохраняют
обрывок. Имена ограничены180 символами, разделители пути/управляющие и bidi-символы
заменены. URL сокращается до2000 с urlTruncated; blob-origin восстанавливает алиас.

Лимиты сессии: файл64 МиБ, восемь незавершённых скачиваний, каталог32 записи.
Живой лимит слушает Browser.downloadProgress и отменяет превышающий объём поток;
быстрый файл дополнительно проверяется stat после завершения. В собственной
browser CDP session нужно вызвать Browser.setDownloadBehavior(eventsEnabled:true,
allowAndName) с тем же downloadsPath, который передан launchPersistentContext:
без этого эта CDP session не получает progress. Отмена, опередившая native finish,
показывается canceled; уже завершившийся файл остаётся completed. Старые завершённые
записи и их файлы удаляются при вытеснении из каталога.

Каталог файлов reader-downloads-UUID внутри профиля удаляется при закрытии, даже
в persistent-режиме. После успешного захвата Chromium-профиля новый старт очищает
старые UUID-каталоги после аварии; папки другого имени и symlink не затрагивает.
Авторизация и остальные данные профиля сохраняются по правилам предыдущего раздела.
Переход/back/forward/reload/new-tab к вложению признаётся скачиванием только при
ошибке Download is starting и подтверждённом событии download: это не успех загрузки
HTML-страницы, текущая страница и каталог показывают фактический результат.

Интеграция с собственным Make: интерактивные превью требуют allow-downloads в
sandbox iframe. Без него вызванный кнопкой Blob-экспорт не создавал файл, хотя
обычные вкладки Reader уже скачивали. Исправлены основной/синхронный/соседний
предпросмотр, общий просмотр и превью компонентов; исторические/тестовые iframe
с отдельными ограничениями не расширены. E2E проверяет экспорт из настоящего
/api/preview/make iframe через MCP и чтение файла моделью.
...
отказ одного действия не блокирует следующие. Дополнительная очередь сессии
сериализует input вместе с selector/evaluate модели; отдельная очередь Page
сохраняет атомарность составного drag или модифицированного клика.
...
Ограничение, подтверждённое аудитом 2026-09-10 и пока не исправленное:
raw `resize` меняет `session.viewport` до успешного `setViewportSize`, поэтому
отрицательная ширина после ошибки остаётся в метаданных. Изменение
`deviceScaleFactor` отражается в метаданных, но не в `window.devicePixelRatio`
текущих и новых вкладок: `setViewportSize` меняет только размеры. Начальный DPR
контекста задаётся отдельно. Не использовать метаданные resize как доказательство
изменившейся плотности пикселей.
...
Этот отказ относится к переводу действий модели. В raw-командах раннера пока
остался пробел: неизвестный `command.type` возвращает обычные метаданные ready,
а неизвестный inspect kind с selector попадает в чтение styles. Воспроизведено
2026-09-10; это неисправленные случаи, не поддержка дополнительных команд.
...
писать не пришлось. С 2026-09-10 (цикл проверки16) `browserEvaluation.ts` задаёт
код до4000 символов и timeoutMs100–15000, по умолчанию5000. Пустой код и неверный
лимит отвергаются до исполнения. Явный timeout в MCP не уходит в fallback
Web Reader; старый раннер без valueFormat/elapsedMs не подтверждает расширение.

`evaluation.ts` сначала получает JSHandle контейнера, затем запускает код.
Так даже синхронный бесконечный цикл оставляет ссылку для отмены ожидающего RPC.
По deadline раннер посылает Runtime.terminateExecution, завершает внутреннее
ожидание через cancel и освобождает handle/CDP-сессию/таймер. Поздно пришедший
handle также освобождается, код после timeout не запускается. Для OOP iframe
используется его CDP-сессия, для обычного — ближайший родитель с отдельным
renderer. Во вкладке допускается один evaluate; параллельный запрос получает
явный отказ. JS-диалог приостанавливает расход бюджета до ответа, не отвечает
автоматически. elapsedMs включает фактическое ожидание. Timeout возвращает
ok:false/timedOut и поясняет, что уже начатые асинхронные fetch/timers страницы
могут продолжиться: перед повтором нужно проверить состояние.

`evaluationValue.ts` сериализует значение внутри renderer. Обычный JSON остаётся
структурой (`valueFormat: json`); undefined, BigInt, особые числа, Date, RegExp,
Error, Map/Set, бинарные массивы и функции имеют $type; циклические/повторные
ссылки — $ref: JSON Pointer от корня ответа (например, /value/entries/0/1),
с экранированием slash/тильды и точным путём в представление Map (`preview`). DOM-узел возвращает tag/id/role/label/text/connected,
не value поля password. Getter обычного объекта не вызывается. Ограничения:
500 узлов, глубина8,100 ключей/элементов,4000 символов строки; truncated сообщает
сокращение. Итоговый бюджет20000 JSON-символов учитывает экранирование. Если
структура всё ещё велика, valueFormat=preview-json явно обозначает неполный
JSON-префикс; точную часть можно запросить отдельным выражением. Тело хранится
как JS-строка, чтобы транспиляция tsx не добавляла helper __name в renderer.

`BrowserDialogs.title` даёт кешированный заголовок через200мс, если renderer
занят; повторные status разделяют один незавершённый запрос title. Это сохраняет
доступ к вкладкам и управлению во время зависшего JavaScript. Проверки:
`sessionEvaluation`, `evaluationLifecycle` (включая строгие CSP/Trusted Types),
shared/MCP и отдельный запуск tsx+Chromium. E2E прерывает sync/async внутри
настоящего Make iframe и подтверждает ручной prompt после паузы дольше deadline.
...
`stop-loading` пока вызывает `window.stop()` через JavaScript страницы. Он ждёт
освобождения занятого renderer: при бесконечном evaluate с лимитом1000мс команда
stop в локальном Chromium заняла975мс. Это ограничение не исправлено циклом16;
прерывание собственного evaluate и остановка загрузки — разные операции.
...
У HTTP-клиента остаются два подтверждённых ограничения (аудит 2026-09-10).
Таймер `call` снимается после получения заголовков; последующие `json()` и
`arrayBuffer()` уже не ограничены этим deadline. Локальный сервер с немедленными
заголовками и телом через250мс успешно ответил при timeout80мс за253–262мс.
Listener внешнего AbortSignal после успешного запроса не снимается: три вызова
с одним signal оставили1/2/3 обработчика. Исправления не выполнены; остановка
работы после цикла16 запрошена пользователем.
...
С 2026-09-10 (цикл проверки15) `BrowserDiagnostics` регистрирует консоль,
исключения и request/response/finished/failed с момента создания каждой вкладки.
Два кольцевых буфера ограничены200 записями. Каждая строка содержит id, sequence,
tabId, pageUrl, а сеть — также frameUrl, resourceType, state и durationMs.
`failed` сохраняет причину даже без HTTP-ответа (status0); HTTP>=400 также входит
в failedOnly. Перенаправления связаны redirectedFrom/To; 3xx не ошибка.
Навигация, переданная скачиваниям, помечается download и не выдаёт ложный
ERR_ABORTED; итог файла проверяется каталогом downloads.

По умолчанию журналы показывают историю активной вкладки. tabId выбирает другую
(включая закрытую, пока её строки в буфере), allTabs включает общий обзор.
frame для журналов отвергается: источник вложенного документа виден в строке.
Адреса сети, source и стека разворачиваются обратно в публичные алиасы.
Native warning нормализуется в warn, assert — в error; sourceType сохраняет
исходный вид события. Модельный console трактует pattern как буквальную
подстроку без учёта регистра; raw inspect сохраняет прежний regex по умолчанию,
`regex:false` явно выключает его. MCP не предоставляет regex.

Тело сериализатора args хранится как JS (`new Function`), чтобы `tsx` не
добавлял недоступный в renderer helper `__name`; это проверяет отдельный
`diagnosticsRuntime.test.ts`, запускающий настоящий процесс tsx+Chromium.
Console args читаются отдельно, максимум6 аргументов, глубина4,100 узлов,
20 ключей и1000 символов строки; getter не вызывается. `argsPending`,
`argsUnavailable`, `argsTruncated` обозначают неполноту. Через500мс ожидание
renderer прекращается, поэтому JavaScript-диалог не блокирует чтение журналов.
Текст ограничен2000, стек4000 символами, отдельная строка JSON —10000.

`readBrowserDiagnostics` выдаёт хвост в хронологическом порядке с бюджетом24000
JSON-символов, total/returned/truncated и nextBefore для старых порций. since
принимает cursor предыдущего ответа: завершение запроса и дочитанные console
args обновляют sequence. Это журнал текущих состояний, а не неизменяемый поток
всех промежуточных событий. dropped сообщает вытеснение всего буфера до фильтра.
clear удаляет ровно возвращённые строки, сохраняя другие вкладки, уровни и
непрочитанные порции; невалидные параметры не меняют данные.

MCP console/network принимает scope, cursors и расширенные сетевые фильтры
только для Chromium; fallback Web Reader получает явный отказ. Старый раннер
без cursor не подтверждает расширенное чтение. Панель фиксирует tabId на оба
запроса, игнорирует поздние ответы после смены вкладки/сессии и скрытия,
показывает источник, сетевую причину, время и частичность выдачи. Ошибка HTTP,
ok:false или старый ready не выдаются за «Страница не жаловалась»; есть повторное
чтение. Проверки — sessionDiagnostics, diagnosticReading, shared, MCP, DOM и
сквозные случаи с реальной страницей проекта в e2e/playwrightReader.

Стили пока не имеют такого же бюджета, как журналы и evaluate: custom property
длиной50000 символов возвращается целиком и не помещается в MCP resultJson.
Псевдоэлемент не входит в контракт styles; неизвестное raw-поле pseudo игнорируется,
и `getComputedStyle` читает основной элемент. Оба ограничения подтверждены
локальным Chromium 2026-09-10 и пока не исправлены.
...
- **`errors` из словаря модели — это консоль с уровнем `error`**, отдельного
  журнала не заводили.
- **Callback чтения styles объявляет `getComputedStyle` локальным узким
  типом**: у пакета нет библиотеки DOM (это Node-сервис), а подключать её ради
...
Chromium. Ограничения первого круга не описывают текущий словарь:
...
## Клавиатура, фокус и буфер обмена (круг 1 паритета с пользователем, 15.09.2026)

Словарь модели умел мышь и текст, но не умел того, чем человек пользуется
постоянно: дойти до элемента табом, нажать сочетание, выделить и вставить.
Добавлены действия `hotkey`, `focus`, `focusOrder`, `clear`, `selectText`,
`copy`, `paste`, параметр `repeat` у `press` и `delay` у `type`
(`BrowserSelectorAction`/`BrowserInputAction` в shared, исполнение —
`apps/browser-runner/src/focusActions.ts` и `selectorActions.ts`, инструменты —
`previewMcp`: `hotkey`, `focus`, `focused`, `focus-order`, `clear`,
`select-text`, `copy`, `paste`).

Что здесь стоит помнить:

- **`primary` вместо `ctrl`.** Модификатор сочетания `primary` переводится в
  `ControlOrMeta` Playwright. На macOS `Control+A` уводит курсор в начало строки
  вместо выделения, и для модели это выглядело как молчаливый отказ: команда
  успешна, выделения нет. Проверено вживую на локальном раннере 15.09.2026.
- **Сочетание — одно `keyboard.press('Control+a')`,** а не тройка down/press/up:
  Playwright сам отпускает модификаторы, поэтому упавшее нажатие не может
  оставить Control зажатым до конца сессии.
- **`focus` без селектора не двигает фокус,** а сообщает текущий: обход формы
  табом — цикл «нажал → посмотрел, куда попал», и ответ, меняющий фокус, ломает
  его. В ответе — `visibleRing` (нарисовано ли кольцо) и `withinDialog`: уход
  фокуса из открытого модального окна это классическая ловушка клавиатуры.
- **`type` с `delay` сначала очищает поле и печатает посимвольно.** `fill()`
  ставит значение целиком, и автодополнение, поиск с задержкой и логика
  «по одному нажатию» страницы не срабатывают вовсе.
- **`copy` читает и выделение внутри поля ввода:** выделение input/textarea не
  попадает в `window.getSelection()`, поэтому проверяется `selectionStart/End`
  активного элемента.
- **`paste` шлёт настоящее событие `paste` с `clipboardData`** и только при
  отказе страницы обрабатывать его пишет значение сам. Редакторы, которые читают
  именно событие, иначе не видят вставку.
- **`focusOrder` повторяет порядок браузера:** положительный `tabindex` идёт
  первым по возрастанию, остальные — в порядке DOM. Так видно недостижимые
  кнопки, не нажимая Tab по разу.

## Формы целиком (круг 2 паритета с пользователем, 15.09.2026)

Модель заполняла форму полем за вызов, а причину отказа узнавала только по тому,
что страница решила нарисовать. Добавлены `fillForm`, `formState`, `validity`,
`submit`, `options`, `dropFile`, `set` с несколькими значениями и `upload`
нескольких файлов (`apps/browser-runner/src/formActions.ts`, инструменты
`fill-form`, `form-state`, `validity`, `submit`, `options`, `drop-file`).

- **Форма заполняется одним вызовом,** потому что управляемые поля React
  перерисовываются между обращениями и теряют введённое раньше. Ответ содержит
  `filled` с результатом по каждому полю, а частичный успех возвращается как
  `ok: false` — иначе модель жмёт «Отправить» и разбирается уже с ошибками.
- **`validity` спрашивается до `submit`.** Браузер уже знает, какое поле
  блокирует отправку и почему (`valueMissing`, `typeMismatch`, `rangeOverflow`),
  и говорит это на языке пользователя.
- **`submit` зовёт `reportValidity` перед `requestSubmit`** — ровно то, что
  делает браузер при нажатии Enter, поэтому человек и модель видят одно сообщение.
- **`formState` не возвращает значение поля пароля:** оно попало бы в журнал хода.
- **`drop-file` шлёт настоящие `dragenter/dragover/drop` с `DataTransfer`.**
  Загрузчики без `input[type=file]` слушают только drop, и `setInputFiles`
  до них не достаёт.
- **Бюджет загрузки общий на все файлы** (8 МиБ): он упирается в память раннера,
  а не в число файлов. Разбор base64 у `upload` и `drop-file` один и тот же —
  `Buffer.from` молча проглатывает мусор, и без сверки страница получала другой
  файл, а модель `ok`.

## Дойти до содержимого (круг 3 паритета с пользователем, 15.09.2026)

Чтение было плоским, а прокрутка слепой: на ленте с ленивой подгрузкой модель
останавливалась на первом экране либо крутила колесо вслепую, а причину «клик не
сработал» (элемент перекрыт липкой шапкой) узнать было нечем.

Добавлены `scrollUntil`, `count`, `table`, `list`, `metrics`, `measure`,
`highlight` (`apps/browser-runner/src/contentActions.ts`) и два условия
ожидания — `network: 'idle'` и `stable`.

- **`scroll-until` делает паузу 250 мс между шагами:** лента подгружает
  содержимое после прокрутки, и без паузы следующий шаг упирается в тот же низ.
  Отказ различает «дошли до конца» и «не уложились в maxScrolls».
- **`table` берёт заголовком первую строку с `th`, иначе первую строку вообще.**
  Так размечает таблицу большинство страниц; угадывание здесь честнее пустого
  ответа, а связь ячейки с заголовком — то, что теряет плоский `read`.
- **`list` отдаёт кнопки внутри каждого блока** с их селекторами: раньше модель
  читала ленту текстом и нажимала кнопку соседней карточки.
- **`measure` сообщает `covered` и `coveredBy`** — узел, который лежит в точке
  клика поверх цели. Это самая частая причина промаха, и по дереву её не видно.
- **`metrics` считает «экраны», а не пиксели:** «ниже ещё 4 экрана» понятно и
  человеку, и модели, а 9000 px не говорят ничего.
- **`wait` с `network: 'idle'` — то, чего человек ждёт, глядя на спиннер;**
  `load` завершается раньше, чем приедут данные SPA. `stable` сравнивает две
  одинаковые рамки подряд: у меню анимация идёт уже после появления в DOM.

## Среда браузера (круг 4 паритета с пользователем, 15.09.2026)

Модель управляла страницей, но не условиями вокруг неё. Добавлены команды уровня
сессии `environment` и `cookies` (`apps/browser-runner/src/environmentActions.ts`),
селекторное действие `media` и инструменты `emulate`, `cookies`, `media`.

- **Эмуляция — уровень контекста, а не страницы.** `setOffline`,
  `setGeolocation` и разрешения живут на контексте, а `emulateMedia` — на
  странице, поэтому тема применяется ко всем открытым вкладкам и к каждой
  открытой позже (`applyEnvironmentToPage` в обработчике `context.on('page')`).
  Иначе тёмная тема действовала бы на одну вкладку и выглядела случайной.
- **Координаты выдаются вместе с разрешением `geolocation`:** без него страница
  получит отказ, хотя позиция задана, — а спросить человека, как сделал бы
  браузер, здесь некого.
- **Значение cookie сокращается до 4 символов и длины.** Это доступ к аккаунту,
  и в журнале хода ему не место; имя, домен и флаги остаются целиком.
- **Удаление одной cookie возвращает остальные на место:** Playwright умеет
  только `clearCookies()`, и без восстановления «убрать одну» разлогинивало
  всю сессию.
- **`e2e/webReaderProject` нестабилен под нагрузкой:** `fill` внутри двойного
  iframe ждёт 10 с, и при занятой машине (соседний Chromium, параллельный гейт)
  падает, выглядя как регрессия. 15.09.2026 на этом потеряли полчаса: тест
  падал 2 прогона из 3, а после остановки фоновых процессов прошёл 9 из 9 без
  единой правки кода. Прежде чем искать причину в диффе — повторите прогон на
  спокойной машине.
- **`media` возвращает отказ автовоспроизведения причиной.** `play()` без
  жеста пользователя отклоняется — это не сбой раннера, а ровно то, что увидел
  бы человек.

## Лента сессии и проверки (круг 5 паритета с пользователем, 15.09.2026)

Модель работала молча: человек смотрел на кадр и не понимал ни намерения, ни
порядка шагов. Раннер получил ленту сессии (`SessionHistory` в
`apps/browser-runner/src/sessionHistory.ts`), команды `history` и `note`,
селекторное действие `expect` и одноимённые инструменты.

- **Лента ведётся в раннере, а не в ядре или панели:** только он видит обе
  стороны — человека в панели и модель через MCP.
- **Запись ведётся вокруг исполнения** (`dispatch` вынесен из `command`),
  поэтому неудачная команда попадает в ленту с причиной. Именно на ней человек
  понимает, где модель застряла, — успешные шаги не спрашивают.
- **Чтение страницы в ленту не попадает.** `read`, `find`, `metrics`, `count`,
  `table`, `formState` и прочие вызываются десятками за ход, и лента человека
  превратилась бы в поток «прочитал, нашёл, прочитал».
- **Хвост ленты (20 записей) едет в метаданных сессии:** панель показывает её
  без отдельного запроса, который стоил бы ещё одного обращения на каждый кадр.
- **В ленте нет полезной нагрузки команд** — только короткие формулировки: на
  страницах бывают пароли, а полный дамп команд хранил бы их.
- **`expect` — намеренно узкий набор проверок** (текст, видимость, число,
  значение поля, адрес): это то, что человек говорит об экране вслух, а не
  язык выражений. Ответ показывает, что на странице вместо ожидаемого.

## Данные страницы (круг 6 паритета с пользователем, 15.09.2026)

Модель видела страницу, но не её данные. Добавлены селекторные действия
`storage`, `source` и `csv` (`apps/browser-runner/src/contentActions.ts`) и
одноимённые инструменты.

- **`storage` намеренно не `evaluate`.** Хранилище читалось только выполнением
  кода, а его политика проекта гейтит как опасное действие — в закрытом проекте
  модель не могла посмотреть даже флаг темы. Отдельное действие с узким
  контрактом (`read`/`set`/`remove`/`clear`) проходит там, где `evaluate` нет.
- **Значение режется до 500 символов, но `bytes` остаётся полным:** в
  localStorage лежат и токены, и черновики на сотни килобайт.
- **`source` дополняет `read`, а не заменяет:** `read` даёт содержимое глазами
  человека, `source` — то, что на самом деле в документе (атрибуты, `data-*`,
  скрытые поля, порядок узлов).
- **`csv` экранирует кавычки удвоением и берёт поле в кавычки при запятой или
  переводе строки** — иначе таблица с адресами ломает разбор у получателя.

## Почему действие не сработало (круг 7 паритета с пользователем, 15.09.2026)

`apps/browser-runner/src/actionErrors.ts` разбирает отказ Playwright на вид,
причину и **один** следующий шаг; к отказам «не найден», «таймаут» и
«неоднозначно» прикладываются похожие элементы страницы.

- **Совет — это действие, а не диагноз:** перекрытие отправляет в `measure`
  (он назовёт перекрывающий узел), исчезнувший элемент — искать заново,
  таймаут — ждать `network: 'idle'`. Раньше модель получала первую строку
  Playwright и повторяла тот же клик.
- **Кандидаты ищутся с запасом:** равенство, вхождение и общее начало подписи —
  «Сохранить» и «Сохранить как» человек считает одним и тем же.
- **Признак отсутствия ищется подстрокой `не найден`,** потому что селектор
  попадает внутрь фразы («Элемент #save не найден»).
- **Незнакомая ошибка отдаётся как есть.** Выдуманный совет хуже отсутствующего:
  модель пойдёт не туда с полной уверенностью.
- **Панель показывает ту же формулировку**, что получает модель, и кнопки с
  кандидатами; отказ селекторного действия (`ok: false` значением, а не
  исключением) раньше вообще не показывался человеку.

## Сценарий со стороны модели (круг 8 паритета с пользователем, 15.09.2026)

Человек в панели записывает свой проход и получает воспроизводимые шаги — ради
этого Reader и делается инструментом автотестов. У модели этого не было, хотя
путь чаще всего проходит именно она: в задаче канбана, где потом нужен автотест.

`apps/web-reader/src/mcp/turnRecorder.ts` — запись, инструменты `record`,
`record-check` и `replay`.

- **Запись живёт в MCP-слое, а не в раннере.** Здесь известно исходное действие
  модели (`PreviewAction`); до раннера доезжает уже команда Chromium, из которой
  селекторный шаг обратно не собрать.
- **Читающие действия в запись не идут:** прогонять `read` или `screenshot`
  нечего, а сценарий из них состоял бы наполовину.
- **Проверка прикрепляется к последнему шагу,** а не становится своим: в
  сценарии «нажал — увидел» это одно событие, и ждать текст нужно сразу после
  действия.
- **Потолок 100 шагов с честным счётчиком пропущенного** и вытеснение старых
  записей (50 разговоров): процесс один на всех, и молча растущая память тут
  никому не нужна.
- **`replay` останавливается на первом упавшем шаге:** дальше страница уже не
  та, и следующие отказы ничего не объясняют.
- **В панели шаг можно поднять, опустить и выключить** (выключенный остаётся в
  записи, но в сценарий не уезжает), а сам сценарий — вставить из JSON.

## Настоящий мобильный веб (круг 9 паритета с пользователем, 15.09.2026)

`apps/browser-runner/src/deviceActions.ts`: команда `device` эмулирует
устройство целиком, команда `touch` выполняет жесты пальцем.

- **Эмуляция идёт через CDP, а не через Playwright-контекст.** `hasTouch` и
  `isMobile` задаются только при создании контекста, а у нас он персистентный —
  в нём живёт авторизация сессии, и пересоздавать его ради «посмотреть на
  телефоне» нельзя. `Emulation.setDeviceMetricsOverride` +
  `setTouchEmulationEnabled` + `setEmitTouchEventsForMouse` работают на живой
  странице.
- **Без обеих команд тача `maxTouchPoints` остаётся нулевым:** страница считает
  посетителя мышью, и мобильные дефекты не находятся вовсе.
- **Свайп отправляется шагами (8 кадров по 16 мс).** Страница со «свайпом для
  удаления» считает мгновенный перенос пальца сбоем и жест не засчитывает.
- **CDP-сессия эмуляции живёт вместе со страницей и не отсоединяется.**
  `Emulation.*` привязан к сессии: `detach()` снимает всю эмуляцию, и телефон
  превращался обратно в десктоп ровно в тот момент, когда команда отвечала
  «готово». Проверено вживую 15.09.2026: `390/1/0` вместо `390/3/5`.
- **`setTouchEmulationEnabled` не принимает `maxTouchPoints: 0`** — Chromium
  отвечает протокольной ошибкой «Touch points must be between 1 and 16»;
  при выключении поле не передаётся вовсе.
- **`setEmitTouchEventsForMouse` включать нельзя.** Он превращает мышиные
  события в тач, и обычный клик Playwright (а с ним и клик человека по кадру)
  перестаёт доходить до страницы: E2E ловил это таймаутом `locator.click` через
  два круга после введения эмуляции. Тач-эмуляции достаточно
  `setTouchEmulationEnabled`, а настоящие жесты идут `Input.dispatchTouchEvent`.
- **`setViewportSize` вызывается ДО CDP-оверрайда:** внутри он сам шлёт
  `setDeviceMetricsOverride` со своими параметрами и затирает тач и плотность.
- **Эмулируемая плотность пикселей не попадает в `session.viewport`.** Она нужна
  странице, а не рендеру: иначе профиль сохранял бы ×3, контекст пересоздавался
  в тройном разрешении, и кадр телефона весил бы как четыре десктопных. Именно
  на этом E2E ловил кадр 3840 px при окне 1280.
- **Панель переключает пресет командой `device`,** а при отказе старого раннера
  откатывается на `resize`: человек всё равно увидит мобильную вёрстку.
  Ответ команды — метаданные сессии, поэтому панель обновляет размер и признак
  тача одним ответом, без отдельного запроса статуса.

## Просьба к человеку (круг 10 паритета с пользователем, 15.09.2026)

Команда `ask` (и ответ `answer`) в `sessionManager`: модель просит человека
сделать то, что не должна делать сама — ввести код из СМС, пройти капчу, войти
паролем из менеджера — и ждёт ответа одним вызовом.

- **Просьба и ответ идут мимо очереди команд.** Иначе просьба блокирует ровно
  того, кого просят: команды человека встали бы за ожидающей просьбой модели, и
  он не смог бы ни ввести код в браузере, ни нажать «Сделал». Поймано тестом
  `sessionAsk.test.ts` сразу после первой реализации.
- **Отказ — это ответ, а не ошибка:** `done: false` возвращается моделью как
  нормальный результат, и она может выбрать другой путь.
- **Отвечать может только человек** (`actor: 'user'`), иначе модель закрыла бы
  собственную просьбу и пошла дальше.
- **Просьба живёт в метаданных сессии**, поэтому панель показывает её над
  кадром — там же, где экран, которого она касается, — а не в переписке.

## Сеть под контролем (круг 11 паритета с пользователем, 15.09.2026)

`apps/browser-runner/src/networkRules.ts` + команда `network-rules`: подменить
ответ, заблокировать запрос, задержать его — то, что человек делает в devtools.

- **Правила применяются до проверки адреса** в общем `context.route('**/*')`:
  подменённый ответ вообще не уходит в сеть, а заблокированный запрос не спорит
  с политикой доступа — его просто нет.
- **Правило с тем же шаблоном заменяется, а не добавляется вторым:** иначе
  порядок решает, какой ответ увидит страница, и проверка невоспроизводима.
- **Подмена по умолчанию — JSON:** подменяют почти всегда ответ API, и без типа
  страница разбирает его как текст и падает не там, где интересно.
- **Список правил не отдаёт тело целиком** (до 200 символов): тело бывает в
  сотни килобайт, а в ответе модели ему не место.
- **Правила живут до конца сессии**, поэтому панель показывает их человеку и
  даёт снять: забытая подмена выглядит как сломанный сайт.

## Снимок «до» и сравнение «после» (круг 12 паритета с пользователем, 15.09.2026)

`apps/browser-runner/src/snapshots.ts` + команда `snapshot`: именованные снимки
состояния и их сравнение с текущей страницей.

- **Пиксели сравниваются в самой странице** (`canvas` + `getImageData`):
  Chromium уже умеет рисовать картинку и читать пиксели, тащить разбор PNG в
  Node ради этого незачем.
- **Порог различия по каналу (по умолчанию 8) обязателен:** сжатие шевелит
  соседние пиксели на пару единиц, и без порога «различаются» любые два кадра
  одной и той же страницы.
- **Ответ содержит область различий, а не только долю:** «12% и всё в шапке» —
  это диагноз, «12%» — нет.
- **Текстовая разница считает повторы строк, а не множество:** строка «Итого: 0»,
  ставшая второй такой же, — это изменение.
- **Снимки живут в памяти сессии, их не больше десяти** и старые вытесняются:
  это рабочий материал одной проверки, а не архив.

## Отчёт о проверке (круг 13 паритета с пользователем, 15.09.2026)

`apps/browser-runner/src/sessionReport.ts` + команда `report`: лента действий,
заметки, ошибки страницы и снимки собираются в markdown для комментария задачи.

- **Markdown, а не JSON:** отчёт читает человек в канбане, а не парсер.
- **Неудачные шаги — отдельный раздел:** по ним человек решает, дефект это или
  особенность стенда, и именно их перепроверяет руками.
- **Шаги берутся хвостом:** свежие объясняют итог, первые — только вход.
- **Заметка не считается действием** — иначе счётчик шагов врёт.
- **Итог «замечаний нет» считается по неудачным шагам и ошибкам консоли/сети,**
  а не по мнению модели.

## Вкладки по-человечески (круг 14 паритета с пользователем, 15.09.2026)

Команда `tabs-do` и инструменты `find-tab`, `wait-new-tab`, `close-other-tabs`.

- **`wait-new-tab` сравнивает список вкладок до и после,** а не считает их: во
  время ожидания вкладка может и закрыться, и открыться повторно.
- **Поиск по части заголовка или адреса** — человек называет вкладку «та, где
  корзина», а не идентификатором из `tabs`.
- **Пустой запрос отвергается:** он выбрал бы первую попавшуюся вкладку и увёл
  проверку не туда.

## Перезапуск не теряет настройку проверки (круг 15, 15.09.2026)

Профиль сессии (`profileState.ts`) хранит не только cookie и адрес, но и
эмулированные устройство и среду; `sessionManager` применяет их при старте до
открытия сохранённой страницы.

- **Половинчатое состояние отбрасывается:** файл переживает обновления раннера,
  и применённая наполовину эмуляция хуже отсутствующей — она молчит.
- **Восстановление идёт до `goto`,** иначе страница успевает отрисоваться в
  десктопной теме и мигает при смене.
- **`session-info` отвечает на «что здесь вообще происходит»** одной командой:
  возраст сессии, вкладки, действия, снимки, правила сети, устройство и среда.
  Модель, вернувшаяся к разговору через час, иначе принимала подменённый ответ
  или тёмную тему за дефект сайта.

## Вердикт сравнения (круг 16, 15.09.2026)

Живая проверка снимков показала, что доля пикселей сама по себе вводит в
заблуждение: «различий 0%» при изменившемся тексте человек читает как «ничего не
изменилось», хотя почти всегда это изменение **ниже сгиба** — снимок снимает
видимую часть.

- **`verdict`: `identical` | `visual` | `dom-only` | `resized`.** `dom-only`
  прямо говорит, что делать: повторить со снимком `fullPage`.
- **`resized` важнее доли пикселей:** изменившийся размер страницы — это другой
  разговор, и сравнивать проценты в нём бессмысленно.
- **Снимок умеет `fullPage`.** Без него сравнение длинной страницы смотрит один
  первый экран и объявляет «различий нет», когда сломалось всё ниже.
- **В панели признак передаётся параметром, а не из замыкания:** кнопка вызывает
  колбэк, созданный до клика по галочке, и снимок уходил бы без «всей страницы».

## Слияние с параллельной работой над Reader (15.09.2026)

Ветка «паритета с пользователем» и работа в `main` независимо пришли к похожим
идеям, и слияние пришлось разводить руками. Что стоит помнить:

- **Ожидание сетевой тишины в main называется `idle`,** а не `network: 'idle'` —
  второе имя того же условия заводить незачем, контракт остаётся один.
- **`focus` в объединённом словаре ставит фокус и требует цель** (так в main);
  чтение «где сейчас курсор» — отдельный вид `focusState` и инструмент
  `focused`: он сознательно ничего не двигает, иначе обход формы табом
  превращается в цикл «спросил — сам же и сдвинул».
- **`report` main — отчёт панели о действиях модели, `check-report` — отчёт о
  проверке для комментария в задаче.** Это разные вещи, и одно имя на двоих
  ломало регистрацию MCP целиком: сервер молча переставал отдавать `tools/list`.
