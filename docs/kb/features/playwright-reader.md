...
### Ошибки словами и честный обрыв прогона (круг 30)

- **Сетевые ошибки навигации переводятся в раннере** (`humanizeNavigationError`
  из `packages/shared/src/browserErrors.ts`): «page.goto:
  net::ERR_NAME_NOT_RESOLVED at …» становится «Страница не открылась: имя сайта
  не разрешается (проверьте адрес) — адрес». Не сетевой текст не трогается —
  подменять «примерно тем же» хуже, чем оставить.
- **Коды раннера не доходят до человека буквально.** `not_found`,
  `stale_incarnation`, `stale_tab` клиент (`runnerClient.asError`) переводит
  через `describeBrowserRunnerCode`, а сам код кладёт в
  `BrowserRunnerError.code`; роут отдаёт его полем `code`, мост
  `makeBrowserBridge` вешает на `Error`. Панель по `isBrowserSessionLostCode`
  отличает потерянную сессию от разовой ошибки.
- **Потерянная сессия — ошибка с кнопкой «Перезапустить».** Сессию убирает
  сборщик простоя (полчаса в скрытой вкладке) или перезапуск раннера; раньше
  `refreshFrame` глотал 404 вместе с пропущенным кадром, и панель показывала
  старый кадр со статусом «Готово». Фаза `error` теперь умеет перезапуск.
- **Ошибка команды — 422, а не 502.** Раннер отвечает 422, когда не принял саму
  команду (адрес не открылся, действие не выполнилось); клиент отдавал это как
  502 «плохой шлюз», и по нему шли искать беду в инфраструктуре.
- **Прерванный прогон не «пройден» и не «дефект».** Отмена между шагами
  обрывала цикл молча — шагов в отчёте становилось меньше, и разовая проверка
  считала сценарий пройденным по двум шагам из пяти; отмена посреди шага давала
  «failed: запрос отменён», то есть дефект реализации за нажатое «Отменить».
  Теперь прерванный шаг и недошедшие — `skipped` с причиной, прогон `blocked`;
  причина различается по `signal.reason`: `TimeoutError` от
  `AbortSignal.timeout` — «Исчерпан бюджет времени прогона сценария», иначе
  «Прогон отменён» (`abortReason` в `automatedQaScenario.ts`).
- **Итоговый снимок этапа слушает сигнал отмены.** Круг 29 научил
  `screenshot()` его принимать, но единственный вызывающий сигнал не передавал:
  после «Отменить» снимок висел до собственного таймаута в 35 с.
- **MIME снимка — по запрошенному формату** (`screenshotMimeType` в
  `apps/browser-runner/src/server.ts`): роут отдавал любой бинарь как
  `image/png`, а панель просит jpeg, и data-URL врал о содержимом.
- **`selectTab` на неизвестную вкладку — `stale_tab`**, активная остаётся
  прежней. Раньше id записывался как есть, и сессия отвечала `stale_tab` на всё
  подряд, пока человек не кликал по другой вкладке.
- **Последняя вкладка закрывается с пустой взамен** — как в браузере. Сессия
  без страниц была непригодна, хотя Chromium жив; панель пряталa кнопку у
  последней вкладки, но модель и сама страница (`window.close()`) кнопок не
  видят. Тот же путь в обработчике `close` страницы; флаг `stopping` не даёт
  открывать вкладку в закрывающемся контексте.
- **Сетевой журнал видит упавшие запросы**: слушается `requestfailed`, запись
  идёт со `status: 0`, `ok: false` и `error` (`net::ERR_BLOCKED_BY_CLIENT` и
  т. п.). Раньше слушался только `response`, и заблокированный вызов API — то,
  ради чего открывают «Ошибки страницы», — в журнал не попадал. Панель
  показывает такой запрос как «нет ответа — причина».
- **Прогон в панели можно остановить** («Остановить прогон»): проверка между
  шагами, идущий шаг доигрывает. Раньше прогон с ожиданиями по 30 с прерывался
  только перезапуском всей сессии.
