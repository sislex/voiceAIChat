...
Проверка 2026-09-10 выявила ещё не исправленные расхождения: `planModelAction`
теряет `click.modifiers`, `press.selector` и `scroll.selector`, а прокрутка к краю
ограничена шагом 10 000 пикселей. Поэтому наличие инструмента не гарантирует
поддержку всех его параметров. Заявленные 8 МБ upload тоже пока не проходят весь
путь: `PREVIEW_ACTION_LIMITS.uploadBase64` ограничен 1 500 000 символами, а HTTP-роут
команд раннера оставляет стандартный лимит Fastify. Эти пункты запланированы для
следующей проверки инструментов; они не включены в завершённые исправления цикла 02.
