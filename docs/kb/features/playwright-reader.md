...
**Отклоняются, и причины разные** — это важно, до круга 9 у всех была одна общая
формулировка:

- `edits` — правки edit-режима копит **прокси превью**; у изолированного
  Chromium такого режима нет и быть не может;
- `upload` — содержимое файла нужно сначала записать на стороне раннера, это
  отдельная работа;
- `drag` по координатам — раннер тянет локатор к локатору, а подменять
  координаты «примерно тем же» хуже, чем отказать.
...
Отдельный вход `browserScreenshot` (рядом с `browserExecutor`) снимает вьюпорт у
раннера; `null` означает «этот разговор не про изолированный браузер, иди
обычным путём». Снимок по селектору раннер не умеет, и об этом честно сказано в
тексте ответа: молча отдать вьюпорт в ответ на запрос узла — значит соврать.
