...
Сценарий — **настройка проекта, а не артефакт рана**: воспроизводимость нужна
между попытками, а ран фиксирует только то, что именно прогонялось. Шаги с
неизвестным действием отбрасываются при записи (`normalizeAutomatedQaScenario`),
иначе раннер падал бы на середине.
