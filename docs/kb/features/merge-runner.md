...
target `main`, сохранённый source SHA и привязанную машину. Создание идемпотентно:
...
Merge строится от полученного target SHA в отдельном соседнем Git worktree и
создаёт merge-коммит с идентификатором задачи, не переключая и не загрязняя
рабочую копию CI. При конфликте пути из
...
выполняется в merge-worktree до push. Результат проверки хранит времена,
