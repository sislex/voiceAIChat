...
же формате, который использует release-gate проекта. Результат проверки хранит времена,
...
разрешение неизбежно считалось бы stale source. После любого терминального исхода
