...
правилам базы (`kb-workflow` → «Почему нет merge-конфликтов»): у тем сверяются
обе стороны без строк `updated:`/`checked:` — если тела совпадают, берётся
`--ours` и метаданные нормализует `node scripts/kb.mjs touch`; индекс
`README.md` перегенерируется `kb.mjs index`; итог валидирует `kb.mjs check`,
затем `commit --no-edit` — и merge продолжается. Содержательное расхождение тем
(тела различаются) и любой конфликт вне `docs/kb` переводят ран и карточку в
`decision_required`; автоматическое LLM-разрешение в текущей реализации не
запускается.
