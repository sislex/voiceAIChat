...
распознаёт её по тексту ошибки): раньше такой retry заново закреплял SHA снимка
workspace и детерминированно повторял ту же ошибку. Обычный retry (упавшие проверки
и прочие сбои) по-прежнему сохраняет строгую сверку с SHA development-рана. После
любого терминального исхода менеджер асинхронно удаляет временный merge-клон.
