...
ответа и добавляет изменения `docs/kb` через amend в тот же merge-коммит.
Ошибка останавливает ран до push. Безопасная семантика таймаута разрешает
продолжить только если файловые темы уже валидно изменены.
