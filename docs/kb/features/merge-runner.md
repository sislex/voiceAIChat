...
снимке. Конфликты, ограниченные `docs/kb/*.md`, менеджер разрешает сам по
правилам базы (`kb-workflow` → «Почему нет merge-конфликтов»): строки
`updated:`/`checked:` во всех трёх git-стадиях темы нормализуются плейсхолдером,
затем честный трёхсторонний `git merge-file` — чистый результат означает, что
конфликтовали только метаданные (неконфликтующие правки тела обеих сторон
сохраняются), и `node scripts/kb.mjs touch` ставит свежие значения; индекс
`README.md` перегенерируется `kb.mjs index`; итог валидирует `kb.mjs check`,
затем `commit --no-edit` — и merge продолжается. Конфликт после нормализации
(содержательное расхождение темы) и любой конфликт вне `docs/kb` переводят ран
и карточку в `decision_required`; автоматическое LLM-разрешение в текущей
