...
`node scripts/kb.mjs touch` ставит свежие значения. Содержательное расхождение
темы и любой конфликт вне `docs/kb` требуют решения пользователя; автоматическое
LLM-разрешение таких конфликтов не запускается.
