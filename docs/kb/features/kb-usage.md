...
В тулбаре каждого чата есть кнопка «Использование БЗ» (📚) с бейджем новых обращений после последнего успешного просмотра. Панель сохраняет историческую статистику: какие разделы базы знаний получила модель, сколько раз, сколько это дало символов и токенов (оценка), а также агрегат по всем чатам проекта. Считается только то, что видела модель: авто-инъекция контекста сервером и вызовы инструментов `mcp__kb__*`. Ручной поиск человека на странице «База знаний» в телеметрию не попадает — иначе метрика «сколько раз модель обращалась» врала бы.
...
Кнопка — в `.mhead-right` компонента `ChatColumn`: бейдж показывает серверный `unreadCount`, скрывается при нуле, а начиная со 100 рисует `99+`. Точное число новых обращений (или «новых обращений нет») входит в `aria-label`; сам бейдж скрыт от скринридера, при активном обращении вместо него `Dots`. Общий исторический `totals.queries` остаётся внутри панели. Панель — `packages/ui/src/components/kb/KbUsagePanel.tsx` (`ToolFrame variant='modal'`) с вкладками «Этот чат» / «По проекту», сводкой, таблицей разделов (сортировка через `aria-sort`, название раздела — кнопка) и лентой событий. Данные приходят пропсами; снапшот просит стор (`loadKbUsage`, `loadProjectKbUsage`), инкременты — `applyKbUsageQuery`.
