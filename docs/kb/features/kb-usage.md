...
Основная шапка `ChatColumn` не показывает кнопку, live-индикатор или счётчик использования БЗ. Вход в статистику находится во вкладке «Общее» настроек активного разговора, в карточке «База знаний проекта». Он открывает ту же панель `packages/ui/src/components/kb/KbUsagePanel.tsx` (`ToolFrame variant='modal'`) для текущих `conversationId` и `projectId`, с вкладками «Этот чат» / «По проекту», сводкой, таблицей разделов (сортировка через `aria-sort`, название раздела — кнопка) и лентой событий. Данные приходят пропсами; снапшот просит стор (`loadKbUsage`, `loadProjectKbUsage`), инкременты — `applyKbUsageQuery`.
