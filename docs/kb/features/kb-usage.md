...
Раньше дорожка была одна и работала наоборот: при наличии бэктиков от описания оставляли ТОЛЬКО их содержимое. На технических задачах (CHAT-54/68/70) от темы оставался набор идентификаторов, лексический поиск проваливался, и авто-инъекция молчала ровно там, где знание кодовой базы нужнее всего. Регрессия закреплена тестом `kb/taskInjection.test.ts` на дословных описаниях тех трёх задач. Кроме блока контекста в задание уходит `kbRunDirective` — требование начать с базы знаний и только потом читать код (в `manual` — усиленная формулировка: инструменты единственный путь к базе). Инструменты подключаются к трём ходам рана: работа модели, fix-loop и резюме, — в том числе в фазе «план» и в ходе без машины: база read-only и от агента не зависит.
