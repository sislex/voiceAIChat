...
Каждая карточка всегда монтирует собственные взаимоисключающие кнопки «Успешно»,
«Неуспешно», «Заблокировано» (и «Пропустить» для необязательного сценария),
подписанный комментарий и текстовое состояние «несохранено / сохраняем /
сохранено / ошибка». Выбор передаётся через `aria-pressed`. Для failed и blocked
комментарий обязателен; клиентская либо серверная ошибка сохраняет ввод и
предлагает повтор, а повторный клик блокируется только для сохраняемой карточки.
Локально изменённая карточка не принимает серверный snapshot поверх draft.
Ниже списка сохраняются дополнительные баги. Сводка считает total, checked,
passed, failed, blocked и remaining. «Отправить на доработку» запускает связанный
development-ран и открывает его ленту; «Прошёл тестирование» переводит задачу в
`awaiting_merge`. Закрытая, stale либо недоступная по возвращаемому сервером
`canEdit` session показывается read-only. На мобильной ширине детали переходят
в одну колонку, органы управления переносятся без горизонтальной прокрутки.
