...
Storybook и Playwright в `beforeAll` имеет лимит 120 секунд. `afterAll` с тем же
явным лимитом дожидается завершения и закрытия Playwright browser, и остановки
всей detached-группы Storybook по отрицательному PID. Обе операции очистки
всегда доводятся до settled-состояния: сбой одной не отменяет ожидание второй,
после завершения всех попыток одиночная ошибка пробрасывается как есть, а
несколько ошибок объединяются в `AggregateError`. Поэтому успешные assertions
не маскируют реальный сбой teardown, а полный `npm run gate:fast` возвращает 0
только при успешном завершении тестов и очистки.
...
optimistic concurrency. Конфликт возвращается явной ошибкой; карточка сохраняет локально выбранный статус и комментарий для
повторной отправки и не подменяет их ответом сервера. Закрытая или stale session
не принимает новые правки.

`canCompleteQa` допускает завершение, только когда session активна и не stale,
preview SHA совпадает с commit SHA, каждый обязательный критерий имеет актуальный
результат `passed`, отсутствуют failed-результаты и общие замечания. Результат
необязательного критерия для допуска не требуется; доменный статус
`not_applicable` остаётся в контракте и истории, но `ManualQaPanel` пользователю
его выбрать не предлагает. Для обязательного критерия missing, failed, blocked,
...
Для `failed` и `blocked` обязательно пользовательское поле `comment`:
фактический результат, наблюдение, шаг ошибки, отличие от ожидания либо причина
блокировки. Для `passed` комментарий необязателен. `failed` сохраняет
существующий структурированный issue, а `blocked` — существующие
`blockerReason/blockerType/blockerOwner`. Сохранение результата само не закрывает session. Общие замечания хранятся в `qa_sessions.additional_issues` и сохраняются
...
session, сохранение результата или черновика и добавление вложения. Для каждого
изменения результата payload содержит actor, serverTime, sessionId, criterionId
и предыдущие/новые status, comment и revision. Текущая реализация не
предоставляет отдельный API чтения аудита и не транслирует изменения QA через
WebSocket.
...
собственный комментарий и ровно три взаимоисключающих результата: «Работает»
(`passed`), «Нет возможности проверить» (`blocked`) и «Не работает» (`failed`).
Выбор передаётся через `aria-pressed`; локальные status/comment/dirty/error и
функция отложенного сохранения принадлежат конкретной карточке, поэтому изменение
одного теста не меняет остальные. Для failed и blocked комментарий обязателен.
Успешный `saveResult` заменяет соответствующий результат в QA state; при новом
монтировании панели или карточки сохранённые значения берутся из QA session.
Клиентская либо серверная ошибка оставляет фактический локальный ввод для повтора,
показывает ошибку и перечитывает общий QA state, но dirty-карточка не принимает
серверный snapshot поверх своего ввода.

Ниже списка сохраняются дополнительные баги. Сводка считает total, checked,
passed, failed, blocked и remaining. Итоговые «Следующий этап» и «Отправить на
...
результата обязательного теста, общие замечания или устаревшую session. Закрытая,
stale либо недоступная по возвращаемому сервером `canEdit` session показывается
read-only. На мобильной ширине детали и обе группы действий переходят в одну
колонку без горизонтальной прокрутки.
