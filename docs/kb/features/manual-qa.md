...
имеет актуальный результат `passed` либо `not_applicable`. Результат
`not_applicable` в UI называется «Нет возможности проверить» и считается
обработанным без обязательного комментария. Missing, failed, blocked,
not_tested, in_progress и stale
...
Для финального `failed` обязательны expected/actual, выполненные шаги,
комментарий и структурированный issue с classification, severity, frequency и
reproduction. Сохранение failed-результата само не закрывает session и не
перемещает карточку: пользователь может оформить все тесты одной попытки.
`POST …/qa/sessions/:sessionId/fix` проверяет наличие failed-комментариев,
запускает один development-ран через `CiRunManager`, закрывает session как
failed и записывает его id в `qa_issues.linked_fix_run_id`. Повтор запроса
возвращает уже связанный ран. Модель получает исходную задачу и структурированный
список ожидаемых/фактических результатов, шагов и комментариев только из
связанной QA-попытки. Для `blocked` обязательны причина, тип и ответственный.
...
три пользовательских исхода «Работает», «Не работает» и «Нет возможности
проверить», а также загрузку нескольких скриншотов. Сводка отдельно считает все
исходы и непроверенные тесты. «Внести правки» доступна, когда есть failed,
комментарии заполнены и нет конфликтующего активного рана; после ответа сервера
карточка открывает ленту связанного development-рана. Stale session видна, а
