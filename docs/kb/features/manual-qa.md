...
системную колонку `qa_preparation`. В карточке создаются подробные сценарии на
основе acceptance criteria: название, цель, предусловия и URL, пошаговые действия,
данные для ввода и ожидаемый результат. `completeQaPreparation` требует хотя бы
