...
само не закрывает session. Общие замечания хранятся в `qa_sessions.additional_issues`
и сохраняются отдельным PATCH session. `POST …/qa/sessions/:sessionId/fix`
доступен при failed-тестах либо общих замечаниях, проверяет комментарии failed,
запускает один development-ран через `CiRunManager`, закрывает session и сохраняет
id рана в session и issues. Повтор возвращает связанный ран. Модель получает
исходную задачу, ошибки тестов и дополнительные замечания этой QA-попытки.
...
попытка, время, длительность, потоковый лог, диагностика и retry. Пока preparation
выполняется, тест-кейсы скрыты. Ниже session показывает commit/preview SHA,
...
задачу в `awaiting_merge`. Закрытая, stale либо недоступная по permission session
...
назначения тестировщика, просмотра версий/аудита, удаления вложений и realtime-
конфликтов нет. Optimistic conflict возвращается сервером, после ошибки панель
перечитывает актуальный снимок при следующем запросе/опросе.
