...
рестарт, хранят потоковый лог/ошибку и защищены UNIQUE(task_id, commit_sha).
Модель получает задачу, acceptance criteria, статусы шагов development-run,
feature-ветку/SHA и названия активных сценариев. Ответ сохраняется как подробные
структурированные criteria: название, цель, предусловия, пошаговые действия,
данные для ввода и ожидаемый результат. Новый SHA помечает активную QA session stale. `completeQaPreparation` требует хотя бы
один активный сценарий и непустые title/steps/expectedResult, после чего переносит
задачу в `manual_qa`. Ручная приёмка завершается отдельной серверной операцией:
