...
Центр релизов работает только с удалёнными ветками строгого вида `release/x.y.z` без ведущих нулей. Общая валидация находится в `packages/shared/src/release.ts`. Перед показом списка сервер выполняет fetch с prune и читает только `refs/remotes/origin/release/*`, поэтому UI выбирает существующую ревизию из актуального origin. Обе части shell-команды являются Git-подкомандами: после fetch вызывается именно `git for-each-ref`; вариант без второго `git` завершается в zsh ошибкой `command not found: for-each-ref`, которую маршрут списка оборачивает в HTTP 400.
