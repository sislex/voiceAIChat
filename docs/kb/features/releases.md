...
Release Center run in the database.
...
**Цикл улучшений Release Center 5 (2026-09-12) — живые обновления.** Новый WS-кадр `release.updated {projectId, releaseId, status}` (`packages/shared/src/protocol.ts`, мост `window.board.onReleaseUpdated` — необязательный метод `RendererBoardBridge`). Источник — `ReleaseManager`: все записи статусов и шагов идут через `setStep`/`setStatus`, которые после записи в БД зовут опцию конструктора `onChange` (для шага статус дочитывается `getProjectReleaseStatus` без проверки членства); в `kanban/module.ts` он подключён к `boardHub.emitRelease`. Дальше — тот же путь, что у `qa.stage.updated`: `BoardHub.onReleaseChange` → `KanbanService.board.subscribeReleases` → в standalone-канбане событие `{ kind: 'release' }` на шину, `kanbanBridge/remote.ts` воспроизводит его у ядра → `session.ts` шлёт кадр участникам, которым видна доска проекта. `ReleaseCenter` подписывается на кадр: список перечитывается с коалесингом 300 мс (шаги регрессии меняются несколько раз в секунду), открытые подробности дочитываются точечно `releases:get`, при reconnect — полный `refresh`; опрос 5 с/2 с остаётся только без моста (desktop, старый сервер). Завершение сборки или деплоя, которое пользователь наблюдал, объявляется тостом («release/0.1.301 опубликован в production», ошибка — с причиной из `failure`). Тесты: `releaseManager.test.ts` (порядок статусов подписчику), `kanbanBridge/remote.test.ts`, `ReleaseCenter.dom.test.tsx` (фейковый `window.board`).

**Цикл улучшений Release Center 6 (2026-09-12).** Готовность production считается на клиенте той же логикой, что `releaseProductionTarget` на сервере: `productionReadiness(detail)` в `packages/shared/src/release.ts` (gitUrl, машина и её привязка, checkout для legacy, команды деплоя и health-check); `App` передаёт результат в `ReleaseCenter` пропом `production` вместе с именем машины, и вкладка «Деплой» показывает либо баннер «Production не настроен — не хватает: …» со ссылкой «Открыть настройки проекта» (`onOpenSettings`), либо карточку «Production: машина · режим · health-check». Исправлен баг подсказки деплоя: `compareReleaseBranches` возвращает разницу версий, а код сравнивал с `-1`, поэтому откат через несколько версий назывался «обновлением»; теперь любое отрицательное значение — откат, а в подсказке указано, сколько версий пропускается («минуя N версий»). Оценка длительности идущей сборки/деплоя — среднее последних трёх успешных (`typicalDurationMs`): «обычно ≈ 4 мин 30 с, осталось ≈ …». На вкладке «Релизы» — фильтры «Все / Готовые / Ошибки (N) / В production» и поиск по версии или SHA; у завершённых строк — кнопка «Лог» (скачивает лог без открытия подробностей). Поле версии принимает `release/0.1.302` и `v0.1.302` (`normalizeReleaseVersionInput`). Настройки лимитов валидируются (1 с … 24 ч) и сбрасываются к умолчаниям. Сервер: `listBranches` кэширует `git ls-remote` на 10 с (`BRANCH_LIST_TTL_MS`, опция `branchListTtlMs`) с точным сбросом после создания/удаления ветки и KB-коммита; проверки существования ветки и SHA перед deploy читают origin свежим (`{ fresh: true }`).

**Цикл улучшений Release Center 7 (2026-09-12).** Глубокие ссылки: маршрут `/projects/:id/releases/:releaseId` (`ProjectsRoute { kind: 'releases', releaseId? }` в `packages/projects-app/src/routes/projectsRoute.ts`); `App` передаёт `initialReleaseId` и `onOpenRelease`, центр открывает подробности при загрузке и зеркалит открытие/закрытие в адрес (`replace`), кнопка «Ссылка» копирует текущий адрес. В подробностях: Esc возвращает к списку (кроме фокуса в полях ввода), строка «N из M шагов пройдено · пропущено K · есть ошибка», у идущего шага — `ProgressTrack` «прошло … из лимита …» (тон warning после 80 % лимита), строки лога с `error/fail/fatal/ошибка/не удалось/×` подсвечены (`.release-log-error`). Пока идёт сборка или деплой, заголовок вкладки браузера начинается с «⏳ Деплой release/x · …» и восстанавливается после. Режим «Весь проект / Приложения» и окружение режима приложений запоминаются (`RELEASES_MODE_KEY = 'vc.releases.mode'`, `RELEASES_APP_ENVIRONMENT_KEY = 'vc.releases.appEnvironment'`; `ApplicationReleaseCenter` принимает `environmentStorageKey`).

**Цикл улучшений Release Center 8 (2026-09-17) — состав и сравнение.** Состав релиза читает `GET /api/projects/:id/releases/:releaseId/changes` через мост `releases:changes`; источник контракта — `packages/shared/src/release.ts`, маршрут и выбор базы — `apps/server/src/routes/releases.ts`. `ReleaseManager` выполняет на машине сборки `git log` для диапазона SHA и кэширует результат на 60 секунд. Явный `from` всегда означает сравнение двух указанных релизов; без него база — SHA текущего успешного production deploy. Если production ещё не было, сервер возвращает `fromSha: null, changes: null`: UI пишет «Нет production для сравнения», не подменяет неизвестный состав нулём и разрешает первый deploy. Подробности группируют conventional commits (`feat`, `fix`, `docs` и другие, неизвестные — `other`) и для GitHub-репозитория строят ссылки на коммиты. На вкладке «Деплой» та же выборка показана сворачиваемой сводкой. Два чекбокса в списке подготовок открывают сравнение статусов и длительностей шагов и запрашивают коммиты между их SHA.

**Цикл улучшений Release Center 8 — действия и доступность.** Каждый deploy предваряет `useConfirm` с версией, SHA, production-машиной, режимом и известным числом коммитов; отсутствие production показывается явно, но не блокирует действие. Повторный deploy той же версии и откат используют опасный вариант подтверждения. Помимо текстового лога подробности скачивают `release-<version>.json` с метаданными рана, шагами и лимитами. На ширине до 720 px `IntersectionObserver` показывает нижнюю `release-sticky-actions`, когда исходная форма сборки или deploy ушла из viewport; дублированные кнопки сохраняют состояние disabled и подсказки основных действий. Наблюдавшиеся терминальные переходы одновременно вызывают тост и формируют русское объявление в `aria-live`.

**Цикл улучшений Release Center 8 — уведомления и архив.** Терминальный статус `ready`, `released` или `failed` создаёт единственную запись `release_notifications` для владельца проекта. Она включается в существующий снимок `tasks:listPreparationNotifications`, инвалидируется через общий `NotificationHub`, закрывается тем же действием колокольчика и открывает глубокую ссылку релиза. При старте сервера и затем раз в сутки фоновая задача ставит `archived_at` только упавшим подготовкам старше 30 дней (`previous_release_id IS NULL`, `status='failed'`); origin-ветка и запись релиза не удаляются. `GET /api/projects/:id/releases` скрывает архив по умолчанию, а `?archived=1` и переключатель «Показать архивные» возвращают его.

Владелец может удалить завершённую подготовку `ready`/`failed`: backend заново сверяет id и строгую ветку, запрещает активный deploy и текущий production-релиз, проверяет, что ветка не занята ни одним worktree, удаляет `refs/heads/release/x.y.z` из origin и одноимённый локальный ref в checkout, затем ставит `deleted_at`. Локальный ref тоже обязателен к удалению: иначе повторная сборка той же версии падает на `git branch … FETCH_HEAD` с `fatal: a branch named … already exists`, хотя origin уже чист. Deploy-попытки остаются в БД для аудита. Кнопка deploy доступна только для `ready`. Переход между строгими `release/x.y.z` сравнивается по числовым компонентам SemVer: меньшая версия показывается как откат, большая — как обновление production. Production-машина, checkout и разрешённые команды редактируются владельцем в настройках проекта. История релизов обратно совместима с сохранёнными шагами прежних версий: неизвестный текущему контракту `kind` отображается как есть, а краткая ошибка берётся из диагностического лога (или заменяется нейтральным fallback), поэтому старые `cleanup`/`push_main` не роняют центр релизов.
