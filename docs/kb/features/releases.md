...
**Цикл 8 (2026-09-16).** Состав релиза читается через `GET /api/projects/:id/releases/:releaseId/changes` и мост `releases:changes`: `ReleaseManager` выполняет на машине сборки `git log --format=%H%x1f%an%x1f%at%x1f%s <from>..<to>` и кэширует результат на 60 секунд. Явный `from` сравнивает два SHA; без него база — SHA текущего успешного production deploy. Если production ещё не было, ответ имеет `fromSha:null, changes:null`: UI пишет «Нет production для сравнения», не показывает нулевое количество и не блокирует первый deploy. Подробности группируют conventional commits и дают GitHub-ссылки; вкладка «Деплой» показывает сводку с раскрытием. Две подготовки выбираются чекбоксами для сравнения статусов, длительностей шагов и коммитов. Перед каждым deploy `useConfirm` перечисляет версию, SHA, production-машину, режим и состав; повтор и откат используют `danger`. Экспорт доступен как текст и `release-<version>.json`. На экране до 720 px `IntersectionObserver` показывает нижнюю `release-sticky-actions`, когда основная форма ушла из viewport. Терминальные переходы объявляются в `aria-live` и создают владельцу запись `release_notifications`, которая приходит через существующую invalidation-ленту колокольчика и ведёт по глубокой ссылке. Ежедневная задача ставит `archived_at` упавшим подготовкам старше 30 дней, не удаляя origin-ветки; список скрывает их по умолчанию, параметр `?archived=1` и переключатель «Показать архивные» возвращают их.
