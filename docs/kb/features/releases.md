...
До создания попытки и фиксации SHA release-preflight запускает `kb:index` на выбранной release-ветке. Если изменился только генерируемый `docs/kb/README.md`, он автоматически коммитится и отправляется в эту ветку; любые другие изменения останавливают запуск без автокоммита. Затем ветки повторно читаются из origin, поэтому попытка фиксирует уже подготовленный SHA. Упавший шаг показывает короткое человекочитаемое резюме, вычисленное из вида шага и лога, а полный исходный лог остаётся в раскрываемой «Расшифровке».

Merge строится от свежей `origin/<ciBaseBranch>` и использует сохранённый SHA, поэтому движение release-ветки после старта не меняет публикуемый код. После merge ворота push создают тег `v<version>` на итоговом `HEAD` основной ветки и атомарно отправляют ветку вместе с тегом: именно этот tagged commit собирает production. Повтор того же релиза безопасен, а уже занятый другим SHA тег отклоняет всю отправку. Прод-скрипты читают этот тег и передают номер в `VC_RELEASE_VERSION`. Production deploy вызывается через штатный `deployTrigger`. После принятия фонового деплоя release health-check до 300 секунд опрашивает `/api/health` и пропускает ворота только при `ok: true` и точном совпадении `version` с публикуемым релизом; старый ещё работающий контейнер больше не даёт ложный успех. Проверка обращается по loopback самого сервера, а не через default-машину проекта: default-машина обслуживает Git release-веток и может быть MacBook без production на `127.0.0.1:8787`. Host-side deploy независимо ждёт реальный health после сборки. Production deploy перезапускает сам сервер, поэтому при старте `ReleaseManager` восстанавливает из БД публикации со статусом `running`: уже пройденные ворота пропускаются, а выполнение продолжается с первого шага без `passed`. Обычно это ожидающий `health_check`, после которого cleanup удаляет preview задач из `done` и переводит их активные CI workspace в `released`. Инфраструктурная привязка находится в `apps/server/src/server.ts`.
