...
`image`, `grep`, `edit`), а не один `bash`: в headless (`claude -p`) этот флаг — allow-list
...
движков: они называют файловые инструменты поимённо (`mcp__remote__read`/`image`/`grep`/
`edit` у claude, `read`/`image`/`grep`/`edit` того же сервера у codex — allow-list у него
...
снова пройдёт незамеченной. `image` отдельно возвращает MCP `ImageContent`: бинарные
данные не превращаются в текстовый вывод, а имя разрешается по вложениям хода,
рабочему каталогу и папке проекта. CLI-хинт закрепляет тот же порядок, прямую
