...
безопасные варианты, не отменяя и не заменяя процесс. Ручные циклы хранятся как неизменяемые строки `task_rework_cycles` с уникальными
`(task_id, sequence)` и `(task_id, idempotency_key)`; снимок Make-источников и
критерии сериализуются в ревизию, а вложения связываются через
`task_attachments`. `GET/POST /api/projects/:id/tasks/:taskId/rework-cycles`
читает историю и атомарно создаёт следующий цикл вместе с переводом задачи в
системную колонку `preparation`. POST требует `Idempotency-Key`, повтор
возвращает прежний результат, а любой активный нормализованный ран отклоняется
конфликтом `TASK_ACTIVE_RUN` без отмены процесса.

`TaskCardContainer` лениво загружает постоянную историю, исходные вложения и
разрешённые Make-проекты при переключении на новую карточку; presentation-компонент
показывает loading/empty/error/retry, позволяет независимо выбрать whole-project
либо файлы каждого источника и отправляет снимок через постоянный IPC/HTTP-клиент.
Исходные и временные вложения цикла загружаются через
`GET/POST/DELETE /api/projects/:id/tasks/:taskId/attachments`; временные файлы
атомарно привязываются к создаваемой ревизии и после этого не удаляются обычным
редактированием. Legacy остаётся начальным представлением и не зависит от новых
запросов. Контекст модельного рана дополняется исходными полями, исходными
вложениями и всеми циклами по sequence, включая Make-снимки и статусы вложений.
