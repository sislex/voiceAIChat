...
## Ритм запросов доски и очередь улучшений (2026-09-06)

Замер на стенде: пятнадцать `board.changed` за шесть секунд (так ведёт себя активный
ран) давали **8 запросов доски и 8 запросов `/improvements`** — панель улучшений висела
на ссылках `props.board`/`props.ciSummaries` и ходила за списком ровно столько же раз,
сколько обновлялась доска. После правки тот же поток даёт 3 запроса доски и ноль
запросов улучшений.

**Доска.** `scheduleBoardSync` в `projectsStore` совмещает три правила: дебаунс 400 мс
(склеить поток), минимальный интервал 1,5 с между снимками (не ходить на каждое
событие) и потолок ожидания 2 с (иначе непрерывный поток откладывал бы обновление
бесконечно и доска замерла бы до тишины). Одиночное изменение по-прежнему приезжает
почти сразу. Ритм закреплён тестом в `appRuntime.projects.test.ts`; тесты, которые
промотывают таймеры вокруг доски, должны учитывать минимальный интервал.

**Улучшения.** Открытие доски за очередью **не ходит вовсе**: колонка «Улучшения»
стоит свёрнутой и грузит список по кнопке «Показать» (`kanban-improvements-open`).
Доска показывает задачи, а очередь предложений смотрят отдельно и редко — на боевом
проекте это были 68 карточек на каждом открытии канбана. Пока колонка открыта,
список обновляется по адресному кадру `project.improvements.updated`
(`BoardHub.emitImprovements`; эмиты — в роутах `/api/improvements/*` и после анализа
завершённого рана в `ci/runManager.ts`); свёрнутая колонка на кадры не подписана.
Смена проекта сворачивает очередь обратно.

В карточке задачи улучшения грузятся **по открытию своей вкладки** (`seenTabs`), а
не при открытии карточки: её открывают ради описания и ранов.

**Дедупликация параллельных чтений.** `refreshProjects`, `refreshConversations` и
`loadTaskChatBadges` держат in-flight промис и возвращают его повторным вызовам: на
старте список проектов и список бесед просили сразу два независимых места, и сервер
отдавал один и тот же ответ дважды.
## Доска приезжает двумя фазами

`GET /api/projects/:id/board` (`db.getBoardSkeleton`) отдаёт **скелет**: колонки и
карточки — что за задача и в какой она колонке. Один запрос к `tasks`, без единого
join и без тяжёлых текстов (перечень колонок — `BOARD_TASK_COLUMNS`). Состояние
процессов приезжает следом, отдельным запросом
`GET /api/projects/:id/board/statuses` (`db.getBoardStatuses`, мост
`board:getStatuses`): связанный чат, merge-поля, подготовка, `latestRunResult` и
сводки CI. Склеивает их чистая `applyTaskStatuses` из `@shared/projects`; она же
даёт `db.getBoard` — прежний полный снапшот для MCP, автопрохода и тестов.

Клиент (`projectsStore.syncBoardStatuses`) накладывает вторую фазу на **текущий**
стор, а не на снимок первой: между фазами карточку могли перетащить, и
оптимистичное перемещение терять нельзя. Отказ второй фазы доску не роняет —
скелет уже на экране, ошибка уходит в `console.warn`.

Зачем разделили (замер на боевом проекте ChatAI, 2026-09-06): один ответ доски шёл
**3.1 с и весил 1.59 МБ**, и всё это время сервер не отвечал никому — Node
однопоточный, а `better-sqlite3` синхронный, так что `/api/health` в этот момент
отвечал 1.7 с вместо 41 мс, а `/api/session/me` на старте страницы висел 22 с.
Причины были в трёх местах, и все три вылечены:

- **N+1 на карточку.** `latestTaskRunResult` (UNION по восьми таблицам ранов)
  вызывался по разу на задачу — теперь это одна оконная выборка
  `latestTaskRunResults` на всю доску. Коррелированные подзапросы merge/чата/
  подготовки заменены запросами «по проекту целиком» с `ROW_NUMBER()`.
  `mergePermitted` — свойство участника, считается один раз, а не на карточку.
- **Сводки CI всего проекта.** `latestCiRunSummaries` собирала раны **всех** задач
  за всю историю: 383 сводки на 19 видимых карточек, 1.07 МБ из 1.09 МБ ответа.
  Теперь она принимает `scope` — подзапрос с id карточек доски. Плюс история
  длительностей шагов (`ciStepDurationHistory`, JOIN с `LIMIT 200`) считается один
  раз на доску, а не внутри каждой сводки.
- **Недостающие индексы.** `EXPLAIN QUERY PLAN` показывал `SCAN` по
  `ci_workspaces` (четыре подзапроса × число карточек), `qa_sessions` (частичный
  `idx_qa_sessions_one_active` годится только для активной сессии) и перебор всех
  бесед пользователя вместо поиска по `task_id`. Добавлены
  `idx_ci_workspaces_task`, `idx_qa_sessions_task`, `idx_conversations_task`.

Итог на синтетике того же профиля (400 задач, 20 в окне): было 62 мс и 344 КБ
одним ответом — стало 0.22 мс и 14 КБ скелетом плюс 2.2 мс и 27 КБ статусами.

Отсечка завершённых считается один раз и уходит **в SQL**:
`completedVisibilityCutoff` (`@shared/projects`) переводит порог
`doneRetentionDays` в границу `done_at`. Прежде колонка «Готово» вычитывалась
целиком, чтобы тут же быть отброшенной в памяти. Правило то же, что у
`isCompletedHidden`, — эквивалентность закреплена тестом.
...
включает этот флаг сама (эффект в `App.tsx`). **Открытие проекта всегда начинается
с окна**, даже когда «показывать завершённые» включено в виде доски: первый запрос
идёт без флага, доска появляется, и только потом `setBoardIncludeCompleted(true)`
догружает старые карточки. Иначе постоянный пользователь фильтра платил бы за
сотни архивных карточек на каждом старте. При каждом переключении фильтра
...
**Доска грузится только там, где её видно (2026-09-06).** `openProject(id, { board })`
в `projectsStore` разделил «открыть проект» и «открыть доску»: вкладки без канбана
(`releases`, `settings`, `code`) берут один `projects:get`, а `board:get`,
`board:getView`, `board:getStatuses` и подписка `board.changed` не делаются вовсе —
на проекте с работающим раном подписка перечитывала доску каждые пару секунд, пока
человек смотрел релизы. Переход на вкладку «Канбан» догружает доску через
`ensureBoard(id)` (детали из стора не перезапрашиваются, повторный вызов на
загруженной доске — no-op); `openBoard(id)` остался обёрткой над
`openProject(id, { board: true })`. Решение о том, нужна ли доска, принимает `App.tsx`
по разобранному маршруту (`routeNeedsBoard`). Побочный эффект — на этих вкладках нет
`board.tasks`, поэтому в палитре команд не будет пунктов задач; `openTaskProposal` этот
случай уже умеет и догружает доску запросом сам.
...
колонку, Make-связи и последний ран в типизированный `TaskCardViewModel`, а
`NewTaskCardView` получает только эту модель и callbacks, без API и store.
Переключатель в шапке меняет только локальную версию представления; «Старая»
рендерит прежний `TaskModal` со всеми его вкладками и действиями. Черновик формы
ручной доработки, выбранная вкладка новой карточки и локально полученная история
циклов принадлежат контейнеру, поэтому не являются мутацией `Task`.

**Сколько карточка просит у сервера (2026-09-07).** Замер на проде показал три
источника лишних запросов, все — в клиенте. Первый: история доработок
перечитывалась ~раз в секунду, пока карточка открыта, — эффект в
`TaskCardContainer` держал в зависимостях `props.loadReworkCycles`, а `App.tsx`
отдавал инлайн-стрелку, новую на каждый свой рендер. Теперь загрузчик живёт в
`useRef`, эффект срабатывает на `task.id` и `task.updatedAt` (снимок задачи
приходит с доски по `board.changed`), а сам проп в `App.tsx` обёрнут в
`useCallback`. Второй: `CiTaskSettings` рендерится тремя экземплярами
(`machine`, `model`, `commands`), и каждый грузил все четыре источника —
`ci/commands`, `tasks/:id/ci`, `ci/llm`, `ci/machines` уходили по три раза;
теперь секция берёт только своё. Третий: панель настроек монтировалась всегда, и
её запросы уходили даже на вкладке «Подготовка» — она смонтирована лениво, при
первом заходе на вкладку, и потом не размонтируется (черновик и загруженные
значения переживают переключение вкладок).
...
исходную постановку, Make-файлы с точечными ошибками и историю циклов. Ручной
переход в `preparation` разрешён из post-development стадий и `done`, но не из
`cancelled`; выход автоматики из `decision_required` по-прежнему запрещён.
При активном ране форма не вызывает подтверждение и перечисляет только явные
безопасные варианты, не отменяя и не заменяя процесс. Новая карточка при открытии
параллельно с полной задачей загружает сохранённую историю ручных циклов; сбой
истории переводит представление в явное error-состояние с повтором. Исходные
вложения приходят в `Task.attachments`, а недоступный файл из сохранённого цикла
показывается со статусом `missing`. Источник поведения —
`packages/ui/src/components/kanban/TaskCardContainer.tsx` и
`NewTaskCardView.tsx`.

Файлы нового цикла сначала загружаются через общий upload API. Пока есть
`uploading` или `error`, отправка заблокирована; ошибочный файл можно повторить
или удалить. Ошибка загрузки либо создания остаётся видимой и не очищает описание,
критерии, Make-пути и уже загруженные файлы. Для повторной отправки контейнер
сохраняет тот же idempotency key; только успешный ответ очищает черновик, закрывает
форму и добавляет возвращённый цикл в историю по `sequence`.

Постоянный контракт — `GET/POST
/api/projects/:id/tasks/:taskId/rework-cycles` (маршруты и валидация:
`apps/server/src/routes/projects.ts`). Неизменяемый снимок описания, критериев,
Make-источников и метаданных вложений хранится в `task_rework_cycles` и
`task_rework_attachments`; уникальны пары задача+sequence и
задача+idempotency key. POST принимает только uploads текущего пользователя,
требует предыдущий успешный development-run, отсутствие активного рана и
разрешённый переход из post-development стадии либо `done`. Создание цикла,
привязка вложений и перенос задачи в `preparation` выполняются одной транзакцией;
повтор того же ключа и payload возвращает прежний цикл, а иной payload даёт
`idempotency_conflict`. Источники хранения —
`apps/server/src/db/schema.ts` и `apps/server/src/db/database.ts`.
...
## Компоненты проекта в Make и быстрый тикет к слиянию (2026-09-05)

Make умеет открывать компоненты **реального репозитория**, а не только своей песочницы.
Источник — рабочая копия проекта на машине (`GitWorkspaceService`), поэтому прав, локов и
проверок доступа новых нет: маршруты `/api/projects/:id/components*`
(`apps/server/src/routes/projectComponents.ts`) стоят под теми же тремя слоями, что панель
кода, — `repository:write` на любой не-GET (`users/auth.ts`), возможность типа проекта `git`
и `resolve()` сервиса рабочих копий.

**Список компонентов у двух источников.** Живой Storybook отвечает `/index.json`, и это
лучший источник: в нём настоящие `storyId`, они же адрес кадра. Пока Storybook не поднят,
список собирается `git ls-files '*.stories.*'` (скрипт `storyFilesScript`, метод
`GitWorkspaceService.storyFiles`, потолок вывода 64 КБ с честным признаком `truncated`), а
стори одного файла разбираются по клику тем же `parseStoryFile`, что у песочницы Make.

**Два разных пути «Make и проект», не путать.** `make:projectFiles`/`make:projectLinks`/
`make:projectPull` копируют файлы репозитория на машине **в песочницу** Make (до
`MAKE_PROJECT_SYNC_MAX_FILES`), чтобы ассистент работал с ними как со своими. Вкладка
«Репозиторий» ничего не копирует: она читает и пишет прямо в рабочую копию через
`projects:git*`, и правка сразу оказывается в ветке. Первый путь — про черновик по мотивам
проекта, второй — про изменение самого проекта.

**Модули dev-сервера переписываются прокси.** Vite отдаёт ESM с абсолютными импортами
(`from "/@vite/client"`), и браузер резолвил бы их относительно origin ChatAI — на стенде это
давало 404 на каждый модуль и пустой кадр. Поэтому `rewritePreviewBody` для хостов
`*.machine.internal` переписывает и JavaScript: `rewriteModuleSpecifiers` переводит абсолютные и
относительные спецификаторы `import`/`export from`/`import()` на `/api/preview?url=…`, голые имена
пакетов не трогает. Внешних сайтов Reader это не касается — переписывание включено только для машин.

**Storybook живёт в PTY-сессии машины** (`apps/server/src/components/storybookSessions.ts`,
ptyId `storybook:<workspaceId>`). Не `exec`: он убивает группу процессов по таймауту, а нам
нужен dev-сервер на часы. Готовность определяется не по выводу процесса, а пробой
`/index.json` через `AgentRegistry.http` — `storybook dev` печатает адрес задолго до того,
как соберётся первый бандл, и кадр, открытый раньше, ловил бы таймаут моста. Порт ищется
свободный в диапазоне 6006…6046 среди наших же сессий. Остановка — Ctrl-C и `ptyKill`.
Сессии живут в памяти процесса, поэтому у панели есть **подхват**: если на порту уже отвечает
`/index.json`, `start` не поднимает второй процесс, а подключается к существующему и помечает
сессию `adopted` (в шапке — «запущен вне панели», «Остановить» недоступна: процесс не наш).
Подхват делается только по явному запуску и только когда своих сессий на машине нет — иначе
занятый порт принадлежит соседней копии. Плюс список компонентов у подхваченной сессии
сверяется с `git ls-files` копии: индекс без пересечения по путям сториз считается чужим, и
список берётся из репозитория. Ключ сессии включает каталог копии — у `project:<agentId>` путь
меняется в настройках машины, и без этого панель показывала индекс прежнего репозитория как свой
(поймано на стенде).

**Падение команды видно сразу.** Команда уходит в живой shell, и его `pty.exit` не наступает,
когда падает только она: «Missing script: storybook» выглядел как бесконечная сборка до самого
таймаута. Поэтому к команде дописывается строка-сентинел `printf '\n__VCSB_<id>_%d__\n' $?`
(тот же приём, что в `console_run`: `%d` в эхе ввода не подходит под `(\d+)`), и её появление
до готовности переводит сессию в `failed` с кодом выхода.

**Кадр стори открывается тремя путями, и выбирает их сервер.** `POST …/components/storybook/open`
отвечает `ProjectStorybookAccess`: `direct` (Storybook на той же машине, где браузер — идём на
`127.0.0.1` без посредников), `tunnel` (companion-агент рядом с браузером поднимает локальный порт
до машины Storybook — тот же `AgentRegistry.createTunnel`, что у feature-preview) и `proxy`
(HTTP через агента). Панель сначала сама пробует прямой адрес (`fetch` к `/index.json`, 1,5 с), потом
спрашивает сервер, и только затем остаётся на прокси. Причина: замер на проде показал, что один
`iframe.html` через мост идёт 5,6 секунды, а Vite тянет сотни модулей — кадр просто не собирается.
Туннель закрывается при уходе с вкладки, id детерминированный (`sha256` от пользователя, копии,
машины и порта), и `DELETE` пересчитывает его заново: чужой туннель не погасить, зная id.

**Ответы машины кэшируются на минуту** (`apps/server/src/routes/machineCache.ts`): только GET,
только 200, только статика (js/css/json/шрифты/картинки) до 2 МиБ, общий предел 64 МиБ. Браузеру
уходит `ETag` от **переписанного** тела и `cache-control: private, max-age=60`, поэтому повторные
загрузки модулей не идут до машины вовсе. HTML не кэшируется никогда — в него инъецируются шимы.

**Ошибка кадра — страница, а не JSON.** Прокси отличает запрос документа (`sec-fetch-dest`,
`accept: text/html`) и отдаёт `previewErrorPage` с причиной и кнопкой повтора: раньше в кадре
висел сырой `{"error":"preview_unavailable"}`.

**Быстрый тикет** (`apps/server/src/components/componentTicket.ts`) закрывает единственный
разрыв между правкой руками и merge-раном: запись `ci_workspaces` с `pushed = 1` создавал
только dev-ран. Сервис делает ровно то же, что CI: ветка по `ciBranchTemplate` от базовой,
коммит выбранных путей, push, `createCiWorkspace` + `updateCiWorkspaceRevision(pushed=true)`,
и карточка сразу заводится в колонке `awaiting_merge`. Права на копию проверяются **до**
создания задачи — иначе на доске оставался бы тикет без ветки, у которого «Слить» никогда не
включится. Рабочая копия возвращается на базовую ветку в `finally`, даже после отказа push:
общая папка проекта, оставленная на ветке задачи, ломает системный ход проекта, который ждёт
