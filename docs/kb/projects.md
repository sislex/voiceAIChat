...
статус, а вычисляемое представление задач, у которых есть предложения со статусом
`new` или `accepted`. Источник — отдельный REST-запрос списка таких задач; карточка
остаётся в своей исходной колонке, сохраняет `column_id` и успешный результат CI.
Поэтому представление не участвует в development-, QA- и merge-переходах и не
меняет их гейты. Реализация отображения находится в
`packages/ui/src/components/kanban/KanbanBoard.tsx`, выборка — в
`apps/server/src/db/database.ts`.

Карточка в представлении показывает число актуальных предложений и исходное имя
колонки. Нажатие открывает обычную `TaskModal` сразу на вкладке `improvements`;
