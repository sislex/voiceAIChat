...
Новые проекты получают девять системных колонок: «Бэклог», «Готово к разработке»,
«В разработке», «Автотестирование», «Создание сценариев ручного QA», «Ручное QA»,
«Ожидает мержа», «Требуется решение», «Готово». Их машинный смысл хранит `semantic_type`, поэтому
