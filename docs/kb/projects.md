...
поддержаны навигация в настройки/карточку, предложения task patch,
перефразирования, критериев приёмки и безопасного набора пользовательских
настроек (включая provider/model). Навигационные команды выполняются сразу.
Все `propose.*` сначала рендерятся `WidgetProposalCard` как before/after;
в стор они попадают только после кнопки «Применить», «Отмена» ничего не меняет.
Этот же контракт предназначен для будущих адаптеров проводника, консоли и
браузера. Служебный чат по умолчанию динамически наследует
...
разговора. Карточки мутирует только `kanban.task.update`: после отображения
`WidgetProposalCard` клиент передаёт подтверждение, id сообщения-предложения,
идемпотентный ключ и `expectedVersion=Task.updatedAt`. Сервер отклоняет чужой
scope, неподтверждённый или устаревший action, повтор того же ключа в пределах
жизни процесса возвращает сохранённый в памяти результат с `replayed=true`.
Успешное применение пишет структурированное событие `widget.action` в runtime-log
и вызывает `boardHub.emit`; отдельного хранилища аудита или идемпотентности у
шлюза нет. Обычный `board.update` заменяет снапшот активной доски в UI, поэтому
следующий query видит новую ревизию.
