...
Состояние сохраняется в `localStorage` по ключу версии `v2`, включающему устойчивый id пользователя и `projectId` доски; при смене контекста старое состояние очищается до загрузки нового. Заголовок отфильтрованной колонки показывает `N из M`. DnD строит `afterId`/`beforeId` по полному порядку задач, включая скрытые фильтром карточки, поэтому фильтрация не меняет порядок.
