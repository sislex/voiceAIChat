...
Граница текущего адаптера важна: `TaskChatPanel` хранит выбранные вложения и режим
доступа только локально, не передаёт вложения в `messages:add`/`claude.send`,
отключает голосовой ввод и задаёт пустой обработчик настроек разговора. Поэтому
переиспользуется визуальная поверхность обычного чата, но провайдер/модель,
инструменты, метаданные и голосовые действия обычной страницы здесь кодом пока не
подключены; это нельзя считать отдельной второй реализацией этих возможностей.
