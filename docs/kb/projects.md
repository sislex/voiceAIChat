...
машину владельца проекта. Участник видит безопасные метаданные связанной машины,
но управлять связью и глобальными настройками машины не может. Единый гейт
`canUseAgent(userId, agentId, projectId)` разрешает личную машину без контекста
либо связанную машину действующему незаблокированному участнику только при явно
переданном проекте. Его используют ход чата, REST команд/файлов и PTY; после
удаления связи или участника следующий запрос сразу получает отказ.
