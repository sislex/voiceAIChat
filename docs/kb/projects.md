...
**Панель канбан-ассистента сохраняет функции доски, но визуально следует живому Make-макету «Проект 14».** Компоновку, зелёную палитру, поверхности, интервалы и адаптивный переход к вертикальному split задают токены и правила `.widget-assistant` в `packages/ui/src/styles/app.css`; при узком экране переключение виджета и чата начинается на 820 px. `KanbanAssistant` в `packages/ui/src/components/KanbanAssistant.tsx` явно различает загрузку, пустой разговор, активное и потоковое состояние, ошибку и отсутствие транспорта. Ошибка загрузки предлагает повторить запрос, транспортная ошибка оставляет ввод доступным, а новый submit очищает прежнюю ошибку. Контекст проекта и открытой задачи остаётся видимым и обрезается без расширения панели.
