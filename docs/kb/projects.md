...
При выборе TODO UI получает доску и ищет колонку по стабильному `semanticType === 'backlog'` (с запасным вариантом — первая колонка), а не по отображаемому имени «TODO». Выбор InProgress создаёт задачу и запускает CI через существующее действие; выбор работы в чате карточку не создаёт. Новый пользовательский ход через `answerQuestions` отправляется только для выбора «Работать в текущем чате»: создание карточки в TODO или InProgress не продолжает работу модели в исходном чате. Набор допустимых семантик колонки определён в `packages/shared/src/projects.ts`: `backlog`, `ready`, `development`, `testing`, `awaiting_merge`, `done`, `custom`.
