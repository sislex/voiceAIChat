...
(`awaiting_merge`) и «Готово» (`done`). Исключения попадают в
`decision_required`; автоматический выход из неё запрещён. Машинный смысл
хранит `semantic_type`, поэтому переименование подписи не меняет workflow.
`migrate()` идемпотентно досеивает отсутствующие новые стадии; legacy
`testing`, `qa_preparation` и `merge` остаются читаемыми для старых досок и
истории.
...
Это общий чистый контракт переходов; серверный development-run дополнительно
отвергает старт из `backlog` и `preparation`.

Общий QA-контракт находится в `packages/shared/src/qa.ts`. Он описывает стабильные
структурированные тест-кейсы, UI-impact, затронутые компоненты и требуемое
Storybook-покрытие, а также SHA-привязанные связи кейса с автотестом.
`canConfirmDevelopmentReadiness` не допускает Ready без функциональных требований,
критериев приёмки, полного обязательного кейса и определённого UI-impact; для
UI-работы требуется компонент со Storybook-связью либо документированное исключение
с альтернативной проверкой. `canCompleteAutomation` требует для каждого
обязательного автоматизируемого кейса непустую ссылку на тест именно текущего SHA,
а для неавтоматизируемого — причину и альтернативную ручную проверку. Это чистые
quality-gate функции общего пакета; хранение и оркестрация полного Component QA и
Automated QA pipeline этой правкой не добавлены.
