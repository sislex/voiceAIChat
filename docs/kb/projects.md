...
Сам флаг — настройка взгляда и переживает перезапуск приложения: `projectsStore`
читает и пишет его через порт `prefs` (`vc.board.includeCompleted`), а `reset()`
при выходе берёт сохранённое значение заново. Туда же переехал **вид доски**:
