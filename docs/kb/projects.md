...
В шапке ассистента показывается список обычных бесед этого проекта: компонент
загружает `conversations:list` и оставляет элементы с совпадающим `projectId`.
Последний выбранный id хранится отдельно для каждого проекта в localStorage под
ключом `voicechat.projectAssistantChat.<projectId>` и перечитывается при смене
проекта. Нажатие сохраняет id и открывает беседу на её обычном маршруте; кнопка
«Новый чат» создаёт разговор, привязывает его к текущему проекту и также
открывает его.
