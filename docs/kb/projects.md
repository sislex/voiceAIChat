...
Карточка связывается с макетом, собранным в Make: связь — строка в таблице
`task_designs` (`task_id`, `conversation_id`, `path`, `label`), контракт —
`TaskDesignLink`/`ProjectDesignSource`/`MakeTaskLink`/`MakeLinkableTask` в
`packages/shared/src/projects.ts`. Отдельная таблица, а не JSON-колонка в `tasks`:
панель Make спрашивает обратное направление («какие карточки ссылаются на эту
страницу»), и по JSON такой запрос не построить.
