...
`resolveTaskLlmConfig` по обычной цепочке наследования. На вкладке «Workflow и CI»
владелец выбирает production-машину из уже привязанных машин проекта
(`ProjectDetail.productionAgentId`) и задаёт `productionDeployCommand`; команда
предназначена для выполнения на этой машине в её `project_machines.path`.
Активная вкладка хранится локально в компоненте и не сбрасывается, когда сервер
обновляет detail проекта.
...
`default_agent_id` и `production_agent_id` — обычные колонки без внешних ключей
на `agents`. Первая выбирает машину обычного выполнения/CI, вторая — хост
production-деплоя; обе должны входить в `project_machines`. Удаление самой машины
(не отвязка от проекта, а `DELETE /api/agents/:id`) автоматически их не трогает,
поэтому `db.deleteAgent` очищает обе колонки; то же делает `unlinkMachine`.
Без очистки проект ссылался бы на уже отсутствующую машину.
