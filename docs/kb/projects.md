...
HTTP: серверный реестр знает только разрешённый `kanban`. Перед каждым ходом UI
вызывает `widget:query` с семантическим снимком текущей доски. Непустой снимок
имеет приоритет (`source='ui'`); если UI-состояния нет или в нём нет элементов,
адаптер перечитывает доступную пользователю доску из БД (`source='api'`). Результат
попадает в эфемерный `assistantContext.toolResults`, а не в историю чата.
...
scope, неподтверждённый или устаревший action, повтор того же ключа возвращает
сохранённый результат с `replayed=true`, а успешное применение пишет
структурированный audit-log и вызывает `boardHub.emit`. Обычный `board.update`
заменяет снапшот активной доски в UI, поэтому следующий query видит новую ревизию.
