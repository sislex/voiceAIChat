...
**Улучшения.** Очередь читается один раз на проект и дальше только по адресному кадру
`project.improvements.updated` (`BoardHub.emitImprovements`; эмиты — в роутах
`/api/improvements/*` и после анализа завершённого рана в `ci/runManager.ts`). Кадр
уходит подписчику той же доски, что и `board.changed`.
