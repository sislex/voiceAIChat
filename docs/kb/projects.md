...
безопасные варианты, не отменяя и не заменяя процесс. Ручные циклы постоянно
хранятся в `task_rework_cycles`: сервер назначает sequence/автора/время, сохраняет
снимок Make-источников и одной транзакцией переводит задачу в системную колонку
`preparation`; уникальный `(task_id,idempotency_key)` исключает дубли, а повтор этого ключа возвращает сохранённую ревизию до повторной проверки изменившихся Make-источников. Вложения
исходной задачи и черновика живут в `task_attachments`; при создании цикла
черновые строки атомарно привязываются к неизменяемой ревизии и больше не удаляются
обычным attachment API. `TaskCardContainer` загружает историю и исходные вложения
с сервера, а Make-источники — при открытии формы с состояниями loading/empty/error
и retry. Повторная подготовка добавляет исходную постановку, все циклы по sequence,
их Make-снимки и вложения через общий `taskReworkContext`.
