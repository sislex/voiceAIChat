...
`TaskCardContainer` лениво загружает постоянную историю и разрешённые Make-проекты
при переключении на новую карточку; presentation-компонент показывает
loading/empty/error/retry, позволяет независимо выбрать whole-project либо файлы
каждого источника и отправляет снимок через постоянный IPC/HTTP-клиент. Legacy
остаётся начальным представлением и не зависит от новых запросов. Контекст
модельного рана дополняется исходными полями и всеми циклами по sequence, включая
Make-снимки и статусы вложений.
