...
безопасные варианты, не отменяя и не заменяя процесс. Постоянного серверного
хранилища, REST-контракта и загрузки вложений для ручных неизменяемых циклов пока
нет: `TaskCardContainer` принимает создание через опциональный
`onCreateReworkCycle`, а полученный цикл и его типизированные вложения держит
только в локальном React-состоянии до закрытия карточки.
