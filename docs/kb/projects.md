...
До открытия черновика `App.tsx` использует уже загруженный `ProjectDetail` этого проекта либо вызывает `fetchProjectDetail`, а затем сохраняет `members` прямо в состоянии предложения и передаёт их в `TaskModal`. Поэтому список исполнителей относится к проекту чата и не зависит от того, какой project detail пользователь откроет позже. Новый черновик начинается с `assignee: null`; селект предлагает «Не назначен» и только участников, у которых `active !== false`. Для TODO поля черновика уходят через `tasks:create`, а для подготовки — через `tasks:createFromProposalInPreparation`; серверная операция получает заголовок, описание, критерии, тип и родителя, приоритет, исполнителя, метки, навыки, оценку и срок, поэтому выбор целевого действия не обрезает заполненную карточку.
...
`KanbanBoard` предоставляет глобальный чекбокс «Показывать только мои задачи» и независимый селект исполнителя в заголовке каждой колонки. Сопоставление выполняется по `ProjectMember.username`/идентификатору текущего пользователя, а не по подписи аватара. Значения селектора: «Все исполнители», «Без исполнителя» и активные участники проекта.

Глобальный режим временно имеет приоритет над локальными селекторами: локальные значения сохраняются, селекторы становятся disabled с доступным пояснением и восстанавливаются после выключения режима. Фильтры объединяются с поиском, типом, приоритетом, метками, эпиками, флагом и недавними задачами; пустая колонка предлагает сброс локального фильтра, а «Сбросить фильтры» очищает общие и локальные значения.

Состояние сохраняется в `localStorage` по ключу версии `v2`, включающему устойчивый id пользователя и `projectId` доски; независимые значения колонок лежат в `columnAssignees` под id колонки. При смене пользовательского или проектного контекста прежнее состояние сначала сбрасывается, затем загружается состояние нового ключа. Заголовок при активных фильтрах показывает `N из M`, где `M` — полный размер колонки, а `N` — число карточек после совместного применения общих и локального фильтров. Поскольку фильтрация вычисляется из актуального `board` на каждом рендере, входящие обновления назначения, создания и перемещения задач сразу меняют видимость и счётчики.
...
272 px, не сжимаются и являются вертикальными flex-контейнерами. Их шапки и
композеры неподвижны, а каждый `.jcol-body` занимает оставшуюся высоту и независимо
прокручивается по вертикали. Поэтому горизонтальная позиция принадлежит доске, а
вертикальная — отдельному списку карточек; прокрутка не увеличивает документ и не
сдвигает заголовки соседних колонок. Глобального перехвата `wheel` нет:
колесо, тачпад и обычный touch-жест обслуживаются нативными scroll-контейнерами,
а `touch-action: none` задан только ручкам захвата.
...
индексах. Поэтому `scrollLeft` поверхности и `scrollTop` списков не сбрасываются
...
Режим свимлейнов намеренно устроен иначе: `.jboard--lanes` получает общий
вертикальный scroll, а ячейки `.jcol--incell` имеют автоматическую высоту и видимое
переполнение. Таким образом обычная доска хранит независимую вертикальную позицию
каждой колонки, а поверхность свимлейнов прокручивается целиком.
...
- **Цель считает доска, а не движок.** Ячейка (колонка × дорожка свимлейна) — по
  ...
  плейсхолдера. При переносе карточки `autoScroll` двигает по горизонтали только
  `.jboard`, а по вертикали — только `.jcol-body` активной колонки. При переносе
  колонки автоскролл работает исключительно по горизонтальной оси `.jboard` и не
  меняет вертикальные позиции поверхности или списков.
