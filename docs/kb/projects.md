...
Связь общей машины хранится в `project_machines`: составной PK гарантирует
уникальность пары, FK каскадно удаляют связь вместе с проектом или машиной, а
`added_at` / `added_by` фиксируют добавление. Привязать можно только собственную
машину владельца проекта; повторная привязка отвечает конфликтом. Маршруты списка
машин проекта доступны всем участникам, а список ещё не подключённых личных машин,
добавление и удаление связи — только владельцу. Участник получает безопасные
метаданные (имя, владелец, время добавления и вычисленный через `AgentRegistry`
online-статус), но не может менять связь или глобальные настройки машины.

Единый гейт `canUseAgent(userId, agentId, projectId)` разрешает личную машину без
контекста либо связанную машину действующему незаблокированному участнику только
при явно переданном проекте. Его используют ход чата, REST команд/файлов и PTY;
после удаления связи или участника следующий запрос сразу получает отказ.
