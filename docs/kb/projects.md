...
`openOrCreateTaskChat`, когда задан пользовательский `source`; это охватывает
обычный REST и `task-launch`, включая повтор по `Idempotency-Key`. Для остальных
участников чат создаётся при первом открытии карточки: `TaskModal` зовёт
`ensureTaskChat` (тот же идемпотентный `tasks:openChat`, но без перехода на чат).
Системные создания без `source`, эпики и стори чат автоматически не получают.
...
(«Component QA · Выполняется»), номером цикла, кнопками «Открыть чат» и
...
`TaskChatPanel` из `packages/ui/src/components/kanban/TaskModal.tsx`. Legacy
монтирует её по кнопке на вкладке «Общее», новая карточка — через `renderPanel`
контейнера для вкладки `chat`. При открытии панель идемпотентно получает связанный
разговор через `tasks:openChat`, затем читает его сообщения через
`conversations:get` со scope `kanban` и `projectId`; поэтому показывается
сохранённая история именно этой задачи, а сервер применяет проектный доступ.
Пользовательское сообщение сначала сохраняется через `messages:add`, затем его id
передаётся в `window.claude.send`; сохранённый ответ добавляется только из
`onDone` того же `conversationId`. Панель отдельно показывает загрузку,
отправку и ошибку, сохраняет текст неудавшегося хода для «Повторить» и не блокирует
остальные редактируемые поля карточки. На каждом ходе актуальные название,
описание, критерии и статус задачи добавляет серверный task-chat context; источник
и проверки — `apps/server/src/taskChatContext.test.ts`.
