...
**Источник ограничен Make-проектами, привязанными к тому же проекту**
(`projects:designSources` → `GET /api/projects/:id/design-sources`). Причина —
доступ: связать можно только то, что откроется у всей команды. Привязка Make-чата
к проекту (настройки чата) и есть акт, открывающий его участникам на чтение —
`VoiceChatDb.isMakeProjectViewer` пускает участника проекта в `access(…, 'viewer')`
роутов Make, включая `GET /api/preview/make/:id/*`. Раньше превью открывал только
владелец, и связанный макет у остальных не грузился бы вовсе.
...
Карточка показывает тот же отсортированный список и точечные ошибки недоступности;
preview остаётся только UI-действием для режима всего проекта. В панели Make — пункт
