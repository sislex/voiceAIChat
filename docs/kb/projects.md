...
карточка обязана показывать текущее состояние экрана; `path` — файл внутри
проекта (`index.html`, `src/components/Card.jsx`), пустая строка — «проект
целиком». Путь нормализуется тем же `normalizeMakePath`, что и файлы Make.
...
(`packages/ui/src/components/kanban/TaskDesigns.tsx`): список связей с подписью,
страницей, «Открыть в Make» (`onOpenMake` → маршрут `#/make/<id>`) и ссылкой
«Превью» (`makeDesignPreviewUrl` из `@shared/protocol`; показывается только для
адресуемых превью путей — html и «проект целиком»). Источники и список файлов
грузятся лениво, только когда форма связывания открыта. В панели Make — пункт
меню «⋯» → «🗂 Задачи проекта» (`MakeTaskLinksDialog`): связанные карточки,
переход на доску и связывание открытой страницы с задачей.

**Дизайн уходит модели.** `designPromptLines` (shared) строит строки «Дизайн:
«имя» — Make-проект <id>, страница <path>; превью <url>»; их добавляют и ход чата
