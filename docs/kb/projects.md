...
Для ролей `developer`/`tester`/`observer` действует квота на проекты, где пользователь состоит владельцем: по умолчанию `DEFAULT_OWNED_PROJECT_LIMIT = 5`, текущее значение хранится в `app_config` под ключом `projects.ownedLimit`. `POST /api/projects` читает настройку на каждый запрос и при исчерпании отвечает `409` с понятным текстом; удаление проекта освобождает слот, роль `admin` не ограничивается. `GET /api/projects/quota` (мост `projects:quota`) возвращает `{ owned, limit, unlimited }` для подсказки в `NewProjectDialog`: при последнем месте окно предупреждает, при исчерпании блокирует кнопку создания. Администратор меняет значение в секции открытой регистрации через `GET/PUT /api/admin/signup`; изменение сразу применяется без перезапуска.
...
Встроенное дерево: `type-software` «Разработка ПО» (все возможности) →
`type-software-web` «Веб-приложение» (возможности наследует целиком, отличается
только заготовками; существует как точка опоры для соседей и личных подтипов без
превью) и `type-general` «Общий проект» (все возможности выключены, своя короткая
доска из пяти нейтральных колонок). Узлы сеются идемпотентно при открытии базы,
помечены `builtin` и не правятся даже админом.
