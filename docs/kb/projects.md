...
`qa_preparation_runs`. Запуск идемпотентен для активного рана задачи; ошибка,
отмена или рестарт сервера оставляют карточку в `preparation` и разрешают новую
попытку. Карточка и модальное окно показывают действие «Начать подготовку задачи»,
статус, ошибку и отдельную realtime-ленту ответа модели.
