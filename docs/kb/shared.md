...
Клиент загружает индексы `chat`, Web Reader, Playwright Reader, Console и Make отдельными запросами и при открытии передаёт область известного разговора. Поэтому сохранённый URL после F5 восстанавливает разговор только в своей мастерской, а id из другого маршрута сервер возвращает как отсутствующий; Kanban использует отдельный проектный запрос. Реализация выбора и раздельных коллекций находится в `packages/chat-app/src/store/chatStore.ts`.
...
а пятиминутный тариф с фактом не сходится. Строки sonnet и haiku остаются непроверенными по фактической цене CLI. Для
OpenAI/Codex `pricing.ts` теперь содержит отдельные официальные записи по точному
modelId (`gpt-6-astra`, модели `gpt-5.6` и прежние каталоговые id), а не семейный
regex: неизвестная или пустая модель остаётся unpriced. Без явных метаданных
выбирается Standard/short context; Standard/long применяется только при
`usage.contextTier = 'long'`, а неизвестный специальный режим не подменяется
базовым тарифом. Тест `pricing.test.ts` сверяет расчёт с
...
Промпт собирается централизованно. Здесь добавляются история, speaker labels, пути вложений, контекст проекта/KB и инструкции для `tool/questions/image/task-launch` — последние берутся из `chatInstructions.ts` по включённым пунктам `Settings.chatInstructions` (см. [llm.md](llm.md), «Договорённости в тексте ответа»). Служебные fenced-блоки затем вырезаются из произносимого текста, чтобы TTS не читал JSON.
...
`RendererApi` remains a complete mapped contract: every current IPC channel is
required from the active host bridge. Independently versioned UI libraries keep
their generic test fake forward-compatible with later additive channels instead
of weakening the application contract. The Core UI release gate then verifies
that its concrete HTTP bridge implements the full pinned contract.
