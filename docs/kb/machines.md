...
## Выбор машины на каждое сообщение

Цель выполнения — свойство сообщения, а не только глобальная настройка:
`claude.send.execTarget` (id машины | `null` — сервер | `'none'` — команды
запрещены), `undefined` → берётся `settings.execTarget`. Разрешение цели в
...
`--permission-mode plan`. Выбранная цель пишется снимком в `messages.exec_target`
и видна в истории.
