...
Изменяемая цель хранится в `conversations.exec_target` и выбирается в сайдбаре
отдельно для каждого чата: id машины | `null` — сервер | `'none'` — команды
...
`--permission-mode plan`. В `messages.exec_target` пишется неизменяемый снимок:
в истории вопрос и ответ лишь показывают, где выполнялся их ход, без селектора.
