...
continuation, поэтому поздний callback ничего не запускает.

Артефакты выбираются endpoint-реестром по platform/arch; сейчас единственная
запись — `macos/arm64`. Download endpoint отдаёт DMG напрямую с
