...
The companion runtime and its installers now live in `sislex/agent`. Protocol
and capability minimum versions come from `@sislexa/agent-contracts`. Core serves
the released `@sislexa/agent/voicechat-agent.cjs` bytes without esbuild or local
Agent source. The owner gate executes that exact bundle against a temporary WS
server and verifies registration/command execution; it also checks the VPN guard
entry and retains the optional native PTY/fallback tests.

Core retains the machine registry, user/token authorization, orchestration and
server/agent integration tests. Shared only re-exports the published machine
contract to keep previously released Make/Reader/Identity/UI consumers compatible;
there is no local protocol implementation or duplicate owner unit suite.
Desktop installer lookup uses configured `VC_AGENT_APP`, `VC_DESKTOP_APP` and
`VC_LOGIN_APPLICATION`, or matching `artifacts/` directories. Removed source
folders are never an installer discovery dependency.
...
Three installation options are the published standalone `voicechat-agent.cjs`,
the tray client, and source execution in `sislex/agent`. Core serves the owner's
bundle through `agents/agentScript.ts`; it never embeds an address or token.
Connection credentials are supplied at launch using `VC_AGENT_SERVER` and
`VC_AGENT_TOKEN` or a connection string. Android uses Termux and the public
`/api/agents/install-android.sh`, generated by the owner's `installers/androidInstall.ts`.
The Android guide is in `sislex/agent/apps/agent/ANDROID.md`.

Windows — PowerShell-установщик
...
The September 13 checks below predate extraction. Current protocol and client
checks run in the Agent/Desktop repositories; Core gates consume their published
artifacts and retain public integration coverage.
