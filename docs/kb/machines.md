...
Изменяемая цель хранится в `conversations.exec_target` и выбирается рядом с
названием активной беседы отдельно для каждого чата: id машины | `null` — сервер |
`'none'` — команды
запрещены. Смена цели одного чата не затрагивает остальные. Удаление машины
сбрасывает на сервер только разговоры, которые ссылались на неё.
