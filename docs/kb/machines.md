...
`/api/agents/version`). Текущий релиз — 0.11.2; он включает безопасный файловый
Git credential helper для headless Linux. Поэтому подключённый агент 0.11.1
считается устаревшим: UI показывает доступную v0.11.2, кнопку обновления и команду
для ручного обновления. `TOOL_MIN_VERSION` задаёт минимальную версию агента для
