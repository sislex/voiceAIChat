...
**Второй чекаут монорепо рядом с первым** (параллельная работа над двумя задачами) — `git worktree add ../voiceAIChat1 -b <ветка>`: общий `.git`, отдельное рабочее дерево. `node_modules` не разделяются, `npm install` нужен свой; `.env` в `.gitignore`, поэтому копируется руками. Разводится три вещи, иначе два сеанса дерутся: порты (`PORT`, `VC_API_PORT`, `VC_WEB_PORT`, `VC_RECORDER_PORT`), каталог данных (`VC_DATA_DIR` — по умолчанию `~/.voicechat-server` общий для обоих чекаутов, две SQLite в один файл) и `VC_PUBLIC_URL` (ссылки писем ведут на Vite своего чекаута). Тяжёлые артефакты дублировать не нужно: `VC_WHISPER_CLI`, `VC_MODELS_DIR`, `VC_PIPER_BIN`, `VC_PIPER_VOICES_DIR` в `.env` второго чекаута указываются на первый — `dev-web.sh` перебивает их только если найдёт файлы локально, а в чекауте без `apps/desktop/node_modules` и `.venv-piper` их нет. Там же был скрытый дефект: `[ -f "$X" ] && export ...` под `set -e` возвращает 1 и роняет скрипт, когда артефакта нет; в основном чекауте файлы есть, поэтому дефект не проявлялся, и починен добавлением `|| true`.
