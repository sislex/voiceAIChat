...
Для проверки уже собранного Web Reader на одном origin отдельные Vite-процессы не нужны: сервер запускается через `node_modules/.bin/tsx apps/server/src/index.ts` с `HOST=127.0.0.1`, свободным `PORT`, временным `VC_DATA_DIR`, `VC_ADMIN_PASSWORD` для тестового пользователя и абсолютными `VC_WEB_DIR=<repo>/apps/web/dist`, `VC_WEB_RECORDER_DIR=<repo>/apps/web-recorder/dist`. Для полного браузера отдельно запускается `apps/browser-runner/src/index.ts` на другом порту с временным `VC_BROWSER_DATA_DIR` и общим `VC_BROWSER_RUNNER_TOKEN`; сервер получает `VC_BROWSER_RUNNER_URL` и `VC_BROWSER_PREVIEW_BASE`, а раннер — `VC_BROWSER_PREVIEW_ORIGIN` с адресом сервера. Для страниц настроек также нужен TTS Runner (`VC_TTS_RUNNER_URL`/`VC_TTS_RUNNER_TOKEN` у сервера): `/api/tts/voices` и `/api/tts/catalog` запрашиваются при открытии UI, даже без синтеза речи; без сервиса стенд получает HTTP 500, которые Reader отображает как сетевые ошибки вложенной страницы. После изменения UI требуется новая сборка и reload. При проверке HTTP с DNS-алиасом Chromium адрес диагностики нужно брать от origin самой вкладки или использовать `https://app.internal/api/preview/diagnostics`: другой loopback-host не становится доверенным только потому, что ведёт на тот же процесс. Вложенный проект по `app.internal` имеет отдельную cookie-сессию; форму его входа заполняют тестовым пользователем отдельно от внешнего Reader.
...
**Почему их не поймал typecheck: в `apps/server` были выключены `noUnusedLocals` и
`noUnusedParameters`.** Флаги включены, 42 ошибки разобраны — среди них нашлась
ещё одна мёртвая функция, `runKbUpdateStep` в `ci/runManager.ts` (48 строк):
встроенный CI-шаг «Актуализировать базу знаний» переехал в `merge/runManager.ts`,
а реализация в CI-ране осталась. Теперь такой код ловится на компиляции, и
ручной поиск по счётчику вызовов нужен только для методов, которые вызывает
продукт, но не вызывают тесты.
...
открывал домашнюю БД пользователя: гейт зависел от её схемы `model_prices` и мог
...
`buildServer()` в тесте всегда получает временный `VC_DATA_DIR`, даже если тест проверяет
только HTTP/WS и не работает с данными явно. Иначе `loadConfig({ PORT: '0' })` открывает
`~/.voicechat-server/voicechat.db` пользователя: состояние другого checkout делает тест
недетерминированным (в `server.test.ts` это проявилось как `model_prices has 10 columns
but 9 values were supplied`). Несколько серверов получают разные подкаталоги, закрываются
до удаления корневого временного каталога; cleanup допускает отказ первоначального запуска.
...
`releases.pruneProgressEvents()` через 15 с после старта. Вторая найденная причина (12:47 того же дня, уже на Postgres): исходящая очередь WebSocket клиента, который не
вычитывает кадры, — `ws` копит буферы без предела (127 МБ ArrayBuffer в снимке, аллокации из `session.ts`).
Предохранитель в `ws.ts`: при `bufferedAmount` выше `WS_MAX_BUFFERED_BYTES` (8 МБ) соединение рвётся, в stdout
уходит `[ws] исходящая очередь переполнена` со счётчиками кадров по типам — по ним искать источник потока. Очереди
живых сокетов смотреть через инспектор: `process._getActiveHandles()` → `bufferSize`/`_writableState.length`
(скрипт `sockets.cjs` в журнале). Профиль аллокаций живого процесса: открыть инспектор
