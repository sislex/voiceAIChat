...
Разрезка набора по файлам вскрывает скрытые общие ресурсы, и один такой нашёлся сразу: обвязка собирала каталог данных как `join(tmpdir(), 'vc-rest-test-' + Date.now() + '-' + id)`, где `id` — счётчик, начинающийся с нуля **в каждом файле**. Пока все 138 тестов жили в одном файле, они шли последовательно и имена почти не сталкивались; после разрезки пять файлов идут в разных воркерах, и два теста в одну миллисекунду получали один каталог — один сносил данные другого. Проявлялось как падение make-теста «Публикация не найдена или снята», недетерминированное и только в полном прогоне пакета (файл в одиночку проходил всегда). Правильный приём — `mkdtempSync`, который уникальность обеспечивает средствами ОС, плюс `rmSync` в `afterEach`. Общее правило: **имя временного ресурса в тесте не собирается из времени и счётчика.**
