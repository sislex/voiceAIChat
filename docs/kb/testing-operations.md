...
[--run ЧАСТЬ] [--json]` — таблица «во что обошёлся ран» по данным БД: стоимость
...
строкам `[tool_use] имя: …` — такие числа помечены «~». Методика и вердикт
последнего замера — в `features/ci-runner.md`.
