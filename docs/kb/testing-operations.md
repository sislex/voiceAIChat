...
множитель не виден. `--context` добавляет состав контекста: директива задачи,
инъекция БЗ, ответы инструментов с разбивкой по видам, три самых тяжёлых ответа и
запись кэша (по самому скромному ходу — это верхняя оценка «системный промпт CLI +
схемы инструментов + директива», отдельной строки в БД у него нет). Объём ответов
берётся из `ci_run_tool_calls.chars` и `ci_run_tool_responses`, а у ранов до этих
метрик восстанавливается из ленты (`[tool_result] … · полный текст`) и помечается
