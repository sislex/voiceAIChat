...
Статус: круги 1–3 ☑ (2026-09-08). Ветка `feat/reader-service` от `main` (после #118, #119, #121, #122).
...
### Круг 3 — прогон на прод-копии, документация, PR ☑ (2026-09-08)
1. ☑ Прогон на копии прод-БД (Postgres, 113 таблиц, расхождений 0): ядро 8799 (`VC_READER_MODE=remote`) + ридер
   8795. `GET /api/preview?url=https://example.com/` через ядро и напрямую у ридера — 200, ссылки переписаны в
   `/api/preview`; без сессии — 401 у обоих; `/api/preview/diagnostics` — 200; `/mcp/preview` с подписанным
   токеном хода — ответ relay ядра по RPC («клиент не подключён» без открытого приложения), с поддельным токеном —
   «контекст хода недоступен». Bearer для прогона подписан секретом сессии копии (`session.secret` каталога данных),
   пароли не вводились. Панель браузера вживую не проверялась — логика та же, что во встроенном режиме, и покрыта
   интеграционным тестом с WS-клиентом.
2. ☑ Найден и закрыт дефект одновременного старта на Postgres: два процесса ставили схему `CREATE TABLE IF NOT
   EXISTS …` наперегонки → ядро упало с deadlock (40P01) на `conversations`/`messages`; в тестовых схемах гонка
   `CREATE SCHEMA IF NOT EXISTS` даёт duplicate key. `VoiceChatDb.init()` теперь ставит схему под
   `pg_advisory_xact_lock` — тест `db/database.pgBootstrap.test.ts` (4 экземпляра одновременно, только на Postgres).
   Этапы 2–3 этой гонки не ловили случайно: compose стартует все сервисы разом.
3. ☑ KB: `docs/kb/deploy.md` (таблица распределённого стенда, одновременный старт), `server-internals.md`
   (раздел «Web Reader»), `data-auth.md` (замок схемы), `apps/server/AGENTS.md`, журнал; PR.
