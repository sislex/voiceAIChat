...
Статус: круги 1–3 ☑, прогон на прод-копии ☑ (2026-09-07). Ветка `feat/machines-service` поверх
...
4. ☑ Прогон на копии прод-БД (Postgres, 113 таблиц, расхождений 0): четыре процесса — машины 8793, канбан 8789,
   админка 8794, ядро 8799 в `remote` для всех трёх. API через ядро: `/api/agents` (машины), проекты/доска/релизы
   (канбан), `/api/admin/users|machines/stats|security|make/stats` (админка), `git/workspaces` и сессия — у ядра;
   всё 200. Браузер: доска, страница машин, «Пользователи» — запросы 200, консоль и логи четырёх процессов чистые;
   тосты watchdog из процесса машин дошли до сессии ядра по шине событий. Порядок старта не важен: канбан при
   недоступном ядре предупреждает и догоняет снимок машин следующим пушем.

### Круг 4 — доводка распределённого режима ☑ (2026-09-08)
Цель — чтобы процессы могли жить на разных хостах, а не только в одном compose с общим томом.
1. ☑ Вложения канбана через порт: `KanbanUploads.read(id)` отдаёт байты локального вложения с диска ядра
   (RPC `uploads.read`, base64); `routes/qa.ts` больше не читает `upload.path` — общий том `/data` процессу
   канбана не нужен (скриншоты QA он хранит у себя).
2. ☑ `KanbanService.previews.list()` — список живых feature-preview для preview-MCP чата; в remote — RPC
   `previews` к процессу канбана. Ленивая ссылка `featurePreviewsRef` из `KanbanDeps` убрана.
3. ☑ `ptyBufferText` у ядра в remote — полный кольцевой буфер процесса машин по RPC (у порта тип
   `string | null | Promise<…>`, консольный MCP ждёт `await`); свой частичный буфер у `HttpMachines` убран.
4. ☑ Runbook распределённого стенда — `docs/kb/deploy.md`, раздел «Распределённый стенд».
