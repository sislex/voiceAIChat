...
недели работы и смена прод-инфраструктуры. Вместо этого доказано, что путь замены движка
работает, на одном домене: `releases` (единственный, кого никто внутри слоя не зовёт
синхронно) получил вторую реализацию на Postgres за тем же `AsyncPort`, тесты — на встроенном
`@electric-sql/pglite`, прод и `docker-compose` не тронуты. Что показал PoC: у домена на другом
движке нет общего соединения с соседями — членство в проекте он спрашивает через их порты
(`ports.projects`), а FK на `projects(id)` у него нет, и каскад удаления проекта до его таблиц
не доходит. Это и есть цена разделения, которую нужно закрывать событиями/методами владельца,
когда домен реально уедет. Остальные домены — когда первый из них уедет в отдельный сервис.

| № | Пункт | Статус |
|---|---|---|
| 1 | Ядро принимает реализацию домена снаружи: `DbDeps.ports` — фабрики `(ports) => AsyncPort<Repo>` поверх собранных портов соседей (`PortOverrides` в `repos/base.ts`) | ✅ |
| 2 | `ReleasesPgRepo` (`db/pg/releasesPg.ts`) на встроенном Postgres за узким `SqlClient` (`db/pg/sqlClient.ts`, адаптер pglite; `pg` подключается тем же интерфейсом); контрактный тест `releases.contract.test.ts` гоняет SQLite и pglite одним набором | ✅ |
| 3 | Переключатель `VC_DB_RELEASES=sqlite|pglite` (+ `VC_DB_RELEASES_DIR`), по умолчанию SQLite; сервер с pglite поднимается и создаёт каталог кластера | ✅ |
