...
describe('Reader: модель управляет настоящим App через relay и WebSocket', () => {
  beforeAll(async () => {
    ...
    app = await buildServer({ config: loadConfig({ ...process.env, PORT: String(port), HOST: '127.0.0.1', VC_DATA_DIR: dataDir, VC_ADMIN_PASSWORD: 'reader-model-fixture-only', VC_WEB_DIR: resolve('node_modules/@sislexa/core-ui/web'), VC_BROWSER_HOST_ALIASES: '93.184.216.34:8787=127.0.0.1:' + port }), previewRelay: relay })
