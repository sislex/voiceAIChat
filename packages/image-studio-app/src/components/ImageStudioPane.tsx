...
import type { ImageStudioPaneProps } from '../panelContract'
export type { ImageStudioPaneProps } from '../panelContract'
...
import { buildZip } from '@voicechat/ui-foundation/lib/zipStore'
...
import { getCachedPreview, putCachedPreview } from '@voicechat/ui-foundation/lib/previewCache'
import { playStopCue } from '@voicechat/ui-foundation/lib/cues'
...
export function ImageStudioPane({ conversationId, api, turnActive, onAttachToChat, otherChats }: ImageStudioPaneProps): JSX.Element {
  ...
  const blobOf = async (path: string): Promise<Blob> => {
    ...
    if (preview) {
      const cached = await readBrowserBlob(preview).catch(() => null)
