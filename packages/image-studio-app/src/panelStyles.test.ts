...
const { decl } = cssRules(
  [
    new URL('../../ui-foundation/src/styles.css', import.meta.url),
...
describe('app.css — лайтбокс студии', () => {
  it('картинка стоит по центру окна, а не в левом верхнем углу', () => {
    ...
    expect(decl('.imgbody > .image-studio-zoom-stage', 'margin-inline')).toBe(
      'auto'
    )
    expect(decl('.util-embed--img .imgbody', 'flex')).toBe('1 1 auto')
    expect(
      decl(
        '.util-embed--img .imgbody > .image-studio-zoom-stage',
        'margin-block'
      )
    ).toBe('auto')
  })
  ...
  it('кнопки-ссылки в свойствах выровнены по левому краю', () => {
    ...
    expect(
      decl('.image-studio-props > .image-studio-cancel', 'justify-self')
    ).toBe('start')
  })
