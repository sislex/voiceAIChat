...
const fakeSignup: { enabled: boolean; role: import('@shared/types').UserRole; mailConfigured: boolean; ownedProjectLimit: number; sessionLimit: number } = { enabled: false, role: 'developer', mailConfigured: false, ownedProjectLimit: 5, sessionLimit: 0 }
...
export function createFakeApi(seedConversations: string[] = []): FakeApi {
  ...
  const api: FakeApi = {
    ...
    'admin:setSignupConfig': async ({ enabled, role, ownedProjectLimit, sessionLimit }) => { if (enabled !== undefined) fakeSignup.enabled = enabled; if (role) fakeSignup.role = role; if (ownedProjectLimit) fakeSignup.ownedProjectLimit = ownedProjectLimit; if (sessionLimit !== undefined) fakeSignup.sessionLimit = sessionLimit; return { ...fakeSignup } },
    ...
    'releases:applicationCatalog': async () => [],
    'releases:applicationOverview': async () => ({ environment: { schemaVersion: 1, revision: 0, applications: [] }, activeDeploymentId: null, releases: [], deployments: [] }),
    'releases:applicationPrepare': async ({ projectId, input }) => ({ id: 'app-prepare-1', projectId, input, branch: `release/${input.applicationId}/${input.version}`, status: 'preparing', manifest: null, createdAt: Date.now(), finishedAt: null, triggeredBy: 'admin', log: '' }),
    'releases:applicationObserve': async () => ({ environment: { schemaVersion: 1, revision: 1, applications: [] }, activeDeploymentId: null, releases: [], deployments: [] }),
    'releases:applicationDeploy': async () => { throw new Error('Настройте результат deploy в тесте') },
    'releases:applicationReconcile': async () => ({ environment: { schemaVersion: 1, revision: 1, applications: [] }, activeDeploymentId: null, releases: [], deployments: [] }),
    'releases:branches': async () => [],
    'releases:machines': async () => ({ machines: [], lastAgentId: null }),
    ...
    'projects:quota': async () => ({ owned: projects.length, limit: 5, unlimited: false }),
...
import type { RendererCiBridge } from '@voicechat/shared'
import type {
  CiCommand,
  CiCommandInput,
  CiGlobalSettings,
