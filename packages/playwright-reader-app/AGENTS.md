# @voicechat/playwright-reader-app — Playwright Reader UI
...
- Гейт: `npm run gate:app -- playwright-reader-ui`; внутренние правки проверяют только приложение.
- Сборка: `npm run -w @voicechat/playwright-reader-app build`; watcher: та же команда с `dev`.
...
- Публичный контракт добавляет адресную проверку host. Релиз `playwright-reader-ui` использует
