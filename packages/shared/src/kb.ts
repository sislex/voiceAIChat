 ...
 *
 * Вторым абзацем идёт `KB_GAP_RULE` (kbGaps.ts): чтение базы без правила
 * «пробел закрывается записью» превращает базу в односторонний расход — ход
 * платит за исследование кода, а следующий платит за него снова.
 */
export function kbToolHint(mode: 'auto' | 'manual'): string {
  const common =
    ...
    `Запрос «обнови прод» выполняй только инструментом mcp__kb__deploy_prod: не запускай деплой через shell.`
