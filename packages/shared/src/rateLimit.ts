// Ограничитель частоты: скользящее окно на ключ (обычно userId или IP). Родился в Make для
// импорта ZIP/URL (самая дорогая операция — распаковка, сеть, запись сотен файлов), теперь общий:
// им же держатся вход по паролю, приглашения и студия картинок.
