...
export function isBrowserWaitOptions(value: unknown): value is BrowserWaitOptions {
  ...
  if (['enabled', 'editable', 'checked', 'idle'].some(key => item[key] !== undefined && typeof item[key] !== 'boolean')) return false
  ...
  return target || item.url !== undefined || item.loadState !== undefined || item.predicate !== undefined || item.idle === true
}
