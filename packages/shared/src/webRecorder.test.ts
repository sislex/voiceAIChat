...
describe('webRecorder client message validator', () => {
  ...
  it('принимает адресованные формы Reader → host', () => {
    ...
    expect(isWebRecorderClientMessage({ type, ...ids, kind: 'control', manual: true })).toBe(true)
    expect(isWebRecorderClientMessage({ type, ...ids, kind: 'control', manual: 'yes' })).toBe(false)
