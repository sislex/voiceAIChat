...
import type { MergeRun, TaskRepository } from './merge'
import type { TaskTimeline } from './timeline'
import type { KbRunUsageReport, KbTaskUsageReport } from './kb'
...
export interface RendererCiRest {
  ...
  getTaskPreparationLlm(
    projectId: string,
    taskId: string
  ): Promise<CiStageLlmSnapshot>
  putTaskCiLlm(
    projectId: string,
    taskId: string,
    config: CiLlmConfig
  ...
  getMergeMachines(
    projectId: string,
    taskId: string
  ): Promise<import('./merge').MergeMachinesResponse>
  ...
  listTaskImprovements(
    projectId: string,
    taskId: string
  ): Promise<TaskImprovement[]>
  listProjectImprovementTasks(
    projectId: string
  ): Promise<Array<{ taskId: string; count: number; improvementId: string }>>
  ...
  createTaskFromImprovement(
    id: string,
    input: CreateTaskFromImprovementInput
  ): Promise<CreateTaskFromImprovementResult>
...
export interface RendererCiBridge extends RendererCiRest {
  ...
  onSnapshot(
    cb: (m: { runId: string; detail: CiRunDetail; log: CiLogLine[] }) => void
  ): () => void
  ...
  onDone(
    cb: (m: { runId: string; run: CiRun; conclusion?: CiRunConclusion }) => void
  ): () => void
  onSummary(
    cb: (m: { projectId: string; summary: CiRunSummary }) => void
  ): () => void
  onInteraction(
    cb: (m: { runId: string; interaction: CiInteraction }) => void
  ): () => void
  /**
   ...
   */
  onChatMessage(
    cb: (m: { conversationId: string; message: Message }) => void
  ): () => void
}
