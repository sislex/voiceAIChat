...
describe('isPreviewAction: как пользователь — field, role, append, element, repeat, state', () => {
  ...
  it('стороны-ориентиры, details, peek, dismiss и parts images проходят валидацию (круг 16)', () => {
    expect(isPreviewAction({ kind: 'find', role: 'button', below: 'Цена' })).toBe(true)
    expect(isPreviewAction({ kind: 'find', text: 'Купить', rightOf: 'Товар', details: true })).toBe(true)
    expect(isPreviewAction({ kind: 'find', text: 'Купить', below: 42 })).toBe(false)
    expect(isPreviewAction({ kind: 'click', role: 'button', leftOf: 'Итого' })).toBe(true)
    expect(isPreviewAction({ kind: 'click', text: 'Подробнее', peek: true })).toBe(true)
    expect(isPreviewAction({ kind: 'click', text: 'Подробнее', peek: 'yes' })).toBe(false)
    expect(isPreviewAction({ kind: 'dismiss' })).toBe(true)
    expect(isPreviewAction({ kind: 'dismiss', what: 'cookies' })).toBe(true)
    expect(isPreviewAction({ kind: 'dismiss', what: 'ads' })).toBe(false)
  ...
  it('read next/toc/table, scroll until, find in и bookmark проходят валидацию (круг 18)', () => {
    expect(isPreviewAction({ kind: 'read', next: true })).toBe(true)
    expect(isPreviewAction({ kind: 'read', toc: true })).toBe(true)
    expect(isPreviewAction({ kind: 'read', table: 'Цены', rowOffset: 20 })).toBe(true)
    expect(isPreviewAction({ kind: 'read', rowOffset: -1 })).toBe(false)
    expect(isPreviewAction({ kind: 'scroll', until: 'Отзывы' })).toBe(true)
    expect(isPreviewAction({ kind: 'scroll', until: 'Отзывы', maxScreens: 60 })).toBe(false)
    expect(isPreviewAction({ kind: 'find', text: 'Условия', in: 'Доставка' })).toBe(true)
    expect(isPreviewAction({ kind: 'bookmark' })).toBe(true)
    expect(isPreviewAction({ kind: 'bookmark', label: 'Сюда вернуться' })).toBe(true)
    expect(isPreviewAction({ kind: 'bookmark', remove: 'https://a.b/' })).toBe(true)
    expect(isPreviewAction({ kind: 'bookmark', label: 'x', remove: 'y' })).toBe(false)
  })
  it('вопрос человеку и передача шага проходят валидацию (круг 19)', () => {
    expect(isPreviewAction({ kind: 'question', question: 'Какой размер?' })).toBe(true)
    expect(isPreviewAction({ kind: 'question', question: 'Какой размер?', options: ['M', 'L'] })).toBe(true)
    expect(isPreviewAction({ kind: 'question', question: '  ' })).toBe(false)
    expect(isPreviewAction({ kind: 'question', question: 'Размер?', options: [] })).toBe(false)
    expect(isPreviewAction({ kind: 'question', question: 'Размер?', timeoutMs: 1_000 })).toBe(false)
