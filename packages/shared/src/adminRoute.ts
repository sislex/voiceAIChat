// Адрес раздела администрирования: разбор и сборка.
//
// Живёт в shared, а не в `@voicechat/admin-app`, потому что маршрут читает
// хост-приложение при каждом рендере — и статический импорт из пакета админки
// сводил её ленивый чанк на нет: модуль, статически достижимый из входа, ложится
// в главный чанк, а `import('@voicechat/admin-app')` возвращает уже загруженное.
// Замер по sourcemap показал так 70 КБ админки и 60 КБ профиля в главном чанке.
//
