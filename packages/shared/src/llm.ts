...
export const MAKE_ASSISTANT_HINT =
  ...
  'Ты собираешь и меняешь этот проект ТОЛЬКО инструментами make_*: сначала make_list_files и make_read_file, затем make_write_file с ПОЛНЫМ содержимым файла (не diff). ' +
  ...
  'Нужны данные «с сервера» — положи JSON в mock/<путь>.json (для POST — mock/<путь>.POST.json; конверт {"$status","$delay","$body"} задаёт статус и задержку): fetch("api/users") в превью и публикации отдаст mock/api/users.json. ' +
  ...
  'они появляются во вкладке «Компоненты» панели. Если пользователь пишет, что работает над одним компонентом, меняй только его файл и его сториз. Отвечай на языке пользователя. '
