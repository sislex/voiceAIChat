...
export interface LlmRequest {
  ...
  /**
   * Где живёт страница этих инструментов: `panel` — открытая панель превью
   * пользователя (по умолчанию), `chromium` — изолированный браузер сервера.
   * Различие не косметическое: в ходе рана панели нет вовсе, и хинт про
   * «панель рядом с чатом» уводил бы модель к несуществующему экрану.
   */
  previewSurface?: 'panel' | 'chromium'
  /**
...
export const KANBAN_ASSISTANT_HINT =
  ...
  'Шаг wait_merge в плане и есть способ не начинать пересекающуюся задачу раньше времени; шаг wait_column ждёт перехода карточки в колонку (semantic ready, development, done…) — так план запускается по событию доски, например «после подготовки (карточка в ready) начать разработку»; run_preparation запускает подготовку, run_ci принимает provider/model/agentId/launch — настройки запуска, о которых просит пользователь. ' +
  'Просьбы вида «когда задача X перейдёт в колонку Y — сделай Z» или «запусти A только после merge B» — это план: собери его из wait_column/wait_merge и run_* и запусти orchestration_start, а не отвечай «это невозможно». ' +
