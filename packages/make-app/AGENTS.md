...
- Гейт: `npm run gate:app -- make-ui`; внутренние правки проверяют только приложение.
- Сборка: `npm run -w @voicechat/make-app build`; watcher: та же команда с `dev`.
...
- Публичный контракт добавляет адресную проверку host. Релиз `make-ui` использует
