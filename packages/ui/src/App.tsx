  ...
  return (
    ...
    >
      ...
      {inProjects && !inTaskChat && routeProjectId && !projectMissing && (
        ...
        >
          {routeReleases ? (
            state.projectDetail?.id === routeProjectId ? <ReleaseCenter projectId={routeProjectId} baseBranch={state.projectDetail.ciBaseBranch ?? 'main'} owner={state.currentUser?.role === 'admin'} machines={state.projectDetail.machines} defaultAgentId={state.projectDetail.defaultAgentId} releaseTimeouts={state.projectDetail.releaseTimeouts} api={api} /> : <div className="proj-page-state" aria-busy="true"><Skeleton variant="list" count={4} item="block" height={64} gap={12} /></div>
          ) : routeSettings ? (
