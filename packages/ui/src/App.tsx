...
function AppBody({ api = window.api, now }: AppProps = {}): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {inSplit && <nav className="chat-split-tabs" aria-label="Режим экрана"><div role="tablist" aria-label={inWorkshop ? 'Панели мастерской' : undefined}><button id={inWorkshop ? 'workshop-chat-tab' : undefined} type="button" role="tab" aria-selected={chatView === 'chat'} aria-controls={inWorkshop ? 'workshop-chat-pane' : undefined} onClick={() => setChatView('chat')}>Чат{splitAttention === 'chat' && <span className="chat-split-tab-dot" role="img" aria-label="Есть новый ответ" />}</button><button id={inWorkshop ? 'workshop-side-tab' : undefined} type="button" role="tab" aria-selected={chatView === 'preview'} aria-controls={inWorkshop ? 'workshop-side-pane' : undefined} onClick={() => setChatView('preview')}><span className="chat-split-tab-label">{inConsoleReader ? 'Консоль' : inMake ? 'Проект' : inImageStudio ? 'Галерея' : siteTabLabel('Сайт', readerPageTitle)}</span>{splitAttention === 'preview' && <span className={`chat-split-tab-dot${splitAttentionFailed ? ' chat-split-tab-dot--failed' : ''}`} role="img" aria-label={splitAttentionFailed ? 'Проверка ассистента не прошла' : 'Ассистент изменил страницу'} />}</button></div>{inReader && readerPendingAction && <button type="button" className="chat-split-live" aria-live="polite" title="Показать сайт" onClick={() => setChatView('preview')}><span className="chat-split-live__dot" aria-hidden="true" />Ассистент {pendingActionLabel(readerPendingAction)}…</button>}{inReader && readerManual && !readerPendingAction && <span className="chat-split-live chat-split-live--manual">✋ Страницей управляете вы — ассистент ждёт</span>}{inReader && readerActionError && !readerPendingAction && !readerManual && <button type="button" className="chat-split-live chat-split-live--error" title="Показать сайт" onClick={() => setChatView('preview')}>Ассистент не смог: {pendingActionLabel(readerActionError.action)}</button>}</nav>}
