...
// В приложении шапка открывается свёрнутой; тестам её содержимого нужен обратный
// дефолт — сворачивание проверяет свой describe в конце файла.
const props = { defaultCollapsed: false } as const

describe('TaskChatHeader', () => {
  it('показывает иерархию, этап, машину, папку и режим рана', () => {
    render(<TaskChatHeader {...props} context={ctx()} onOpenTask={vi.fn()} now={() => 96_000} />)
  ...
  it('у активного рана показывает время работы, у завершённого — итог', () => {
    const { unmount } = render(<TaskChatHeader {...props} context={ctx()} onOpenTask={vi.fn()} now={() => 96_000} />)
  ...
  it('без рана нет ни таймера, ни переключателя ленты', () => {
    render(<TaskChatHeader {...props} context={ctx({ run: null })} onOpenTask={vi.fn()} renderRunFeed={() => <div />} />)
...
describe('TaskChatHeader — подсветка состояния рана', () => {
  ...
  it('живая сводка подсвечивает шапку как карточку на доске', () => {
    const { unmount } = render(
      <TaskChatHeader {...props} context={ctx()} summary={summary({ slotProgress: { done: 2, total: 4, phase: 'Модель исправляет ошибку', fixing: true } })} onOpenTask={vi.fn()} />
    )
    expect(header().className).toContain('taskchat--ci-fixing')
    unmount()

    render(<TaskChatHeader {...props} context={ctx()} summary={summary({ status: 'awaiting_input', awaitingInput: true })} onOpenTask={vi.fn()} />)
  ...
  it('без сводки берёт состояние из контекста, а без рана не подсвечивает', () => {
    ...
    render(<TaskChatHeader {...props} context={ctx({ run: null })} onOpenTask={vi.fn()} />)
    expect(header().className).not.toContain('taskchat--ci-')
  })
})
describe('TaskChatHeader — сворачивание', () => {
  const feed = (id: string): JSX.Element => <div data-testid="feed">лента {id}</div>

  it('шапка открывается свёрнутой: задача и статус рана есть, крошек и меты нет', () => {
    render(<TaskChatHeader context={ctx()} onOpenTask={vi.fn()} renderRunFeed={feed} />)
    expect(screen.getByText('VC-3 Скролл')).toBeInTheDocument()
  ...
  it('разворот возвращает крошки и мету, сворачивание прячет открытую ленту', () => {
    render(<TaskChatHeader context={ctx()} onOpenTask={vi.fn()} renderRunFeed={feed} />)
    ...
    // Лента не закрылась, а спряталась вместе с шапкой — разворот её возвращает.
    fireEvent.click(screen.getByRole('button', { name: 'Развернуть виджет задачи' }))
    expect(screen.getByTestId('feed')).toBeInTheDocument()
  })

  it('развёрнутое состояние не запоминается: следующее монтирование снова свёрнуто', () => {
