...
function DraftRow({ cycle, selected, onToggle, callbacks, canSubmit }: { cycle: TaskReworkCycleViewModel; selected: boolean; onToggle(): void; callbacks: TaskCardCallbacks; canSubmit: boolean }): JSX.Element {
  ...
  return <article className="new-task-rework-row new-task-rework-row--draft" role="listitem">
    ...
    <div className="new-task-rework-content">
      ...
      <footer className="new-task-rework-actions">
        <Button size="sm" variant="primary" disabled={!canSubmit} onClick={() => void callbacks.onSubmitDraft?.(cycle.id)}>Отправить на доработку</Button>
    ...
    <section className="new-task-queue" aria-label="Очередь доработок">
      ...
      {model.drafts.length === 0
        ...
        : <div className="new-task-rework-list" role="list">
          {[...model.drafts].sort((a, b) => draftOrder === 'number' ? a.sequence - b.sequence : b.createdAt - a.createdAt || b.sequence - a.sequence).map((cycle) => <DraftRow key={cycle.id} cycle={cycle} canSubmit={model.actions.canRework && Boolean(callbacks.onSubmitDraft)} selected={selected.includes(cycle.id)} onToggle={() => setSelectedDrafts((all) => all.includes(cycle.id) ? all.filter((id) => id !== cycle.id) : [...all, cycle.id])} callbacks={callbacks} />)}
        </div>}
    </section>
