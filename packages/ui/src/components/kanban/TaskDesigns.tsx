...
/** Страницы, которые имеет смысл предлагать как «экран»: разметка и компоненты. */
const PAGE_RE = /\.(html|jsx|tsx)$/i
...
export function TaskDesigns(props: TaskDesignsProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    void window.api?.['make:state']?.({ conversationId })?.then(
      (state) => { if (alive) setPages(state.files.map((file) => file.path).filter((p) => PAGE_RE.test(p))) },
  ...
  const link = useCallback(async () => {
    ...
    setError(null)
    try {
      setLinks(await window.api['tasks:linkDesign']({ projectId, taskId, conversationId, path, label }))
      setAdding(false)
      setPath('')
      setLabel('')
    } catch (err) {
  ...
  }, [conversationId, label, path, projectId, taskId])
    ...
    <section className="task-designs" aria-label="Дизайн">
      ...
      {links.length > 0 && <ul className="task-designs-list">
        {links.map((item) => (
          <li key={item.id} className="task-design">
            ...
            <span className="task-design__path">{item.path || 'проект целиком'}</span>
            {props.onOpenMake && <Button size="sm" variant="ghost" onClick={() => props.onOpenMake?.(item.conversationId)}>Открыть в Make</Button>}
            {previewable(item.path) && <a className="task-design__preview" href={makeDesignPreviewUrl(item.conversationId, item.path)} target="_blank" rel="noreferrer">Превью</a>}
      ...
      {adding && <div className="task-designs-form">
        <label>
          ...
          <select aria-label="Make-проект" value={conversationId} onChange={(e) => { setConversationId(e.target.value); setPath('') }}>
        ...
        <label>
          <span className="fsub">Страница</span>
          <select aria-label="Страница дизайна" value={path} onChange={(e) => setPath(e.target.value)}>
        ...
        <div className="task-designs-actions">
          <Button size="sm" disabled={busy || !conversationId} onClick={() => void link()}>Связать</Button>
          <Button size="sm" variant="ghost" onClick={() => { setAdding(false); setError(null) }}>Отмена</Button>
        </div>
