...
export function NewTaskQaStagesPanel(props: NewTaskQaStagesPanelProps): JSX.Element {
  ...
  const panelName = props.stage === 'integration_tests' ? 'Интеграционные автотесты' : name
  const panel = <section className="new-task-qa-panel" role="region" aria-label={panelName}>
    <AttemptList ariaLabel={`Попытки ${name}`} selectedId={run?.id ?? null} onSelect={setSelectedId}
      attempts={selected.items.map(item => ({ id: item.id, label: `Попытка ${item.attempt}`, status: statusOf(item), at: item.createdAt }))} />
    ...
    {run && <div className="new-task-section">
  ...
  return <div className="new-task-process" data-testid={`new-task-qa-${props.stage}`}>
    ...
    <StageRail testId={`new-task-qa-rail-${props.stage}`} panel={panel}>
      {stages.map((stage, index) => {
        const shown = stage.items[stage.items.length - 1]
        const status = shown ? statusOf(shown) : 'idle'
        const stageError = shown
          ? ('failureReason' in shown ? shown.failureReason : 'error' in shown ? shown.error : shown.summary)
          : null
        const canRetry = stage.key === stages[stages.length - 1]!.key && shown?.canRetry && !active && !props.runActive
        const retry = canRetry ? () => {
          setSelectedKey(stage.key)
          setSelectedId(null)
          void act(() => props.stage === 'automated_qa' ? window.qa!.retryStageRun!(shown.id) : start())
        } : undefined
        return <StageCard key={stage.key} number={stage.number} status={status} statusLabel={status === 'success' ? 'Завершено' : status === 'idle' ? 'Ожидает' : undefined} eyebrow={`Проход ${stage.number}`} title={stageTitle(name, stage)}
          error={stageError} onRetry={retry}
