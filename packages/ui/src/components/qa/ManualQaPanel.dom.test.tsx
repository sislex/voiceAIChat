...
describe('ManualQaPanel', () => {
  it('renders progress, criterion details, preview and history version', async () => {
    window.qa = { get: vi.fn().mockResolvedValue(qaState()), saveResult:vi.fn(),addAttachment:vi.fn(),complete:vi.fn(),createCriterion:vi.fn(),reviseCriterion:vi.fn(),startSession:vi.fn() }
  ...
  it('saves a draft with optimistic revision', async () => {
    ...
    window.qa={get:vi.fn().mockResolvedValue(qaState()),saveResult,addAttachment:vi.fn(),complete:vi.fn(),createCriterion:vi.fn(),reviseCriterion:vi.fn(),startSession:vi.fn()}
    render(<ManualQaPanel projectId="p1" taskId="t1" />)
  ...
  it('shows stale reason and disables result actions', async () => {
    window.qa={get:vi.fn().mockResolvedValue(qaState('stale')),saveResult:vi.fn(),addAttachment:vi.fn(),complete:vi.fn(),createCriterion:vi.fn(),reviseCriterion:vi.fn(),startSession:vi.fn()}
    render(<ManualQaPanel projectId="p1" taskId="t1" />)
