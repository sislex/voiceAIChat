...
function GenericQaStageRunPanel(props: QaStageRunPanelProps): JSX.Element {
  ...
  const retry = useCallback((id: string) => { if (!busy && !stageActive && window.qa?.retryStageRun) void act(() => window.qa!.retryStageRun!(id)) }, [act, busy, stageActive])
  useEffect(() => {
    props.onRetryActions?.(Object.fromEntries(runs.filter((item) => item.canRetry && !busy && !stageActive && window.qa?.retryStageRun).map((item) => [item.id, () => retry(item.id)])))
  }, [runs, busy, stageActive, retry, props.onRetryActions])
  return <div className="qa-stage-run qa-stage-panel" data-testid={`qa-stage-${props.stage}`}>
    ...
    <PanelHeading
      ...
      actions={<>
        ...
        {run?.canRetry && window.qa?.retryStageRun && <Button size="sm" disabled={busy} onClick={() => retry(run.id)}>{run.scenarios?.length ? 'Повторить те же сценарии' : 'Повторить'}</Button>}
      </>}
    />
