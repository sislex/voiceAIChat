...
const meta={title:'QA/Manual QA',component:ManualQaPanel,args:{projectId:'p1',taskId:'t1'},decorators:[(Story,context)=>{window.qa={get:async()=>context.parameters.qaState as QaTaskState,saveResult:async()=>{throw new Error('Story only')},addAttachment:async()=>{throw new Error('Story only')},complete:async()=>{throw new Error('Story only')},completePreparation:async()=>{throw new Error('Story only')},createCriterion:async()=>{throw new Error('Story only')},reviseCriterion:async()=>{throw new Error('Story only')},startSession:async()=>{throw new Error('Story only')},requestFix:async()=>{throw new Error('Story only')}};return <Story/>}]} satisfies Meta<typeof ManualQaPanel>
export default meta
