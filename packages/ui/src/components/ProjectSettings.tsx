...
function ProjectSettingsForm(props: ProjectSettingsProps & { unsaved: boolean; errors: Record<string, string> }): JSX.Element {
  ...
  const errorDescription = (field: string): string | undefined => props.errors[field] ? `project-error-${field}` : undefined

  return (
    <div className="proj-detail" data-testid="project-settings">
      <h1 className="project-settings-title">Настройки проекта</h1>
      <SettingsPage
        ...
        tabs={tabs}
      />
      <div className="project-settings-scroll" data-testid="project-settings-scroll" role="tabpanel" aria-labelledby={`project-settings-heading-${activeTab}`} tabIndex={0}>
      ...
      {isOwner ? (
        <div className="proj-meta-edit">
          <input className="login-input" aria-label="Название проекта" aria-invalid={!!props.errors.name} aria-describedby={errorDescription('name')} value={name} onChange={(e) => setName(e.target.value)} />{fieldError('name', name)}
          ...
          {features.git && <><input className="login-input" aria-label="Git-репозиторий" aria-invalid={!!props.errors.gitUrl} aria-describedby={errorDescription('gitUrl')} placeholder="git@…" value={gitUrl} onChange={(e) => setGitUrl(e.target.value)} />{fieldError('gitUrl', gitUrl)}</>}
          {features.preview && <><input className="login-input" type="url" aria-label="URL веб-превью" aria-invalid={!!props.errors.previewUrl} aria-describedby={errorDescription('previewUrl')} placeholder="https://example.com" value={previewUrl} onChange={(e) => setPreviewUrl(e.target.value)} />{fieldError('previewUrl', previewUrl)}</>}
        </div>
      ) : (
      ...
      {activeTab === 'workflow' && <section className="proj-section feature-policy" aria-label="Workflow и CI">
        ...
        <><label>Штатная команда production-деплоя<textarea rows={2} className="login-input" disabled={!isOwner} aria-invalid={!!props.errors.productionDeployCommand} aria-describedby={errorDescription('productionDeployCommand')} value={detail.productionDeployCommand ?? ''} onChange={(e) => props.onUpdate(detail.id, { productionDeployCommand: e.target.value })} placeholder="voicechat-deploy" /></label>{fieldError('productionDeployCommand', detail.productionDeployCommand ?? '')}<ProjectCommandCheck project={detail} command={detail.productionDeployCommand ?? ''} label="Штатная команда production-деплоя" /></>
        <><label>Команда health-check<textarea rows={2} className="login-input" disabled={!isOwner} aria-invalid={!!props.errors.productionHealthCheckCommand} aria-describedby={errorDescription('productionHealthCheckCommand')} value={detail.productionHealthCheckCommand ?? ''} onChange={(e) => props.onUpdate(detail.id, { productionHealthCheckCommand: e.target.value })} placeholder="curl -fsS http://127.0.0.1:8787/api/health" /></label>{fieldError('productionHealthCheckCommand', detail.productionHealthCheckCommand ?? '')}<ProjectCommandCheck project={detail} command={detail.productionHealthCheckCommand ?? ''} label="Команда health-check" /></>
        <><label>CI: базовая ветка<input className="login-input" disabled={!isOwner} aria-invalid={!!props.errors.ciBaseBranch} aria-describedby={errorDescription('ciBaseBranch')} value={detail.ciBaseBranch ?? ''} onChange={(e) => props.onUpdate(detail.id, { ciBaseBranch: e.target.value })} placeholder="main" /></label>{fieldError('ciBaseBranch', detail.ciBaseBranch ?? '')}</>
        <><label>CI: шаблон ветки<input className="login-input" disabled={!isOwner} aria-invalid={!!props.errors.ciBranchTemplate} aria-describedby={errorDescription('ciBranchTemplate')} value={detail.ciBranchTemplate ?? ''} onChange={(e) => props.onUpdate(detail.id, { ciBranchTemplate: e.target.value })} placeholder="{task_number}" /></label>{fieldError('ciBranchTemplate', detail.ciBranchTemplate ?? '')}<p className="proj-hint">Подстановки: {'{task_number}'} → CHAT-123, {'{slug}'} → task-title.</p></>
