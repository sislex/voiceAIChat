...
describe('MakeNotesDialog (roadmap-4 пп.6–7)', () => {
  it('сохраняет заметки и режим ассистента', async () => {
    ...
    await waitFor(() => expect(onSaved).toHaveBeenCalledWith({ notes: '- акцент синий', mode: 'designer' }))
    expect(await api['make:notes']({ conversationId: 'make-1' })).toEqual({ notes: '- акцент синий', mode: 'designer' })
  })
})
