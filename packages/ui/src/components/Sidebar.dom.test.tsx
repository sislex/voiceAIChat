...
describe('Sidebar — фильтр проектов чатов', () => {
  ...
  it('показывает полный выбор, массовую кнопку и независимые доступные чекбоксы', async () => {
    ...
    const menu = screen.getByRole('menu', { name: 'Проекты для фильтра' })
    expect(menu.closest('.side-controls')).toHaveClass('side-controls--open')
    expect(document.querySelector('.convolist')).not.toContainElement(menu)
