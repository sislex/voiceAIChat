...
export function createCiRest(httpBase: string): RendererCiRest {
  ...
  return {
    ...
    putTaskCi: (projectId, taskId, config) => req<CiSlotConfig & { enabledStages: import('@shared/ci').CiProcessStage[]; browserCheck: import('@shared/ci').CiBrowserCheck; developmentPreview: import('@shared/ci').DevelopmentPreviewSettings }>(REST.taskCi(projectId, taskId), { method: 'PUT', body: JSON.stringify(config) }),
