# @voicechat/web-reader-app — Web Reader UI
...
- Гейт: `npm run gate:app -- web-reader-ui`; внутренние правки проверяют только приложение.
- Сборка: `npm run -w @voicechat/web-reader-app build`; watcher: та же команда с `dev`.
...
- Публичный контракт добавляет адресную проверку host. Релиз `web-reader-ui` использует
