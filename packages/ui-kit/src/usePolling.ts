...
export interface PollingOptions {
  ...
  enabled: boolean
  intervalMs: number
  /** Set false when a host visibility subscription already refreshes the data. */
  refreshOnVisible?: boolean
}

export function usePolling(poll: () => void, { enabled, intervalMs, refreshOnVisible = true }: PollingOptions): void {
  ...
  useEffect(() => {
    ...
    const sync = (): void => {
      ...
      if (refreshOnVisible) pollRef.current()
      start()
    }
