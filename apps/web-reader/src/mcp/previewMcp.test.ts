...
describe('previewMcp — инструменты browser', () => {
  ...
  it('tools/list показывает все инструменты браузера', async () => {
    ...
    expect(body.result.tools.map((t) => t.name).sort()).toEqual(['a11y', 'accessibility', 'ask', 'audit', 'back', 'cancel-download', 'clear', 'click', 'close-tab', 'console', 'cookies', 'copy', 'count', 'csv', 'delete-download', 'device', 'dialogs', 'downloads', 'drag', 'drop-file', 'edits', 'emulate', 'environment', 'errors', 'evaluate', 'expect', 'fill-form', 'find', 'focus', 'focus-order', 'focused', 'form-state', 'forward', 'frames', 'handle-dialog', 'highlight', 'history', 'hotkey', 'hover', 'list', 'measure', 'media', 'metrics', 'network', 'network-rules', 'new-tab', 'note', 'open', 'options', 'paste', 'press', 'probe', 'read', 'read-download', 'record', 'record-check', 'reload', 'replay', 'reset-session', 'screenshot', 'scroll', 'scroll-until', 'select-tab', 'select-text', 'set', 'snapshot', 'source', 'stop-loading', 'storage', 'styles', 'submit', 'table', 'tabs', 'test-users', 'touch', 'type', 'upload', 'validity', 'viewport', 'wait'])
  })
