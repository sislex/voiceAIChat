...
export function previewAuditHelpers(): string {
  return String.raw`
...
const runAudit=action=>{
  ...
  const limitations=['Checks observe the rewritten proxy document, not the original browser origin.','Shadow root contents and child frame documents are not scanned.','Heuristic findings require visual review; an empty report is not a complete QA pass.'];
  ...
  const audit={version:1,group,groups,mode,surface:'proxy',scope:action.selector||'document',findings:[],rules:[],total:0,checkedRules:0,scannedElements:0,truncated:false,elapsedMs:0,limitations};
  ...
  const matchesCache=new Map(),visibleCache=new WeakMap();
  const context={all:selector=>{if(!matchesCache.has(selector))matchesCache.set(selector,nodes.filter(el=>el.matches(selector)));return matchesCache.get(selector)},visible:el=>{if(!visibleCache.has(el))visibleCache.set(el,actionVisible(el));return visibleCache.get(el)}};
  const findings=[];
