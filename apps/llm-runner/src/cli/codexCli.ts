...
export function codexInvocation(req: LlmRequest): { args: string[]; prompt: string } {
  ...
  if (req.remote && req.permissionMode !== 'plan') {
    ...
    prompt =
      ...
      // Файловые инструменты того же сервера: без этой строки модель знает только
      // про bash и читает файлы `cat` целиком, а правит heredoc'ом — весь файл
      // уходит в контекст, и ход стоит в разы дороже.
      `\nФайлы читай инструментом read того же сервера (окно строк), ищи grep, ` +
      `правь edit (точная замена). Через bash файлы не читай и не правь: команду, вся суть ` +
      `которой — прочитать файл рабочей директории (cat, sed -n, head, tail), мост отклонит ` +
      `и вернёт готовый вызов read. Пайплайны, grep -r и подстановки работают как обычно.` +
      (req.readOnlyRemote
        ? `\nРежим «План»: только исследование — читай файлы инструментом read, ищи grep, ` +
          `историю смотри bash (ls, git log/diff/status). Ничего не меняй: мост отклонит ` +
          `изменяющие команды.`
        : '') +
