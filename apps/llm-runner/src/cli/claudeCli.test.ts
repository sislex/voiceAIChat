...
describe('ClaudeCli', () => {
  ...
  it('previewMcpUrl + remote: инструменты browser попадают в allow-list', () => {
    ...
    const allowed = args[args.indexOf('--allowedTools') + 1]
    for (const tool of ['open', 'read', 'find', 'click', 'type', 'frames', 'styles', 'reset-session', 'downloads', 'read-download', 'cancel-download', 'delete-download', 'dialogs', 'handle-dialog', 'tabs', 'new-tab', 'select-tab', 'close-tab', 'reload', 'stop-loading']) expect(allowed).toContain(`mcp__browser__${tool}`)
  })
