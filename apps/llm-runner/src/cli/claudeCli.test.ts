...
describe('ClaudeCli', () => {
  ...
  it('previewMcpUrl + remote: инструменты browser попадают в allow-list', () => {
    ...
    const allowed = args[args.indexOf('--allowedTools') + 1]
    for (const tool of ['open', 'read', 'find', 'click', 'type', 'frames', 'styles', 'reset-session', 'tabs', 'new-tab', 'select-tab', 'close-tab', 'reload', 'stop-loading']) expect(allowed).toContain(`mcp__browser__${tool}`)
  })
  ...
  it('previewSurface chromium: хинт про изолированный браузер и расширенный allow-list', () => {
    ...
    for (const tool of ['screenshot', 'errors', 'wait', 'console', 'network', 'frames', 'styles', 'reset-session', 'tabs', 'new-tab', 'select-tab', 'close-tab', 'reload', 'stop-loading']) expect(allowed).toContain(`mcp__browser__${tool}`)
  })
