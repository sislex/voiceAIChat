...
describe('поллинг метаданных не присваивает действие модели пользователю', () => {
  it('метаданные текущего проекта показывают логический URL, включая новый fragment', async () => {
    ...
    const session = { queue: new BrowserCommandQueue(), id: 'c', conversationKey: 'c', incarnation: 'inc', activeTabId: 'tab', pages: new Map([['tab',page]]), viewport: { width: 800, height: 600, deviceScaleFactor: 1 }, lastUsedAt: 0 }
  ...
  it('status и screenshot сохраняют lastActor и возвращают логический адрес', async () => {
    ...
    const session = { queue: new BrowserCommandQueue(), id: 'c', conversationKey: 'c', incarnation: 'inc', activeTabId: 'tab', pages: new Map([['tab',page]]), viewport: { width: 800, height: 600, deviceScaleFactor: 1 }, lastActor: 'assistant', lastUsedAt: 0 }
    ;(manager as unknown as { sessions: Map<string,Promise<unknown>> }).sessions.set('c',Promise.resolve(session))
...
it('повторный start ждёт закрытия предыдущего контекста', async () => {
  ...
  const old = { id: 'c', userKey: 'u', conversationKey: 'c', incarnation: 'old', queue: new BrowserCommandQueue(), context: { close: () => closing }, profileDir: '/tmp/unused-reader-restart/old', pages: new Map([['tab', page]]), activeTabId: 'tab', viewport: { width: 800, height: 600, deviceScaleFactor: 1 } }
