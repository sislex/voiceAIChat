import { readNativeAccessibility } from './accessibility.js'
...
export class BrowserSessionManager {
  ...
  private async executeCommand(sessionId: string, request: BrowserCommandRequest): Promise<BrowserSessionMetadata | BrowserCapture | BrowserSelectorResult | BrowserInspectResult | BrowserFramesResult | BrowserSiteDataResetResult | BrowserEnvironmentResult | BrowserCookiesResult | BrowserDeviceResult | BrowserAskResult | BrowserNetworkRulesResult | BrowserSnapshotResult | BrowserReportResult> {
    ...
    if (command.type === 'navigate') await session.downloads.navigate(page, () => page.goto(applyHostAlias(validatePublicUrl(command.url, this.allowedTargets), this.hostAliases).toString(), NAVIGATION_OPTIONS))
      ...
      const result = command.action.kind === 'accessibility' ? await readNativeAccessibility(page, command.action) : command.action.kind === 'evaluate' ? await runEvaluation(page, page.mainFrame(), command.action, () => Boolean(session.dialogs.forPage(page))) : await runInspectAction({ console: session.console, network: session.network }, page, command.action)
