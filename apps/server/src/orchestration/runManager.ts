...
export interface OrchestrationManagerDeps {
  ...
  report?: (plan: Orchestration, text: string) => Promise<void>
  tickMs?: number
}
...
export interface OrchestrationManager {
  ...
  cancel(owner: string, planId: string): Promise<Orchestration | null>
...
export function createOrchestrationManager(deps: OrchestrationManagerDeps): OrchestrationManager {
  ...
  const inColumn = async (plan: Orchestration, taskId: string, payload: WaitColumnPayload): Promise<boolean> => {
    const board = await deps.db.tasks.getBoard(plan.owner, plan.projectId, { includeCompleted: true })
  ...
  const merged = async (plan: Orchestration, taskId: string): Promise<boolean> =>
    (await deps.db.ci.listMergeRuns(plan.owner, plan.projectId, taskId, 10))
      .some((run) => run.status === 'success')

  const startItem = async (plan: Orchestration, item: OrchestrationItem): Promise<void> => {
    const db = deps.db
    const fail = async (message: string): Promise<void> => await db.tasks.updateOrchestrationItem(item.id, { status: 'failed', error: message })
    ...
    if (item.kind === 'create_task') {
      ...
      const board = await db.tasks.getBoard(plan.owner, plan.projectId)
      const columnId = payload.columnId ?? board?.columns[0]?.id
      if (!columnId) { await fail('На доске нет колонок'); return }
      try {
        const created = await db.tasks.createTask(plan.owner, plan.projectId, {
          columnId,
        ...
        if (!created) { await fail('Создать карточку не удалось'); return }
        await db.tasks.updateOrchestrationItem(item.id, { status: 'done', taskId: created.id })
        deps.boardChanged?.(plan.projectId)
      } catch (error) { await fail(error instanceof Error ? error.message : String(error)) }
      return
    }

    const taskId = taskOf(item, plan)
    if (!taskId) { await fail('У шага нет задачи'); return }

    if (item.kind === 'wait_merge') {
      // Ожидание — не работа: шаг остаётся pending и проверяется каждый тик.
      if (await merged(plan, taskId)) await db.tasks.updateOrchestrationItem(item.id, { status: 'done', taskId })
      return
    }
    if (item.kind === 'wait_column') {
      ...
      if (await inColumn(plan, taskId, item.payload as WaitColumnPayload)) await db.tasks.updateOrchestrationItem(item.id, { status: 'done', taskId })
      return
    }

    const runner = await deps.runs()
    if (!runner) { await fail('Менеджеры ранов недоступны'); return }
    try {
      if (item.kind === 'run_preparation') {
        if (!runner.startPreparation) { await fail('Подготовка задач недоступна'); return }
        const run = await runner.startPreparation(plan.owner, plan.projectId, taskId)
        await db.tasks.updateOrchestrationItem(item.id, { status: 'running', taskId, runId: run.id })
        return
      }
      if (item.kind === 'run_ci') {
        const payload = item.payload as RunCiPayload
        const started = await runner.startCi(plan.owner, plan.projectId, taskId, {
        ...
        if ('error' in started) { await fail(started.error); return }
        await db.tasks.updateOrchestrationItem(item.id, { status: 'running', taskId, runId: started.run.id })
        return
      }
      if (item.kind === 'run_qa') {
      ...
      if (item.kind === 'run_preview') {
        ...
        if (!runner.previewOperate) { await fail('Тестовые окружения недоступны'); return }
        ...
        await db.tasks.updateOrchestrationItem(item.id, { status: 'done', taskId })
        return
      }
   ...
   */
  const failOrRetry = async (item: OrchestrationItem, error: string): Promise<void> => {
    const limit = orchestrationItemMaxAttempts(item)
    if (item.attempts < limit) {
      await deps.db.tasks.updateOrchestrationItem(item.id, { status: 'pending', runId: null, attempts: item.attempts + 1, error: `Попытка ${item.attempts + 1} из ${limit + 1}: ${error}` })
      return
    }
    await deps.db.tasks.updateOrchestrationItem(item.id, { status: 'failed', error })
  }

  /** Завершился ли запущенный шагом ран; ищем по его id среди ранов задачи. */
  const settleRunning = async (plan: Orchestration, item: OrchestrationItem): Promise<void> => {
    const db = deps.db
    if (!item.runId || !item.taskId) return
    if (item.kind === 'run_ci') {
      const run = (await db.ci.listCiRunsForTask(plan.owner, plan.projectId, item.taskId)).find((candidate) => candidate.id === item.runId)
      if (!run) return
      if (run.status === 'success') await db.tasks.updateOrchestrationItem(item.id, { status: 'done' })
      else if (run.status === 'failed' || run.status === 'cancelled' || run.status === 'timeout') {
        await failOrRetry(item, run.error ?? `Ран завершился со статусом ${run.status}`)
      }
      return
    }
    if (item.kind === 'run_preparation') {
      const run = (await db.tasks.listTaskPreparationRuns(plan.owner, plan.projectId, item.taskId)).find((candidate) => candidate.id === item.runId)
      if (!run) return
      if (run.status === 'completed' || run.status === 'success') await db.tasks.updateOrchestrationItem(item.id, { status: 'done' })
      else if (run.status === 'failed' || run.status === 'cancelled' || run.status === 'blocked') {
        await failOrRetry(item, run.error ?? `Подготовка завершилась со статусом ${run.status}`)
      }
      return
    }
    if (item.kind === 'run_merge') {
      const run = (await db.ci.listMergeRuns(plan.owner, plan.projectId, item.taskId, 10)).find((candidate) => candidate.id === item.runId)
      if (!run) return
      if (run.status === 'success') await db.tasks.updateOrchestrationItem(item.id, { status: 'done' })
      else if (run.status === 'failed' || run.status === 'cancelled' || run.status === 'timeout') {
        await failOrRetry(item, run.error ?? `Merge завершился со статусом ${run.status}`)
      }
      return
    }
    ...
    const run = (await db.qa.listQaStageRuns(plan.owner, plan.projectId, item.taskId, stage)).find((candidate) => candidate.id === item.runId)
    if (!run) return
    if (run.status === 'success') await db.tasks.updateOrchestrationItem(item.id, { status: 'done' })
    else if (run.status === 'failed' || run.status === 'cancelled' || run.status === 'gate_failed' || run.status === 'interrupted') {
      await failOrRetry(item, `Этап ${stage} завершился со статусом ${run.status}`)
    }
  }
  ...
  const tick = async (planId: string): Promise<void> => {
    ...
    ticking.add(planId)
    try {
      let plan = await deps.db.tasks.getOrchestrationById(planId)
      if (!plan || plan.status !== 'running') { stop(planId); return }
      for (const item of plan.items.filter((candidate) => candidate.status === 'running')) await settleRunning(plan, item)
      ...
      for (let pass = 0; pass <= plan.items.length; pass += 1) {
        const current = (await deps.db.tasks.getOrchestrationById(planId))!
        ...
        const after = (await deps.db.tasks.getOrchestrationById(planId))!.items.map((item) => `${item.id}:${item.status}`).join(',')
        if (before === after) break
      }
      const updated = (await deps.db.tasks.getOrchestrationById(planId))!
      const status = orchestrationStatusOf(updated.items)
      if (status !== 'running') {
        const failed = updated.items.find((item) => item.status === 'failed')
        await deps.db.tasks.updateOrchestrationStatus(planId, status, failed?.error ?? null)
        stop(planId)
        ...
        const final = (await deps.db.tasks.getOrchestrationById(planId))!
      ...
      deps.publish?.((await deps.db.tasks.getOrchestrationById(planId))!)
    } finally {
  ...
  return {
    ...
    cancel: async (owner, planId) => {
      const cancelled = await deps.db.tasks.cancelOrchestration(owner, planId)
      stop(planId)
      if (cancelled) deps.publish?.(cancelled)
      return cancelled
    },
    restore: async () => {
      for (const plan of await deps.db.tasks.listActiveOrchestrations()) void track(plan.id)
    },
    dispose: () => {
