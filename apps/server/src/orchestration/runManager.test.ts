...
beforeEach(() => {
  ...
  projectId = db.projects.createProject('ann', { name: 'Chat' })!.id
  ...
  vi.spyOn(db.ci, 'listCiRunsForTask').mockImplementation((_userId, _projectId, taskId) =>
...
function plan(items: Parameters<VoiceChatDb['tasks']['createOrchestration']>[4]): Orchestration {
  return db.tasks.createOrchestration('ann', projectId, null, 'План', items)!
}
...
describe('createOrchestrationManager', () => {
  it('создаёт задачу и запускает разработку по зависимости', async () => {
    ...
    const afterFirst = db.tasks.getOrchestrationById(created.id)!
    ...
    await manager.tick(created.id)
    const done = db.tasks.getOrchestrationById(created.id)!
    expect(done.items[1]!.status).toBe('done')
    expect(done.status).toBe('done')
  })

  it('упавший ран останавливает план и сохраняет причину', async () => {
    const taskId = db.tasks.createTask('ann', projectId, { columnId, title: 'A' })!.id
    const created = plan([
    ...
    await manager.tick(created.id)
    const failed = db.tasks.getOrchestrationById(created.id)!
  ...
  it('wait_merge держит следующую задачу, пока ветка не влита', async () => {
    const first = db.tasks.createTask('ann', projectId, { columnId, title: 'Первая' })!.id
    const second = db.tasks.createTask('ann', projectId, { columnId, title: 'Вторая' })!.id
    const merges: Array<{ id: string; status: string }> = []
    vi.spyOn(db.ci, 'listMergeRuns').mockImplementation(() => merges as never)
    const created = plan([
      { kind: 'wait_merge', title: 'Дождаться merge первой', taskId: first },
      { kind: 'run_ci', title: 'Разработка второй', taskId: second, dependsOn: [0] }
    ])
    await manager.tick(created.id)
    expect(db.tasks.getOrchestrationById(created.id)!.items[0]!.status).toBe('pending')
    expect(ciRuns).toHaveLength(0)

    merges.push({ id: 'merge-1', status: 'success' })
    await manager.tick(created.id)
    const afterMerge = db.tasks.getOrchestrationById(created.id)!
  ...
  it('wait_column ждёт перехода карточки в колонку, затем run_ci стартует с заданными моделью и машиной', async () => {
    ...
    const task = db.tasks.createTask('ann', projectId, { columnId, title: 'После подготовки' })!.id
    ...
    db.tasks.moveTask('ann', projectId, task, { columnId: ready.id })
    await manager.tick(created.id)
    const after = db.tasks.getOrchestrationById(created.id)!
  ...
  it('wait_column по columnId и run_preparation: подготовка стартует, завершение подхватывается по статусу рана', async () => {
    const task = db.tasks.createTask('ann', projectId, { columnId, title: 'Подготовить' })!.id
    ...
    vi.spyOn(db.tasks, 'listTaskPreparationRuns').mockImplementation(() => prepRuns as never)
    const created = plan([
    ...
    await manager.tick(created.id)
    let current = db.tasks.getOrchestrationById(created.id)!
    ...
    await manager.tick(created.id)
    current = db.tasks.getOrchestrationById(created.id)!
    ...
    await manager.tick(failing.id)
    expect(db.tasks.getOrchestrationById(failing.id)).toMatchObject({ status: 'failed', error: 'Нет машины' })
  })
  ...
  it('отмена останавливает незавершённые шаги и публикует состояние', async () => {
    ...
    await manager.tick(created.id)
    expect(db.tasks.getOrchestrationById(created.id)!.status).toBe('cancelled')
  })
  ...
  it('restore подхватывает незавершённые планы после рестарта', async () => {
    ...
    expect(db.tasks.getOrchestrationById(created.id)!.items[0]!.status).toBe('running')
    revived.dispose()
  })
  ...
  it('шаг с retries перезапускается, а исчерпав попытки — валит план', async () => {
    ...
    const retried = db.tasks.getOrchestrationById(created.id)!
  ...
  it('успешный план отчитывается в чат', async () => {
    ...
    await manager.tick(created.id)
    expect(db.tasks.getOrchestrationById(created.id)!.status).toBe('done')
