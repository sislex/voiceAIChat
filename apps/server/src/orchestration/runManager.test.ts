...
beforeEach(async () => {
  ...
  await db.identity.createUser('ann', '', 'developer')
  projectId = (await db.projects.createProject('ann', { name: 'Chat' }))!.id
  columnId = (await db.tasks.getBoard('ann', projectId))!.columns[0]!.id
  published = []
  ciRuns = []
  reports = []
  previewOps = []
  launchers = {
    ...
    startCi: async (_userId, _projectId, taskId) => {
  ...
  manager = createOrchestrationManager({
    db,
    runs: async () => launchers,
    publish: (plan) => published.push(plan),
    report: async (_plan, text) => { reports.push(text) },
  ...
  vi.spyOn(db.sync.ci, 'listCiRunsForTask').mockImplementation((_userId, _projectId, taskId) =>
...
async function plan(items: Parameters<VoiceChatDb['tasks']['createOrchestration']>[4]): Promise<Orchestration> {
  return (await db.tasks.createOrchestration('ann', projectId, null, 'План', items))!
}
...
describe('createOrchestrationManager', () => {
  it('создаёт задачу и запускает разработку по зависимости', async () => {
    ...
    const afterFirst = (await db.tasks.getOrchestrationById(created.id))!
    ...
    await manager.tick(created.id)
    const done = (await db.tasks.getOrchestrationById(created.id))!
  ...
  it('упавший ран останавливает план и сохраняет причину', async () => {
    ...
    await manager.tick(created.id)
    const failed = (await db.tasks.getOrchestrationById(created.id))!
  ...
  it('wait_merge держит следующую задачу, пока ветка не влита', async () => {
    const first = (await db.tasks.createTask('ann', projectId, { columnId, title: 'Первая' }))!.id
    const second = (await db.tasks.createTask('ann', projectId, { columnId, title: 'Вторая' }))!.id
    const merges: Array<{ id: string; status: string }> = []
    vi.spyOn(db.sync.ci, 'listMergeRuns').mockImplementation(() => merges as never)
    const created = await plan([
      { kind: 'wait_merge', title: 'Дождаться merge первой', taskId: first },
      { kind: 'run_ci', title: 'Разработка второй', taskId: second, dependsOn: [0] }
    ])
    await manager.tick(created.id)
    expect((await db.tasks.getOrchestrationById(created.id))!.items[0]!.status).toBe('pending')
    expect(ciRuns).toHaveLength(0)

    merges.push({ id: 'merge-1', status: 'success' })
    await manager.tick(created.id)
    const afterMerge = (await db.tasks.getOrchestrationById(created.id))!
  ...
  it('wait_column ждёт перехода карточки в колонку, затем run_ci стартует с заданными моделью и машиной', async () => {
    const board = (await db.tasks.getBoard('ann', projectId))!
    const ready = board.columns.find((column) => column.semanticType === 'ready')!
    const task = (await db.tasks.createTask('ann', projectId, { columnId, title: 'После подготовки' }))!.id
    const startOptions: unknown[] = []
    launchers.startCi = async (_userId, _projectId, taskId, options) => {
    ...
    await db.tasks.moveTask('ann', projectId, task, { columnId: ready.id })
    await manager.tick(created.id)
    const after = (await db.tasks.getOrchestrationById(created.id))!
  ...
  it('wait_column по columnId и run_preparation: подготовка стартует, завершение подхватывается по статусу рана', async () => {
    const task = (await db.tasks.createTask('ann', projectId, { columnId, title: 'Подготовить' }))!.id
    const prepRuns: Array<{ id: string; status: string; error: string | null }> = []
    launchers.startPreparation = async () => { const run = { id: 'prep-1', status: 'running', error: null }; prepRuns.push(run); return run }
    vi.spyOn(db.sync.tasks, 'listTaskPreparationRuns').mockImplementation(() => prepRuns as never)
    const created = await plan([
    ...
    await manager.tick(created.id)
    let current = (await db.tasks.getOrchestrationById(created.id))!
    ...
    await manager.tick(created.id)
    current = (await db.tasks.getOrchestrationById(created.id))!
    ...
    const failing = await plan([{ kind: 'run_preparation', title: 'Подготовка 2', taskId: task }])
    prepRuns.length = 0
    await manager.tick(failing.id)
    prepRuns[0]!.status = 'failed'; prepRuns[0]!.error = 'Нет машины'
    await manager.tick(failing.id)
    expect(await db.tasks.getOrchestrationById(failing.id)).toMatchObject({ status: 'failed', error: 'Нет машины' })
  })
  ...
  it('отмена останавливает незавершённые шаги и публикует состояние', async () => {
    ...
    await manager.tick(created.id)
    const cancelled = (await manager.cancel('ann', created.id))!
  ...
  it('restore подхватывает незавершённые планы после рестарта', async () => {
    ...
    const revived = createOrchestrationManager({ db, runs: async () => launchers, tickMs: 60_000 })
    await revived.restore()
    await new Promise((resolve) => setTimeout(resolve, 20))
    expect((await db.tasks.getOrchestrationById(created.id))!.items[0]!.status).toBe('running')
    revived.dispose()
  })

  it('шаг с retries перезапускается, а исчерпав попытки — валит план', async () => {
    const taskId = (await db.tasks.createTask('ann', projectId, { columnId, title: 'A' }))!.id
    const created = await plan([{ kind: 'run_ci', title: 'Разработка', taskId, payload: { retries: 1 } }])
    await manager.tick(created.id)
    ciRuns[0]!.status = 'failed'
    await manager.tick(created.id)

    // Первая неудача — не приговор: шаг ушёл на второй заход.
    const retried = (await db.tasks.getOrchestrationById(created.id))!
  ...
  it('успешный план отчитывается в чат', async () => {
    const created = await plan([{ kind: 'create_task', title: 'Корзина', payload: { columnId, title: 'Корзина' } }])
    await manager.tick(created.id)
  ...
  it('шаг run_preview поднимает тестовое окружение и сразу завершается', async () => {
    const taskId = (await db.tasks.createTask('ann', projectId, { columnId, title: 'A' }))!.id
    const created = await plan([{ kind: 'run_preview', title: 'Окружение', taskId, payload: { operation: 'start' } }])
    await manager.tick(created.id)
