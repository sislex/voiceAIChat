...
export async function materializeProjectMachine(
  ...
  const storage = (await db.machines.listMachineStorages(userId, agentId)).find((item) => item.id === storageId)
  ...
  const current = (await db.projects.getProject(userId, projectId))?.machines.find((item) => item.agentId === agentId)
