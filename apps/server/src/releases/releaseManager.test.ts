...
describe('ReleaseManager separated preparation and deploy',()=>{
  ...
  it('switches production through an attempt-specific ref instead of FETCH_HEAD',()=>{
    ...
    expect(command).toContain("git config 'branch.release/1.2.3.remote' origin")
    expect(command).toContain("git config 'branch.release/1.2.3.merge' 'refs/heads/release/1.2.3'")
  ...
  it('blocks mismatched prepared version before changing production',async()=>{
    ...
    await db.releases.createProjectRelease('owner',projectId,{branch:'release/0.1.35',version:'0.1.0',sha:'fixed-sha',status:'ready'})
    ...
    const list=await db.releases.listProjectReleaseSummaries('owner',projectId)
