...
export interface GeneratedCleanupDeps {
  targets(): Promise<GeneratedCleanupTarget[]>
  ttlDays(userId: string): Promise<number>
  messages(userId: string, conversationId: string): Promise<Message[]>
  ...
  defer(target: GeneratedCleanupTarget, error: string, nextAttemptAt: number): Promise<void>
  complete(target: GeneratedCleanupTarget): Promise<void>
...
export class GeneratedCleanupService {
  ...
  async run(): Promise<GeneratedCleanupCounters> {
    ...
    for (const target of await this.deps.targets()) {
      try {
        let targetDeferred = false
        const storage = await this.deps.resolve(target.userId, target.conversationId)
        if (!storage) {
          await this.deps.defer(target, 'managed_binding_missing', now + 3_600_000)
          result.deferred++
          continue
        }
        const cutoff = now - await this.deps.ttlDays(target.userId) * 86_400_000
        const listing = await this.deps.list(storage.binding.machineId, storage.generated)
        const live = references(await this.deps.messages(target.userId, target.conversationId), storage.binding.machineId)
        for (const entry of listing.entries ?? []) {
          ...
          const fresh = references(await this.deps.messages(target.userId, target.conversationId), storage.binding.machineId)
          ...
          try {
            ...
            await this.deps.defer(target, error instanceof Error ? error.message : String(error), now + 3_600_000)
            targetDeferred = true
            result.deferred++
          }
        }
        if (!targetDeferred) await this.deps.complete(target)
      } catch (error) {
        await this.deps.defer(target, error instanceof Error ? error.message : String(error), now + 3_600_000)
        result.deferred++
      }
