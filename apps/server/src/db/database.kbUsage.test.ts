...
describe('VoiceChatDb — обращения к базе знаний', () => {
  ...
  it('seq монотонен внутри разговора и не мешает соседнему чату', () => {
    ...
    const first = db.kb.addKbUsage({ userId: U, conversationId: a.id, source: 'auto', query: 'q1', chars: 10 })
    const second = db.kb.addKbUsage({ userId: U, conversationId: a.id, source: 'tool_search', query: 'q2', chars: 20 })
    const other = db.kb.addKbUsage({ userId: U, conversationId: b.id, source: 'auto', query: 'q3', chars: 30 })
    expect([first.seq, second.seq]).toEqual([1, 2])
    expect(other.seq).toBe(1)
    expect(db.kb.kbUsageReport(U, a.id)!.lastSeq).toBe(2)
  })

  it('totals.chars не дублируется при нескольких разделах одного обращения', () => {
    const conv = db.chat.createConversation(U, 'Чат')
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', query: 'ходы', chars: 500, injected: true, sections: twoSections() })
  ...
  it('агрегат группируется по documentId + anchor и различает источник', () => {
    const conv = db.chat.createConversation(U, 'Чат')
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', query: 'ходы', chars: 300, sections: [twoSections()[0]] })
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'tool_document', query: 'ходы', chars: 300, sections: [twoSections()[0]] })
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'tool_search', query: 'llm', chars: 200, sections: [twoSections()[1]] })
    const sections = db.kb.kbUsageReport(U, conv.id)!.sections
    ...
    expect(db.kb.kbUsageReport(U, conv.id)!.totals.toolQueries).toBe(2)
  })

  it('attachKbUsageTurn дописывает итоги хода во все обращения этого хода', () => {
    const conv = db.chat.createConversation(U, 'Чат')
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', turnId: 't1', query: 'a', chars: 100 })
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'tool_search', turnId: 't1', query: 'b', chars: 50 })
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', turnId: 't2', query: 'c', chars: 40 })
    expect(db.kb.attachKbUsageTurn({ turnId: 't1', messageId: 'm1', promptChars: 4000, turnInputTokens: 1200 })).toBe(2)
    const report = db.kb.kbUsageReport(U, conv.id)!
  ...
  it('удаление разговора уносит обращения и их разделы (каскад)', () => {
    const conv = db.chat.createConversation(U, 'Чат')
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', query: 'q', chars: 100, sections: twoSections() })
    db.chat.deleteConversation(U, conv.id)
    expect(db.kb.kbUsageReport(U, conv.id)).toBeNull()
  })

  it('чужой чат не отдаёт отчёт (изоляция по владельцу)', () => {
    db.identity.createUser('bob', 'x', 'developer')
    const conv = db.chat.createConversation(U, 'Чат')
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', query: 'q', chars: 10 })
    expect(db.kb.kbUsageReport('bob', conv.id)).toBeNull()
    expect(db.kb.markKbUsageViewed('bob', conv.id, 1)).toBeNull()
  })

  it('граница просмотра идемпотентна, движется вперёд и не удаляет историю', () => {
    const conv = db.chat.createConversation(U, 'Чат')
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', query: 'q1', chars: 10 })
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'tool_search', query: 'q2', chars: 20 })
    expect(db.kb.kbUsageReport(U, conv.id)!.unreadCount).toBe(2)

    expect(db.kb.markKbUsageViewed(U, conv.id, 1)).toEqual({ lastSeq: 1, unreadCount: 1 })
    expect(db.kb.markKbUsageViewed(U, conv.id, 1)).toEqual({ lastSeq: 1, unreadCount: 1 })
    expect(db.kb.markKbUsageViewed(U, conv.id, 0)).toEqual({ lastSeq: 1, unreadCount: 1 })

    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', status: 'error', query: 'q3', chars: 0 })
    const report = db.kb.kbUsageReport(U, conv.id)!
  ...
  it('границы пользователей и разговоров изолированы', () => {
    const a = db.chat.createConversation(U, 'A')
    const b = db.chat.createConversation(U, 'B')
    db.kb.addKbUsage({ userId: U, conversationId: a.id, source: 'auto', query: 'a', chars: 1 })
    db.kb.addKbUsage({ userId: U, conversationId: b.id, source: 'auto', query: 'b', chars: 1 })
    db.kb.markKbUsageViewed(U, a.id, 1)
    expect(db.kb.kbUsageReport(U, a.id)!.unreadCount).toBe(0)
    expect(db.kb.kbUsageReport(U, b.id)!.unreadCount).toBe(1)
  })

  it('проектный агрегат виден участнику и считает чаты, а не участнику — null', () => {
    db.identity.createUser('alice', 'x', 'developer')
    db.identity.createUser('bob', 'x', 'developer')
    const project = db.projects.createProject('alice', { name: 'P' })
    const a = db.chat.createConversation('alice', 'A')
    const b = db.chat.createConversation('alice', 'B')
    db.kb.addKbUsage({ userId: 'alice', conversationId: a.id, projectId: project.id, source: 'auto', query: 'q', chars: 300, sections: [twoSections()[0]] })
    db.kb.addKbUsage({ userId: 'alice', conversationId: b.id, projectId: project.id, source: 'tool_search', query: 'q', chars: 300, sections: [twoSections()[0]] })
    const report = db.kb.kbUsageProjectReport('alice', project.id)!
    ...
    expect(db.kb.kbUsageProjectReport('bob', project.id)).toBeNull()
  })

  it('статусы empty/error попадают в итоги и не считаются доставленными', () => {
    const conv = db.chat.createConversation(U, 'Чат')
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', status: 'empty', query: 'q', chars: 0 })
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', status: 'error', query: 'q', chars: 0, error: 'kb упала' })
    db.kb.addKbUsage({ userId: U, conversationId: conv.id, source: 'auto', query: 'q', chars: 10, injected: true })
    const totals = db.kb.kbUsageReport(U, conv.id)!.totals
    expect(totals).toMatchObject({ queries: 3, delivered: 1, empty: 1, errors: 1 })
    expect(db.kb.kbUsageReport(U, conv.id)!.recent[0].status).toBe('delivered')
  })
})
...
describe('VoiceChatDb — обращения к БЗ внутри CI-рана', () => {
  ...
  function runs() {
    ...
    const task = db.tasks.createTask('alice', project.id, { title: 'T', columnId: board.columns[0].id })!
    const other = db.tasks.createTask('alice', project.id, { title: 'T2', columnId: board.columns[0].id })!
    const conv = db.chat.createConversation('alice', 'Чат задачи')
    const mk = (taskId: string) => db.ci.createCiRun({
  ...
  it('ci_run_id и ci_step_id сохраняются и возвращаются в обращении', () => {
    ...
    expect(db.kb.kbUsageReport('alice', conv.id)!.recent[0]).toMatchObject({ ciRunId: first.id, ciStepId: 'step-7' })
  })

  it('режим БЗ рана — снимок настройки проекта на старте', () => {
    const project = db.projects.createProject('alice', { name: 'P' })
    db.projects.updateProject('alice', project.id, { ciKbContextMode: 'manual' })
    expect(db.projects.getProject('alice', project.id)!.ciKbContextMode).toBe('manual')
    const board = db.tasks.getBoard('alice', project.id)!
    const task = db.tasks.createTask('alice', project.id, { title: 'T', columnId: board.columns[0].id })!
    ...
    db.projects.updateProject('alice', project.id, { ciKbContextMode: 'off' })
    expect(db.ci.getCiRunRaw(run.id)!.kbContextMode).toBe('manual')
  })

  it('отчёт по ране считает только его обращения', () => {
    const { project, conv, first, second } = runs()
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, ciRunId: first.id, source: 'auto', query: 'q', chars: 300, sections: [twoSections()[0]] })
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, ciRunId: first.id, source: 'tool_document', query: 'q', chars: 200, sections: [twoSections()[1]] })
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, ciRunId: second.id, source: 'tool_search', query: 'q', chars: 100, sections: [twoSections()[0]] })
    // Обращение самого чата (без рана) в отчёт рана не попадает.
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, source: 'auto', query: 'q', chars: 999 })

    const report = db.kb.kbUsageRunReport('alice', first.id)!
  ...
  it('отчёт по задаче суммирует все её раны и не берёт чужую задачу', () => {
    ...
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, ciRunId: first.id, source: 'auto', query: 'q', chars: 300, sections: [twoSections()[0]] })
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, ciRunId: second.id, source: 'tool_search', query: 'q', chars: 200, sections: [twoSections()[0]] })
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, ciRunId: foreign.id, source: 'auto', query: 'q', chars: 700, sections: [twoSections()[1]] })

    const report = db.kb.kbUsageTaskReport('alice', project.id, task.id)!
  ...
  it('вопросы без ответа попадают в пробелы рана, отвеченные — нет', () => {
    const { project, conv, first, second } = runs()
    const usage = (query: string, status: 'empty' | 'error' | 'delivered', error?: string) => db.kb.addKbUsage({
    ...
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, ciRunId: second.id, source: 'auto', query: 'чужой ран', status: 'empty', chars: 0 })

    const gaps = db.kb.kbUsageRunGaps(first.id)
    ...
    db.kb.addKbUsage({ userId: 'alice', conversationId: conv.id, projectId: project.id, ciRunId: second.id, source: 'auto', query: 'без причины', status: 'empty', chars: 0 })
    expect(db.kb.kbUsageRunGaps(second.id).find((g) => g.query === 'без причины')!.reason).toBe('база знаний не ответила')
  })

  it('названный моделью пробел хранится один раз, повтор берёт более полный ответ', () => {
    const { first } = runs()
    db.ci.addCiRunKbGaps(first.id, 'step-1', [
    ...
    db.ci.addCiRunKbGaps(first.id, 'step-2', [{ question: 'где живёт fix-loop', answer: 'хук attemptFix в ci/modelHooks.ts, лимиты в настройках CI' }])
    const gaps = db.ci.ciRunKbGaps(first.id)
    ...
    db.ci.addCiRunKbGaps(first.id, 'step-3', [{ question: 'где живёт fix-loop', answer: 'там же' }])
    expect(db.ci.ciRunKbGaps(first.id)[0].answer).toContain('лимиты в настройках CI')
  })

  it('чужому пользователю отчёты по ране и задаче недоступны (404 у роута)', () => {
    const { project, task, first } = runs()
    expect(db.kb.kbUsageRunReport('bob', first.id)).toBeNull()
    expect(db.kb.kbUsageTaskReport('bob', project.id, task.id)).toBeNull()
    // Несуществующие ран/задача — тоже null, а не пустой отчёт.
    expect(db.kb.kbUsageRunReport('alice', 'нет-такого')).toBeNull()
    expect(db.kb.kbUsageTaskReport('alice', project.id, 'нет-такой')).toBeNull()
  })
})
