...
export class VoiceChatDb {
  ...
  // Совместимый фасад для маршрутов, которые ещё не переведены на доменные repos.
  getTaskDetail(userId: string, projectId: string, taskId: string) { return this.tasks.getTaskDetail(userId, projectId, taskId) }
  getColumnIdBySemantic(projectId: string, semanticType: KanbanColumnSemanticType) { return this.projects.getColumnIdBySemantic(projectId, semanticType) }
  deleteConversation(userId: string, id: string): void { this.chat.deleteConversation(userId, id) }
  addMember(userId: string, projectId: string, username: string) { return this.projects.addMember(userId, projectId, username) }
  latestTaskRunResult(taskId: string) { return this.tasks.latestTaskRunResult(taskId) }
  taskReworkCycles(userId: string, projectId: string, taskId: string) { return this.tasks.taskReworkCycles(userId, projectId, taskId) }
  taskReworkCycleByIdempotencyKey(userId: string, projectId: string, taskId: string, key: string) { return this.tasks.taskReworkCycleByIdempotencyKey(userId, projectId, taskId, key) }
  assertTaskDesignSource(userId: string, projectId: string, taskId: string, conversationId: string): void { this.tasks.assertTaskDesignSource(userId, projectId, taskId, conversationId) }
  taskAttachments(userId: string, projectId: string, taskId: string, scope: 'source' | 'rework_draft' = 'source') { return this.tasks.taskAttachments(userId, projectId, taskId, scope) }
  createTaskAttachment(userId: string, projectId: string, taskId: string, input: { name: string; mimeType?: string; dataBase64: string; scope?: 'source' | 'rework_draft' }) { return this.tasks.createTaskAttachment(userId, projectId, taskId, input) }
  deleteTaskAttachment(userId: string, projectId: string, taskId: string, attachmentId: string): boolean { return this.tasks.deleteTaskAttachment(userId, projectId, taskId, attachmentId) }
  createTaskReworkCycle(userId: string, projectId: string, taskId: string, key: string, input: { description: string; criteria: string[]; makeSources: Array<{ conversationId: string; mode: 'whole_project' | 'files'; paths: string[] }>; attachmentIds: string[] }) {
    return this.tasks.createPersistentTaskReworkCycle(userId, projectId, taskId, key, input)
  }
