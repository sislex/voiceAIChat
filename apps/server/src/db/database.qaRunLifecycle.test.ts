...
function componentFixture(uiImpact: 'none' | 'existing_components' = 'existing_components') {
  ...
  const project = db.projects.createProject('owner', { name: 'Component QA' + suffix })
...
describe('Component QA: контекст исполнения', () => {
  it('очередной ран отдаёт машину, каталог и команды', () => {
    ...
    expect(db.ci.componentQaExecutionContext(run.id)).toEqual({
  ...
  it('запущенный ран контекста уже не отдаёт — он выдаётся один раз, на старте', () => {
    ...
    expect(db.ci.componentQaExecutionContext(run.id)).toBeNull()
  })

  it('несуществующий ран — null, а не исключение', () => {
    expect(db.ci.componentQaExecutionContext('нет-такого')).toBeNull()
  })
  ...
  it('своя команда стадии перекрывает общую команду тестирования', () => {
    const { project, task } = componentFixture()
    db.projects.updateProject('owner', project.id, { testCommand: 'npm run gate', componentQaCommand: 'npm run test:storybook' })
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    expect(db.ci.componentQaExecutionContext(run.id)?.commands).toEqual(['npm run test:storybook'])
  })

  it('пустая команда стадии наследует команду тестирования проекта', () => {
    const { project, task } = componentFixture()
    db.projects.updateProject('owner', project.id, { testCommand: 'npm run gate', componentQaCommand: '   ' })
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    expect(db.ci.componentQaExecutionContext(run.id)?.commands).toEqual(['npm run gate'])
  })
  ...
  it('у старой рабочей директории кэш пустой, но контекст выдаётся', () => {
    ...
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    expect(db.ci.componentQaExecutionContext(run.id)?.npmCacheDir).toBeNull()
  })
...
describe('Component QA: журнал рана', () => {
  it('вывод копится только у запущенного рана', () => {
    ...
    db.ci.appendComponentQaLog(run.id, 'stdout', 'до запуска')
    expect(logOf(run.id)).toBe('')
    db.ci.markComponentQaRunning(run.id)
    db.ci.appendComponentQaLog(run.id, 'stdout', 'раз')
    db.ci.appendComponentQaLog(run.id, 'stdout', 'два')
  ...
  it('поток stderr помечается префиксом — иначе причина падения теряется в общем выводе', () => {
    ...
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    db.ci.markComponentQaRunning(run.id)
    db.ci.appendComponentQaLog(run.id, 'stderr', 'ошибка')
  ...
  it('завершённый ран журнал больше не принимает', () => {
    ...
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    db.ci.markComponentQaRunning(run.id)
    db.ci.appendComponentQaLog(run.id, 'stdout', 'до финиша')
    db.ci.finishComponentQaRun('owner', run.id, { status: 'passed', scenarios: [], commands: [], summary: 'ок' })
    db.ci.appendComponentQaLog(run.id, 'stdout', 'после финиша')
...
describe('Component QA: завершение и отмена', () => {
  it('финиш возможен только из running', () => {
    const { project, task } = componentFixture()
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    expect(() => db.ci.finishComponentQaRun('owner', run.id, { status: 'passed', scenarios: [], commands: [], summary: 'ок' }))
      .toThrow(/not running/)
  })
  ...
  it('финиш сохраняет итог, сводку и адрес витрины', () => {
    ...
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    db.ci.markComponentQaRunning(run.id)
    const finished = db.ci.finishComponentQaRun('owner', run.id, {
  ...
  it('отмена работает из очереди и из запуска, но не переписывает завершённый ран', () => {
    const { project, task } = componentFixture()
    const queued = db.ci.startComponentQaRun('owner', project.id, task.id)
    expect(db.ci.cancelComponentQaRun('owner', queued.id).status).toBe('cancelled')

    const { project: p2, task: t2 } = componentFixture()
    const running = db.ci.startComponentQaRun('owner', p2.id, t2.id)
    db.ci.markComponentQaRunning(running.id)
    db.ci.finishComponentQaRun('owner', running.id, { status: 'passed', scenarios: [], commands: [], summary: 'ок' })
    expect(db.ci.cancelComponentQaRun('owner', running.id).status).toBe('passed')
  })
  ...
  it('чужому ран не виден: отмена падает как «не найден», а не как «нет прав»', () => {
    ...
    const { project, task } = componentFixture()
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    expect(() => db.ci.cancelComponentQaRun('stranger', run.id)).toThrow(/not found/)
    expect(db.ci.getComponentQaRun('owner', run.id)!.status).toBe('queued')
  })

  it('привязка fix-рана переводит ран в failed и помечает дефектом реализации', () => {
    const { project, task } = componentFixture()
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    const linked = db.ci.linkComponentQaFixRun('owner', run.id, 'fix-1')
  ...
  it('привязка fix-рана не сдвигает уже проставленное время завершения', () => {
    ...
    const run = db.ci.startComponentQaRun('owner', project.id, task.id)
    db.ci.markComponentQaRunning(run.id)
    const finished = db.ci.finishComponentQaRun('owner', run.id, { status: 'failed', scenarios: [], commands: [], summary: 'упало' })
    const linked = db.ci.linkComponentQaFixRun('owner', run.id, 'fix-1')
...
describe('Integration QA: контекст и журнал', () => {
  ...
  it('очередной ран отдаёт свои команды проверки', () => {
    ...
    expect(db.ci.integrationTestExecutionContext(run.id)).toEqual({
  ...
  it('запущенный ран контекста не отдаёт', () => {
    ...
    expect(db.ci.integrationTestExecutionContext(run.id)).toBeNull()
  })

  it('своя команда этапа перекрывает общую, пустая — наследует', () => {
    const { project, task } = integrationFixture()
    db.projects.updateProject('owner', project.id, { testCommand: 'npm run gate', integrationTestCommand: 'npm run test:integration' })
    const run = db.ci.startIntegrationTestRun('owner', project.id, task.id)
    expect(db.ci.integrationTestExecutionContext(run.id)?.commands).toEqual(['npm run test:integration'])
    db.projects.updateProject('owner', project.id, { integrationTestCommand: '' })
    expect(db.ci.integrationTestExecutionContext(run.id)?.commands).toEqual(['npm run gate'])
  })

  it('кэш гейта отдаёт только точную пару коммит + набор команд', () => {
    const { project, task } = integrationFixture()
    db.ci.recordPassedGateResult({ projectId: project.id, taskId: task.id, commitSha: SHA, signature: 'sig-1', commands: ['npm run gate'], runKind: 'component_qa', runId: 'run-1' })
    expect(db.ci.findPassedGateResult(SHA, 'sig-1')).toMatchObject({ runKind: 'component_qa', runId: 'run-1' })
    expect(db.ci.findPassedGateResult(SHA, 'sig-2')).toBeNull()
    expect(db.ci.findPassedGateResult('b'.repeat(40), 'sig-1')).toBeNull()
    expect(db.ci.findPassedGateResult('', 'sig-1')).toBeNull()
    // Повторная запись того же ключа не ломает уникальный индекс.
    db.ci.recordPassedGateResult({ projectId: project.id, taskId: task.id, commitSha: SHA, signature: 'sig-1', commands: ['npm run gate'], runKind: 'integration_tests', runId: 'run-2' })
    expect(db.ci.findPassedGateResult(SHA, 'sig-1')?.runId).toBe('run-1')
  })
  ...
  it('вывод копится только у запущенного рана', () => {
    ...
    db.ci.appendIntegrationTestLog(run.id, 'до запуска')
    expect(log()).toBe('')
    db.ci.markIntegrationTestRunning(run.id)
    db.ci.appendIntegrationTestLog(run.id, 'проверка')
  ...
  it('финиш возможен только из running и сохраняет причину отказа', () => {
    const { project, task } = integrationFixture()
    const run = db.ci.startIntegrationTestRun('owner', project.id, task.id)
    expect(() => db.ci.finishIntegrationTestRun('owner', run.id, { status: 'failed', commands: [], summary: 'x' })).toThrow(/not running/)
    db.ci.markIntegrationTestRunning(run.id)
    const finished = db.ci.finishIntegrationTestRun('owner', run.id, {
  ...
  it('отмена работает из очереди, чужому пользователю запрещена', () => {
    const { project, task } = integrationFixture()
    const run = db.ci.startIntegrationTestRun('owner', project.id, task.id)
    expect(() => db.ci.cancelIntegrationTestRun('stranger', run.id)).toThrow(/QA permission required/)
    expect(db.ci.cancelIntegrationTestRun('owner', run.id).status).toBe('cancelled')
  })
  ...
  it('fix-ран привязывается один раз — повторная привязка не перетирает первую', () => {
    ...
    const { project, task } = integrationFixture()
    const run = db.ci.startIntegrationTestRun('owner', project.id, task.id)
    expect(db.ci.linkIntegrationTestFixRun('owner', run.id, 'fix-1').linkedFixRunId).toBe('fix-1')
    expect(db.ci.linkIntegrationTestFixRun('owner', run.id, 'fix-2').linkedFixRunId).toBe('fix-1')
  })
})
