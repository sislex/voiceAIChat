...
describe('board: колонки и порядок', () => {
  it('createColumn добавляет в конец; reorderColumns переставляет', () => {
    ...
    expect(cols.map((c) => c.name)).toEqual(['Бэклог', 'Готово к разработке', 'В разработке', 'Автотестирование', 'Создание сценариев ручного QA', 'Ручное QA', 'Ожидает мержа', 'Требуется решение', 'Готово', 'Review'])
