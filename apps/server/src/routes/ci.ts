...
export function registerCiRoutes(
  ...
  const isOwner = async (req: FastifyRequest, projectId: string): Promise<boolean> => (await db.projects.getProject(uid(req), projectId))?.role === 'owner'
  ...
  // --- Справочник команд ---
  app.get<{ Querystring: { projectId?: string } }>('/api/ci/commands', async (req) => await db.ci.listCiCommands(uid(req), req.query.projectId))

  app.get<{ Params: { id: string } }>('/api/ci/commands/:id', async (req, reply) => await db.ci.getCiCommand(uid(req), req.params.id) ?? nf(reply))
  ...
  app.post<{ Body: CiCommandInput }>('/api/ci/commands', async (req, reply) => {
    ...
    if (scope === 'project') {
      if (!body.projectId || !await isOwner(req, body.projectId)) return forbid(reply)
    }
    try {
      return await db.ci.createCiCommand(uid(req), body)
    } catch (e) {
      return bad(reply, e)
    }
  })
  ...
  app.patch<{ Params: { id: string }; Body: CiCommandInput }>('/api/ci/commands/:id', async (req, reply) => {
    const cur = await db.ci.getCiCommand(uid(req), req.params.id)
    if (!cur) return nf(reply)
    if (cur.scope === 'global' ? !isAdmin(req) : !await isOwner(req, cur.projectId!)) return forbid(reply)
    try {
      return await db.ci.updateCiCommand(uid(req), req.params.id, req.body ?? {}) ?? nf(reply)
    } catch (e) {
      return bad(reply, e)
    }
  })
  ...
  app.delete<{ Params: { id: string } }>('/api/ci/commands/:id', async (req, reply) => {
    ...
    if (!cur) return nf(reply)
    if (cur.scope === 'global' ? !isAdmin(req) : !await isOwner(req, cur.projectId!)) return forbid(reply)
    return { ok: await db.ci.softDeleteCiCommand(uid(req), req.params.id) }
  })

  app.get<{ Params: { id: string } }>('/api/ci/commands/:id/usage', async (req, reply) => {
    if (!await db.ci.getCiCommand(uid(req), req.params.id)) return nf(reply)
    return await db.ci.ciCommandUsage(req.params.id)
  })

  // --- Глобальные настройки CI (чтение — любой; правка — глобальный admin) ---
  app.get('/api/ci/settings', async () => await db.ci.getCiSettings())
  app.put('/api/ci/settings', async (req, reply) => {
    ...
    return await db.ci.updateCiSettings((req.body ?? {}) as Partial<CiGlobalSettings>)
  })

  // --- Слот-конфиг проекта (дефолты) ---
  app.get<{ Params: { id: string } }>('/api/projects/:id/ci', async (req, reply) => {
    if (!await db.projects.getProject(uid(req), req.params.id)) return nf(reply)
    return await db.ci.getCiSlotConfig('project', req.params.id)
  })
  app.put<{ Params: { id: string }; Body: { beforeModel?: string[]; afterModel?: string[] } }>('/api/projects/:id/ci', async (req, reply) => {
    if (!await isOwner(req, req.params.id)) return forbid(reply)
    const b = req.body ?? {}
    if (b.beforeModel) await db.ci.setCiSlotCommands('project', req.params.id, 'before_model', b.beforeModel)
    if (b.afterModel) await db.ci.setCiSlotCommands('project', req.params.id, 'after_model', b.afterModel)
    return await db.ci.getCiSlotConfig('project', req.params.id)
  })

  // --- Движок/модель проекта и задачи (с наследованием) ---
  const projectLlmView = async (userId: string, projectId: string) => {
    const inherited = await db.ci.ciLlmDefaultsForUser(userId)
    const own = await db.ci.getCiLlmConfig('project', projectId)
  ...
  app.put<{ Params: { id: string }; Body: CiLlmConfig }>('/api/projects/:id/ci/llm', async (req, reply) => {
    if (!await isOwner(req, req.params.id)) return forbid(reply)
    await db.ci.setCiLlmConfig('project', req.params.id, req.body)
    return projectLlmView(uid(req), req.params.id)
  })
  app.delete<{ Params: { id: string } }>('/api/projects/:id/ci/llm', async (req, reply) => {
    if (!await isOwner(req, req.params.id)) return forbid(reply)
    await db.ci.clearCiLlmConfig('project', req.params.id)
    return projectLlmView(uid(req), req.params.id)
  })
  const taskLlmView = async (userId: string, projectId: string, taskId: string): Promise<{ config: CiLlmConfig; overridden: boolean; projectDefault: CiLlmConfig }> => ({
    config: await db.ci.resolveTaskLlmConfig(projectId, taskId, userId),
    overridden: await db.ci.getCiLlmConfig('task', taskId) !== null,
    projectDefault: await db.ci.getCiLlmConfig('project', projectId) ?? await db.ci.ciLlmDefaultsForUser(userId)
  })
  ...
  app.put<{ Params: { id: string; taskId: string }; Body: CiLlmConfig }>('/api/projects/:id/tasks/:taskId/ci/llm', async (req, reply) => {
    if (!await db.tasks.getCiTask(uid(req), req.params.id, req.params.taskId)) return nf(reply)
    return await db.ci.setCiLlmConfig('task', req.params.taskId, req.body)
  })
  ...
  app.delete<{ Params: { id: string; taskId: string } }>('/api/projects/:id/tasks/:taskId/ci/llm', async (req, reply) => {
    if (!await db.tasks.getCiTask(uid(req), req.params.id, req.params.taskId)) return nf(reply)
    await db.ci.clearCiLlmConfig('task', req.params.taskId)
    return taskLlmView(uid(req), req.params.id, req.params.taskId)
  })
  ...
  const userStageFallback = async (userId: string) => {
    const llm = await db.ci.ciLlmDefaultsForUser(userId)
  ...
  app.get<{ Params: { id: string; stage: string } }>('/api/projects/:id/ci/stages/:stage/llm', async (req, reply) => {
    ...
    return {
      override: await db.ci.getCiStageLlmConfig('project', req.params.id, req.params.stage),
      effective: await db.ci.resolveTaskStageLlmConfig(req.params.id, '', req.params.stage, await userStageFallback(uid(req)))
    }
  })
  app.put<{ Params: { id: string; stage: string }; Body: CiStageLlmSelection }>('/api/projects/:id/ci/stages/:stage/llm', async (req, reply) => {
    if (!await isOwner(req, req.params.id)) return forbid(reply)
    if (!validStage(req.params.stage)) return bad(reply, 'Неизвестный этап workflow')
    return await db.ci.setCiStageLlmConfig('project', req.params.id, req.params.stage, req.body ?? {})
  })
  app.delete<{ Params: { id: string; stage: string } }>('/api/projects/:id/ci/stages/:stage/llm', async (req, reply) => {
    if (!await isOwner(req, req.params.id)) return forbid(reply)
    if (!validStage(req.params.stage)) return bad(reply, 'Неизвестный этап workflow')
    await db.ci.clearCiStageLlmConfig('project', req.params.id, req.params.stage)
    return { effective: await db.ci.resolveTaskStageLlmConfig(req.params.id, '', req.params.stage, await userStageFallback(uid(req))) }
  })
  ...
  app.get<{ Params: { id: string; taskId: string; stage: string } }>('/api/projects/:id/tasks/:taskId/ci/stages/:stage/llm', async (req, reply) => {
    ...
    if (!validStage(req.params.stage)) return bad(reply, 'Неизвестный этап workflow')
    return {
      override: await db.ci.getCiStageLlmConfig('task', req.params.taskId, req.params.stage),
      projectDefault: await db.ci.getCiStageLlmConfig('project', req.params.id, req.params.stage),
      effective: await db.ci.resolveTaskStageLlmConfig(req.params.id, req.params.taskId, req.params.stage, await userStageFallback(uid(req)))
    }
  })
  app.put<{ Params: { id: string; taskId: string; stage: string }; Body: CiStageLlmSelection }>('/api/projects/:id/tasks/:taskId/ci/stages/:stage/llm', async (req, reply) => {
    ...
    if (!await db.tasks.getCiTask(uid(req), req.params.id, req.params.taskId)) return nf(reply)
    if (!validStage(req.params.stage)) return bad(reply, 'Неизвестный этап workflow')
    await db.ci.setCiStageLlmConfig('task', req.params.taskId, req.params.stage, req.body ?? {})
    return { effective: await db.ci.resolveTaskStageLlmConfig(req.params.id, req.params.taskId, req.params.stage, await userStageFallback(uid(req))) }
  })
  app.delete<{ Params: { id: string; taskId: string; stage: string } }>('/api/projects/:id/tasks/:taskId/ci/stages/:stage/llm', async (req, reply) => {
    if (!await isOwner(req, req.params.id)) return forbid(reply)
    if (!validStage(req.params.stage)) return bad(reply, 'Неизвестный этап workflow')
    await db.ci.clearCiStageLlmConfig('task', req.params.taskId, req.params.stage)
    return { effective: await db.ci.resolveTaskStageLlmConfig(req.params.id, req.params.taskId, req.params.stage, await userStageFallback(uid(req))) }
  })
  ...
  app.get<{ Params: { id: string; taskId: string } }>('/api/projects/:id/tasks/:taskId/ci/machines', async (req, reply): Promise<CiTaskMachines | FastifyReply> => {
    ...
    const project = await db.projects.getProject(userId, req.params.id)
    const task = await db.tasks.getCiTask(userId, req.params.id, req.params.taskId)
    if (!project || !task) return nf(reply)
    const personalIds = new Set((await db.machines.listAgents(userId)).map((agent) => agent.id))
    const usable = await db.machines.listUsableAgents(userId, project.id)
    const myDefault = await db.machines.getUserProjectDefaultMachine(userId, project.id)
    ...
    const machines = await Promise.all(usable.map(async (agent) => {
      ...
      const shared = await db.machines.isMachineSharedWithProject(project.id, agent.id)
    ...
    return {
      ...
      unavailableSelection: selectedAvailable || !task.agentId ? null : { agentId: task.agentId, name: await db.machines.agentName(task.agentId) ?? null },
      inheritanceSource: task.agentId ? 'explicit' : 'project_default',
      effectiveAgentId,
      effectiveMachineName: effectiveMachine?.name ?? (effectiveAgentId ? await db.machines.agentName(effectiveAgentId) ?? null : null)
    }
  })
  ...
  app.post<{ Params: { id: string; taskId: string }; Body: { mode?: CiRunMode; provider?: string; model?: string; launch?: string; agentId?: string } | undefined }>('/api/projects/:id/tasks/:taskId/ci/run', workflowGuard, async (req, reply) => {
    ...
    const res = await ci.start(uid(req), req.params.id, req.params.taskId, { mode, provider, model, launch, agentId })
  ...
  app.post<{ Params: { id: string; taskId: string }; Body: { agentId?: string } | undefined }>('/api/projects/:id/tasks/:taskId/ci/run-on-machine', workflowGuard, async (req, reply) => {
    ...
    const res = await ci.forceStartOnMachine(uid(req), req.params.id, req.params.taskId, agentId)
    if ('error' in res) return reply.code(409).send({ error: res.error })
    return reply.code(202).send(res.run)
  })
  ...
  app.post<{ Params: { runId: string; interactionId: string }; Body: { text?: string; decision?: CiPlanDecision } | undefined }>(
    ...
    async (req, reply) => {
      ...
      const res = await ci.answerInteraction(uid(req), req.params.runId, req.params.interactionId, { text: body.text, decision })
      if ('error' in res) return reply.code(409).send({ error: res.error })
  ...
  app.get<{ Params: { runId: string }; Querystring: { limit?: string } }>('/api/ci/runs/:runId/log', async (req, reply) => {
    ...
    return await db.ci.getCiRunLog(uid(req), req.params.runId, Number(req.query.limit) || undefined)
  })
  ...
  app.get<{ Params: { runId: string }; Querystring: { limit?: string } }>('/api/ci/runs/:runId/kb-usage', async (req, reply) => {
    const report = await db.kb.kbUsageRunReport(uid(req), req.params.runId, Number(req.query.limit) || undefined)
  ...
  app.get<{ Params: { id: string; taskId: string }; Querystring: { limit?: string } }>(
    ...
    async (req, reply) => {
      const report = await db.kb.kbUsageTaskReport(uid(req), req.params.id, req.params.taskId, Number(req.query.limit) || undefined)
      return report ?? nf(reply)
    }
  )
  ...
  app.get<{ Params: { runId: string } }>('/api/ci/runs/:runId/report', async (req, reply) =>
    await db.ci.ciRunReport(uid(req), req.params.runId) ?? nf(reply)
  )
  app.get<{ Params: { id: string; taskId: string } }>('/api/projects/:id/tasks/:taskId/report', async (req, reply) =>
    await db.ci.ciTaskReport(uid(req), req.params.id, req.params.taskId) ?? nf(reply)
  )
  app.get<{ Params: { id: string; taskId: string } }>('/api/projects/:id/tasks/:taskId/timeline', async (req, reply) =>
    await db.tasks.taskTimeline(uid(req), req.params.id, req.params.taskId) ?? nf(reply)
  )
  app.get<{ Params: { id: string; taskId: string } }>('/api/projects/:id/tasks/:taskId/improvements', async (req) =>
    await db.tasks.listTaskImprovements(uid(req), req.params.id, req.params.taskId)
  )
  app.get<{ Params: { id: string } }>('/api/projects/:id/improvements/tasks', async (req) =>
    await db.tasks.listProjectImprovementTaskIds(uid(req), req.params.id)
  )
  app.get<{ Params: { id: string } }>('/api/projects/:id/improvements', async (req) =>
    await db.tasks.listProjectImprovements(uid(req), req.params.id)
  )
  app.delete<{ Params: { id: string } }>('/api/improvements/:id', async (req, reply) => {
    const projectId = await db.tasks.improvementProjectId(req.params.id)
    if (!await db.tasks.deleteTaskImprovement(uid(req), req.params.id)) return nf(reply)
  ...
  app.patch<{ Params: { id: string }; Body: { status?: string } }>('/api/improvements/:id', async (req, reply) => {
    ...
    try {
      const projectId = await db.tasks.improvementProjectId(req.params.id)
      const updated = await db.tasks.updateTaskImprovementStatus(uid(req), req.params.id, status)
  ...
  app.post<{ Params: { id: string }; Body: import('@voicechat/shared').CreateTaskFromImprovementInput | undefined }>('/api/improvements/:id/create-task', async (req, reply) => {
    ...
    let created: NonNullable<Awaited<ReturnType<VoiceChatDb['tasks']['createTaskFromImprovement']>>>
    try {
      const result = await db.tasks.createTaskFromImprovement(uid(req), req.params.id, body)
    ...
    const task = await db.tasks.getCiTask(uid(req), created.task.projectId, created.task.id) ?? created.task
  ...
  app.post<{ Params: { runId: string } }>('/api/ci/runs/:runId/dequeue', async (req, reply) => {
    const result = await ci.dequeue(uid(req), req.params.runId)
  ...
  app.post<{ Params: { runId: string } }>('/api/ci/runs/:runId/retry', workflowGuard, async (req, reply) => {
    ...
    const res = await ci.start(uid(req), detail.run.projectId, detail.run.taskId)
    if ('error' in res) return reply.code(409).send({ error: res.error })
    return reply.code(202).send(res.run)
  })
  ...
  app.get<{ Params: { id: string } }>('/api/projects/:id/ci/metrics', async (req, reply) => {
    if (!await db.projects.getProject(uid(req), req.params.id)) return nf(reply)
    return { commands: await db.ci.ciCommandMetrics(uid(req), req.params.id), modelWork: await db.ci.ciModelWorkMetric(uid(req), req.params.id) }
  })

  // --- Предложения модели по правке команд ---
  app.get<{ Querystring: { projectId?: string } }>('/api/ci/suggestions', async (req) => await db.ci.listCiSuggestions(uid(req), req.query.projectId))
  app.post<{ Params: { id: string }; Body: { accept?: boolean } }>('/api/ci/suggestions/:id', async (req, reply) => {
    ...
    const res = await db.ci.resolveCiSuggestion(uid(req), req.params.id, req.body?.accept === true)
    return res ?? nf(reply)
  })
  ...
  app.get<{ Querystring: { projectId?: string } }>('/api/ci/workspaces', async (req) => await db.ci.listCiWorkspaceReport(uid(req), req.query.projectId))
}
