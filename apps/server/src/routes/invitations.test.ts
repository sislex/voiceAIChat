...
import type { MailMessage } from "@sislexa/identity/server/users/mailer"
...
describe('приглашения: сторона приглашённого', () => {
  ...
  it('приглашённый по логину принимает своё приглашение и получает проект', async () => {
    ...
    await db.identity.createEmailVerification({ token: 'verify-1', name: 'dave', email: 'bob@example.com', password: 'x', ttlMs: 60_000 })
    await db.identity.redeemEmailVerification('verify-1', 'developer')
