...
export function registerProjectTypeRoutes(app: FastifyInstance, db: VoiceChatDb): void {
   ...
   */
  const editableOrRefusal = async (
    req: Parameters<typeof uid>[0],
    id: string
  ): Promise<{ node: ProjectTypeNode } | { code: 404 | 409; error: string }> => {
    const node = await db.projects.getProjectType(id)
  ...
  app.get('/api/project-types', async (req) => await db.projects.listProjectTypes(uid(req)))

  app.get<{ Params: { id: string } }>('/api/project-types/:id', async (req, reply) => {
    const node = await db.projects.getProjectType(req.params.id)
    if (!node || !isProjectTypeVisible(node, uid(req))) return notFound(reply)
    return { node, chain: await db.projects.projectTypeChain(node.id), audit: await db.projects.projectTypeReviewAudit(node.id) }
  })
  ...
  app.post<{ Body: { parentId?: string | null; name?: string; description?: string; features?: unknown; defaults?: unknown } }>(
    ...
    async (req, reply) => {
      ...
      if (parentId && !(await db.projects.listProjectTypes(uid(req))).some((t) => t.id === parentId)) return badReq(reply, 'Родительский тип недоступен')
      try {
        return await db.projects.createProjectType(uid(req), {
          parentId,
          name,
  ...
  app.patch<{ Params: { id: string }; Body: { parentId?: string | null; name?: string; description?: string; features?: unknown; defaults?: unknown } }>(
    ...
    async (req, reply) => {
      const access = await editableOrRefusal(req, req.params.id)
      if ('code' in access) return reply.code(access.code).send({ error: access.error })
      const b = req.body ?? {}
      if (b.parentId !== undefined && b.parentId && !(await db.projects.listProjectTypes(uid(req))).some((t) => t.id === b.parentId)) {
        return badReq(reply, 'Родительский тип недоступен')
      }
      try {
        return await db.projects.updateProjectType(req.params.id, {
  ...
  app.delete<{ Params: { id: string } }>('/api/project-types/:id', async (req, reply) => {
    ...
    if ('code' in access) return reply.code(access.code).send({ error: access.error })
    try {
      return { ok: await db.projects.deleteProjectType(req.params.id) }
    } catch (error) {
  ...
  app.post<{ Params: { id: string } }>('/api/project-types/:id/publish', async (req, reply) => {
    ...
    if ('code' in access) return reply.code(access.code).send({ error: access.error })
    try {
      return await db.projects.setProjectTypeStatus(uid(req), req.params.id, 'pending') ?? notFound(reply)
    } catch (error) {
      return reply.code(409).send({ error: errMessage(error) })
    }
  })
  ...
  app.post<{ Params: { id: string } }>('/api/project-types/:id/unpublish', async (req, reply) => {
    ...
    try {
      return await db.projects.setProjectTypeStatus(uid(req), req.params.id, 'private') ?? notFound(reply)
    } catch (error) {
      return reply.code(409).send({ error: errMessage(error) })
    }
  })
  ...
  app.post<{ Params: { id: string }; Body: { name?: string } }>('/api/projects/:id/derive-type', async (req, reply) => {
    ...
    try {
      return await db.projects.deriveProjectType(uid(req), req.params.id, name) ?? notFound(reply)
    } catch (error) {
      return badReq(reply, errMessage(error))
    }
  })
  ...
  app.get('/api/admin/project-types', async () => await db.projects.listPendingProjectTypes())
  ...
  app.post<{ Params: { id: string }; Body: { decision?: 'approve' | 'reject'; note?: string } }>(
    ...
    async (req, reply) => {
      ...
      if (!node) return notFound(reply)
      try {
        return await db.projects.setProjectTypeStatus(uid(req), req.params.id, decision === 'approve' ? 'published' : 'rejected', req.body?.note ?? '') ?? notFound(reply)
      } catch (error) {
        return reply.code(409).send({ error: errMessage(error) })
      }
    }
