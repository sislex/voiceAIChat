...
export function registerInvitationRoutes(app: FastifyInstance, db: VoiceChatDb, options: InvitationRoutesOptions): void {
  ...
  app.get<{ Params: { id: string } }>('/api/projects/:id/invitations', async (req, reply) => {
    const list = await db.projects.listProjectInvitations(uid(req), req.params.id)
    if (list) return list
    ...
    return await db.projects.getProject(uid(req), req.params.id)
  ...
  app.post<{ Params: { id: string }; Body: { invitee?: string; role?: ProjectRole } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        await db.identity.logSecurityEvent({ user: uid(req), type: 'project_invited', ip: req.ip, userAgent: String(req.headers['user-agent'] ?? ''), details: `${req.params.id} → ${invitee}` })
  ...
  app.post<{ Params: { id: string; invitationId: string } }>(
    ...
    async (req, reply) => {
      const project = await db.projects.getProject(uid(req), req.params.id)
      if (!project) return notFound(reply)
      const refreshed = await db.projects.refreshProjectInvitationToken(uid(req), req.params.id, req.params.invitationId)
  ...
  app.delete<{ Params: { id: string; invitationId: string } }>(
    ...
    async (req, reply) => {
      const ok = await db.projects.revokeProjectInvitation(uid(req), req.params.id, req.params.invitationId)
  ...
  app.get('/api/invitations', async (req) => await db.projects.listInvitationsForUser(uid(req)))
  ...
  app.post<{ Params: { token: string } }>('/api/invitations/:token/accept', async (req, reply) => {
    try {
      const { projectId } = await db.projects.acceptProjectInvitation(uid(req), decodeURIComponent(req.params.token))
      options.membershipChanged?.(projectId, uid(req))
      await db.identity.logSecurityEvent({ user: uid(req), type: 'project_invite_accepted', ip: req.ip, userAgent: String(req.headers['user-agent'] ?? ''), details: projectId })
      return { projectId }
    } catch (error) {
      return badReq(reply, errMessage(error))
    }
  ...
  app.post<{ Params: { token: string } }>('/api/invitations/:token/decline', async (req, reply) => {
    try {
      return { ok: await db.projects.declineProjectInvitation(uid(req), decodeURIComponent(req.params.token)) }
    } catch (error) {
      return badReq(reply, errMessage(error))
    }
  })
  ...
  app.get<{ Params: { token: string } }>('/api/session/invitation/:token', async (req, reply) => {
    ...
    const preview = await db.projects.projectInvitationPreview(decodeURIComponent(req.params.token))
