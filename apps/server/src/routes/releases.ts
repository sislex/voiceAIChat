...
export function registerReleaseRoutes(app:FastifyInstance,db:VoiceChatDb,releases:ReleaseManager,resolver?:ManagedEnvironmentResolver,agents?:KanbanMachines):void {
  ...
  const ciTarget=async (req:FastifyRequest,projectId:string):Promise<ReleaseProjectTarget>=>await releaseCiTarget(db,releases,uid(req),projectId)
  const productionTarget=async (req:FastifyRequest,projectId:string):Promise<ProductionTarget|null>=>await releaseProductionTarget(db,managed,uid(req),projectId)
  const owner=async (req:FastifyRequest,projectId:string):Promise<boolean>=>await db.projects.isProjectOwner(uid(req),projectId)
  const prepareGuard={preHandler:async(req:FastifyRequest,reply:FastifyReply)=>{
    const projectId=(req.params as {id?:string}).id
    if(!projectId||!await owner(req,projectId))await forbidden(reply)
  }}
  const deployGuard={preHandler:async(req:FastifyRequest,reply:FastifyReply)=>{
    ...
    const projectId=params.id??(params.runId?(await db.ci.getMergeRun(uid(req),params.runId))?.projectId:undefined)
    if(!projectId||!await owner(req,projectId))await forbidden(reply)
  }}
  ...
  app.post<{Params:{id:string};Body:{confirmationToken?:string}}>('/api/projects/:id/releases/managed/confirm',deployGuard,async(req,reply)=>{
    ...
    try{
      ...
      return await db.projects.updateProject(uid(req),req.params.id,{productionEnvironmentMode:'managed'})??nf(reply)
    }catch(error){return bad(reply,error)}
  })
  ...
  app.post<{Params:{id:string};Body:{agentId?:string;storageId?:string;deployCommand?:string;healthCheckCommand?:string}}>('/api/projects/:id/production/bootstrap',deployGuard,async(req,reply)=>{
    ...
    try{
      const current=await project(req,projectId)
      ...
      if(!await db.machines.canUseAgent(userId,agentId,projectId))return bad(reply,'Нет доступа к выбранной машине')
      ...
      const storages=await db.machines.listMachineStorages(userId,agentId)
      ...
      await db.machines.linkMachine(userId,projectId,agentId,storage.id)
      ...
      await db.projects.updateProject(userId,projectId,{
      ...
      const defaultMachineSet=!await db.machines.getUserProjectDefaultMachine(userId,projectId)
      if(defaultMachineSet)await db.machines.setUserProjectDefaultMachine(userId,projectId,agentId)
      ...
      if(preflight.ok)await db.projects.updateProject(userId,projectId,{productionEnvironmentMode:'managed'})
      const mode=((await db.projects.getProject(userId,projectId))?.productionEnvironmentMode==='managed')?'managed' as const:'legacy' as const
  ...
  app.get<{Params:{id:string;releaseId:string}}>('/api/projects/:id/releases/:releaseId',async(req,reply)=>
    await db.releases.getProjectRelease(uid(req),req.params.id,req.params.releaseId)??nf(reply)
  )
  ...
  app.post<{Params:{runId:string}}>('/api/merge/runs/:runId/deploy',deployGuard,async(req,reply)=>{
    const run=await db.ci.getMergeRun(uid(req),req.params.runId)
    if(!run)return nf(reply)
    if(run.status!=='success')return bad(reply,'Деплой доступен только после успешного merge')
    const production=await productionTarget(req,run.projectId)
    if(!production)return bad(reply,'Production-машина, checkout, deploy-команда или health-check не настроены')
    try{
      ...
      const release=await releases.start(uid(req),await ciTarget(req,run.projectId),production,run.targetBranch)
      await db.ci.updateMergeRun(run.id,{deployId:release.id,deployVersion:release.version||release.branch,productionStatus:release.status})
      return reply.code(202).send(await db.ci.getMergeRun(uid(req),run.id))
    }catch(error){return bad(reply,error)}
  })
}
