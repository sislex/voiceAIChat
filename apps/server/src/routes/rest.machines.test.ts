...
describe('REST: хранилище машины', () => {
  async function connectFs(machineId: string, failMkdir = false, failWrite = false) {
    ...
    agentRegistry.register(machineId, 'Мак', socket, (await db.machines.listAgents(U)).find((item) => item.id === machineId)!.policy, '0.11.0')
  ...
  it('не оставляет ready-запись при ошибке прав', async () => {
    const machine = await db.machines.createAgent(U, 'Закрытый диск')
    await connectFs(machine.id, true)
  ...
  it('bootstrap проектного чата создаёт каталоги окружений, но не пишет environment.json (иначе poisoned manifest → деплой 400)', async () => {
    ...
    const project = await db.projects.createProject(U, { name: 'ChatAI', gitUrl: 'https://example.com/repo.git' })
  ...
  it('copy-to копирует файл на другую машину: в указанный каталог или в ChatAI/incoming цели', async () => {
    const source = await db.machines.createAgent(U, 'Мак')
    const target = await db.machines.createAgent(U, 'Прод')
    const sourceFs = await connectFs(source.id)
    const targetFs = await connectFs(target.id)
    ...
    const offline = await db.machines.createAgent(U, 'Спит')
...
describe('REST: журнал команд машины', () => {
  it('exec через REST попадает в журнал; фильтр по подстроке и CSV-экспорт', async () => {
    ...
    const socket = {
      ...
      async send(data: string) {
        ...
        if (message.t === 'exec.start') {
          await agentRegistry.handleMessage(created.id, { t: 'exec.chunk', execId: message.execId!, stream: 'stdout', data: `ran ${message.command}` })
          await agentRegistry.handleMessage(created.id, { t: 'exec.done', execId: message.execId!, exitCode: message.command === 'false' ? 1 : 0 })
        }
      }
    }
    ...
    // чужая машина — 404
    await db.identity.createUser('user2', '', 'developer')
    const other = await db.machines.createAgent('user2', 'X')
...
describe('REST: доступ участников к машине проекта (п.18)', () => {
  it('доступ «только чтение»: fs-чтение работает, запись/команды/группа — 403; полный доступ возвращает права', async () => {
    ...
    const socket = { close: vi.fn(), async send(data: string) {
      const m = JSON.parse(data) as { t: string; opId?: string; execId?: string; path?: string }
      if (m.t === 'exec.start') await agentRegistry.handleMessage(machine.id, { t: 'exec.done', execId: m.execId!, exitCode: 0 })
      else if (m.opId) await agentRegistry.handleMessage(machine.id, { t: 'fs.result', opId: m.opId, result: { root: '/', cwd: m.path ?? '/', entries: [] } })
    } }
    agentRegistry.register(machine.id, 'Общая', socket, (await db.machines.listAgents(U)).find((a) => a.id === machine.id)!.policy, '0.15.0')
    const project = await db.projects.createProject(U, { name: 'Shared' })
    await db.identity.createUser('dev2', '', 'developer')
    await db.projects.addMember(U, project.id, 'dev2')
    ...
    const conv = await db.chat.createConversation('dev2', 'C')
    await db.chat.setConversationProject('dev2', conv.id, project.id)
...
describe('REST: групповая команда (п.15)', () => {
  it('выполняет команду на нескольких машинах, сводка различает ok/failed/skipped; политика отклоняет весь запуск', async () => {
    ...
    const connect = async (id: string, exitCode: number) => {
      const socket = { close: vi.fn(), async send(data: string) { const m = JSON.parse(data) as { t: string; execId?: string }; if (m.t === 'exec.start') { await agentRegistry.handleMessage(id, { t: 'exec.chunk', execId: m.execId!, stream: 'stdout', data: `из ${id}` }); await agentRegistry.handleMessage(id, { t: 'exec.done', execId: m.execId!, exitCode }) } } }
      agentRegistry.register(id, 'M', socket, (await db.machines.listAgents(U)).find((a) => a.id === id)!.policy, '0.15.0')
    }
    await connect(one.id, 0)
    await connect(two.id, 3)
    await db.identity.createUser('user4', '', 'developer')
    const foreign = await db.machines.createAgent('user4', 'Чужая')
    ...
    expect((await db.machines.listMachineCommands(one.id)).map((r) => r.command)).toEqual(['uptime'])
...
describe('REST: токены агентов (срок, отзыв, привязка к IP)', () => {
  it('перевыпуск с ttlDays задаёт срок, отзыв закрывает вход, события попадают в журнал безопасности', async () => {
    ...
    expect((await db.machines.findAgentByTokenHash(hashAgentToken(rotated.token)))?.id).toBe(created.id)
    expect((await inj({ method: 'DELETE', url: `/api/agents/${created.id}/token` })).json()).toEqual({ ok: true })
    expect(await db.machines.findAgentByTokenHash(hashAgentToken(rotated.token))).toBeNull()
    ...
    await db.identity.createUser('user3', '', 'developer')
    const other = await db.machines.createAgent('user3', 'X')
...
describe('REST: метрики машин для админки', () => {
  it('stats и Prometheus-метрики агрегируют журнал команд и статус реестра', async () => {
    const created = (await inj({ method: 'POST', url: '/api/agents', payload: { name: 'M' } })).json()
    const socket = { close: vi.fn(), async send(data: string) { const m = JSON.parse(data) as { t: string; execId?: string }; if (m.t === 'exec.start') await agentRegistry.handleMessage(created.id, { t: 'exec.done', execId: m.execId!, exitCode: 1 }) } }
    agentRegistry.register(created.id, 'M', socket, (await db.machines.listAgents(U)).find((a) => a.id === created.id)!.policy, '0.15.0')
...
describe('REST: утилиты машины (exec/fs)', () => {
  it('exec: 404 на чужую машину; 400 на офлайн-машину владельца', async () => {
    ...
    const other = await db.machines.createAgent('user', 'UserBox')
