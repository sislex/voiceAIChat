...
export class FileKnowledgeBaseService implements KnowledgeBaseService {
  ...
  async topics(): Promise<KbDocumentSummary[]> { return this.documents.map(({ document }) => summaryOf(document)) }
  async document(id: string): Promise<KbDocument | null> { return this.byId.get(id)?.document ?? null }
  ...
  async context(query: string, budget = 3500): Promise<KbContextBundle> {
    ...
    return buildContext(query, await this.search({ query, limit: 8 }), async (result) => texts.get(result.chunkId))
  }
}
