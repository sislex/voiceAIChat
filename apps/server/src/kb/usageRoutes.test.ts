...
describe('GET /api/conversations/:id/kb-usage', () => {
  it('отдаёт итоги, разделы, ленту и lastSeq своего чата', async () => {
    const conv = await db.chat.createConversation('admin', 'Чат')
    ...
    await db.kb.addKbUsage({ userId: 'admin', conversationId: conv.id, source: 'tool_search', query: 'ws', chars: 120 })
  ...
  it('режим разговора виден в отчёте (панель показывает чип режима)', async () => {
    const conv = await db.chat.createConversation('admin', 'Чат')
    await db.chat.setConversationKbContextMode('admin', conv.id, 'manual')
  ...
  it('успешно отмечает загруженную границу, не захватывая более новое событие', async () => {
    const conv = await db.chat.createConversation('admin', 'Чат')
    await db.kb.addKbUsage({ userId: 'admin', conversationId: conv.id, source: 'auto', query: 'q1', chars: 1 })
    const snapshot = (await get(adminTok, REST.conversationKbUsage(conv.id))).json() as KbUsageReport
    await db.kb.addKbUsage({ userId: 'admin', conversationId: conv.id, source: 'tool_search', query: 'q2', chars: 1 })
