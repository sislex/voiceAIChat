...
function setup(result: { exitCode: number | null; timedOut: boolean } = { exitCode: 0, timedOut: false }) {
  ...
  const db = {
    getProject: () => ({ id: 'p1', gitUrl: 'https://example.test/repo.git', ciBranchTemplate: 'feature/{task_number}-{slug}', defaultAgentId: 'a1', machines: [{ agentId: 'a1', reposRoot: '/repos', path: '/repos/project' }, { agentId: 'a2', reposRoot: '/other', path: '/other/project' }] }),
...
describe('FeaturePreviewManager', () => {
  ...
  it('prepares an isolated feature workspace on another project machine', async () => {
    ...
    expect(env.workspacePath).toBe('/other/preview/vc-preview-p1-t1')
    expect(executor.run).toHaveBeenCalledWith(expect.objectContaining({
      agentId: 'a2', workdir: '/other/project', env: expect.objectContaining({ VC_PREVIEW_BRANCH: 'feature/1-task-one' })
    }), expect.any(Function))
    await wait()
  })
