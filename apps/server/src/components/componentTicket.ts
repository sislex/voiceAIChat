...
export class ComponentTicketService {
  ...
  async create(userId: string, projectId: string, input: ComponentTicketInput): Promise<ComponentTicketOutcome> {
    ...
    const ref = await this.deps.git.resolve(userId, projectId, input.workspaceId, { write: true })
    ...
    const task = await this.deps.db.tasks.createTask(userId, projectId, {
      columnId: column.id,
      title,
    ...
    try {
      ...
      const workspace = await this.deps.db.ci.createCiWorkspace({ projectId, taskId: task.id, agentId: ref.agentId, path: ref.path })
      await this.deps.db.ci.updateCiWorkspaceRevision(workspace.id, branch, push.sha || commit.sha, true)
      return {
        taskId: task.id,
