...
describe('WS: живые изменения списка сессий', () => {
  ...
  it('«выйти везде» адресно гасит каждую убитую вкладку', async () => {
    await db.identity.createUser('bulk', 'bulk-user-pass-2026', 'developer')
    const login = async (ua: string): Promise<string> =>
    ...
    const phoneSid = (await db.identity.listSessions('bulk')).find((s) => s.userAgent === 'Phone/1.0')!.sid
  ...
  it('чужому пользователю кадры не приходят', async () => {
    await db.identity.createUser('mine', 'mine-user-pass-2026', 'developer')
    await db.identity.createUser('other', 'other-user-pass-2026', 'developer')
  ...
  it('переименование и админский отзыв тоже обновляют список живьём', async () => {
    await db.identity.createUser('renamer', 'renamer-pass-2026-ok', 'developer')
...
describe('WS /agent: смена политики с машины', () => {
  it('agent.setPolicy сохраняется в БД владельца', async () => {
    const created = await db.machines.createAgent('admin', 'Box') // admin засеян buildServer
    ...
    for (let i = 0; i < 20 && (await db.machines.listAgents('admin'))[0]?.policy.allowWrite !== false; i++) {
      await new Promise((r) => setTimeout(r, 25))
    }
    expect((await db.machines.listAgents('admin'))[0].policy.allowWrite).toBe(false)
    ws.close()
  })
})
...
describe('WS: роль user не выполняет на сервере (форс plan)', () => {
  ...
  const echoPerm: LlmClient = {
    send(req, h) {
      void h.onDone(`perm:${req.permissionMode ?? 'none'}`)
      return { cancel: () => {} }
    }
  }
  ...
  async function permForRole(role: 'admin' | 'developer', convMode?: 'plan' | 'acceptEdits' | 'bypassPermissions'): Promise<string> {
    ...
    const conv = await rdb.chat.createConversation(role, 'Чат')
...
describe('WS: выбор движка Codex', () => {
  it('llmProvider=codex → используется codex-клиент; session-id с префиксом codex', async () => {
    ...
    await cdb.settings.saveSettings(U, { ...(await cdb.settings.getSettings(U)), llmProvider: 'codex', codexModel: 'gpt-5-codex' })
    const mockCodex: LlmClient = {
      send(req, h) {
        // модель берётся из codexModel
        void h.onSession(`thread-${req.model}`)
        void h.onDone('Ответ Codex')
        return { cancel: () => {} }
      }
    }
    ...
    const conv = await cdb.chat.createConversation(U, 'Чат')
    ...
    expect((await cdb.chat.getConversation(U, conv.id))?.claudeSessionId).toBe('codex:thread-gpt-5-codex')
    await capp.close()
    cdb.close()
  })
})
...
describe('WS: кадры использования базы знаний', () => {
  it('kb.usage доходит только своему пользователю', async () => {
    ...
    ;(await tracker.begin({ userId: U, conversationId: conv.id, source: 'tool_search' }, 'ws')).complete({ deliveredChars: 42 })
...
describe('сброс пароля по подтверждённому email', () => {
  it('не раскрывает наличие адреса, шлёт ссылку и одноразово меняет пароль с отзывом сессий', async () => {
    await db.identity.createEmailVerification({ token: 'verify-reset-user', name: 'reset-user', email: 'reset@example.test', password: 'old-password-1', ttlMs: 60_000 })
    expect(await db.identity.redeemEmailVerification('verify-reset-user', 'developer')).not.toBeNull()
    ...
    expect(await db.identity.verifyUserPassword('reset-user', 'new-password-2')).not.toBeNull()
    ...
    expect((await db.identity.listSecurityEvents({ user: 'reset-user', limit: 10 })).some((event) => event.type === 'password_reset' && event.details.includes('email'))).toBe(true)
  })

  it('отличает истёкшую ссылку и ограничивает запросы', async () => {
    await db.identity.createEmailVerification({ token: 'verify-expired-user', name: 'expired-user', email: 'expired@example.test', password: 'old-password-1', ttlMs: 60_000 })
    expect(await db.identity.redeemEmailVerification('verify-expired-user', 'developer')).not.toBeNull()
    await db.identity.createPasswordResetToken('expired-user', 'expired-token', -1)
