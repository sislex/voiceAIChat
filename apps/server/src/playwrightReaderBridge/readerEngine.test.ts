...
describe('выбор исполнителя модели по разговору Web Reader', () => {
  const make = (conversation: unknown) => createLocalPlaywrightReaderCore({ db: { chat: { getConversation: async () => conversation }, identity: { getUser: async () => ({name:'alice',blocked:false}), getAccountAccess: async () => ({capabilities:['playwright-reader.use']}) } } as never, issuePreviewRunKey: async () => 'fixture', logBrowserShot: async () => undefined })
