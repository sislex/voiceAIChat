...
beforeEach(async () => {
  ...
  const project = (await db.projects.createProject('ann', { name: 'Chat' }))!
  projectId = project.id
  columnId = (await db.tasks.getBoard('ann', projectId))!.columns[0]!.id
  taskId = (await db.tasks.createTask('ann', projectId, { columnId, title: 'Починить логин' }))!.id
  conv = (await db.chat.ensureKanbanAssistantConversation('ann', projectId))!.id
  ...
  startedCi = []
  const launchers = {
    startCi: async (_userId: string, _projectId: string, taskIdArg: string) => {
  ...
  orchestration = createOrchestrationManager({ db, runs: async () => launchers, tickMs: 60_000 })
  app = Fastify({ logger: false })
  registerKanbanMcp(app, { db, contexts, ui, boardChanged: (id) => boardEvents.push(id), runs: async () => launchers, orchestration: () => orchestration }, SECRET)
  await app.ready()
})
afterEach(async () => {
...
describe('kanbanMcp', () => {
  it('без секрета — 403, для чужого или непроектного разговора — 404', async () => {
    ...
    const loose = await db.chat.createConversation('ann', 'Просто чат')
    ...
    const make = await db.chat.createConversation('ann', 'Make', 'make')
    await db.chat.setConversationProject('ann', make.id, projectId)
  ...
  it('в режиме автономии карточка создаётся без вопросов и рассылает доску', async () => {
    ...
    expect((await db.tasks.getBoard('ann', projectId))!.tasks.map((task) => task.title)).toContain('Корзина')
  })

  it('в режиме подтверждений изменение уходит пользователю и отказ его отменяет', async () => {
    await db.chat.setConversationAutonomy('ann', conv, 'confirm')
    await rpc(INIT)
    ...
    expect((await db.tasks.getTaskDetail('ann', projectId, taskId))!.title).toBe('Починить логин')
  ...
  it('комментарии карточки: модель добавляет, правит и удаляет с подтверждением', async () => {
    ...
    const taskId = (await db.tasks.createTask('ann', projectId, { columnId: board.columns[0]!.id, title: 'С комментами' }))!.id
    ...
    expect((await db.tasks.taskActivity('ann', projectId, taskId))!.comments).toEqual([])
  ...
  it('в режиме «План» изменения запрещены', async () => {
    ...
    expect((await db.tasks.getBoard('ann', projectId))!.tasks).toHaveLength(1)
  })
  ...
  it('перенос уважает карту переходов workflow', async () => {
    ...
    await db.tasks.moveTask('ann', projectId, taskId, { columnId: development.id })
    await rpc(INIT)
  ...
  it('создание задачи-дубликата блокируется, пока модель не подтвердит разницу', async () => {
    const board = (await db.tasks.getBoard('ann', projectId))!
    const development = board.columns.find((column) => column.semanticType === 'development')
    if (development) await db.tasks.moveTask('ann', projectId, taskId, { columnId: development.id })
    await rpc(INIT)
  ...
  it('kanban_find_similar отличает «сделано и вмержено» от «сделано, но не вмержено»', async () => {
    ...
    const done = board.columns.find((column) => column.semanticType === 'done')
    if (!done) return
    await db.tasks.moveTask('ann', projectId, taskId, { columnId: done.id })
    await rpc(INIT)
  ...
  it('пароли тестовых учёток проекту не отдаются', async () => {
    await db.projects.updateProject('ann', projectId, { testUsers: [{ role: 'admin', name: 'qa', password: 'secret' }] })
    await rpc(INIT)
  ...
  it('повтор того же вызова в ходе не создаёт вторую карточку', async () => {
    ...
    expect((await db.tasks.getBoard('ann', projectId))!.tasks.filter((task) => task.title === 'Корзина')).toHaveLength(1)
  ...
  it('релизные инструменты закрыты для не-владельца и спрашивают подтверждение у владельца', async () => {
    ...
    await db.projects.addMember('ann', projectId, 'bob')
    const memberConv = (await db.chat.ensureKanbanAssistantConversation('bob', projectId))!.id
