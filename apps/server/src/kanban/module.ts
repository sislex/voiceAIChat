...
async function createKanbanModuleImpl(deps: KanbanDeps) {
  ...
  const parseTaskPreparation = (text: string): DevelopmentReadiness => {
    ...
    for (const [index, item] of testCases.entries()) {
      ...
      if (typeof testCase.testType !== 'string' || !['ui', 'api', 'integration', 'negative', 'regression', 'manual'].includes(testCase.testType)) issues.push(`${path}.testType содержит недопустимый тип проверки`)
