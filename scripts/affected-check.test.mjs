...
test('selectAffected выбирает пакеты и безопасный fallback', async (t) => {
  ...
  await t.test('shared проверяет себя и всех известных потребителей', () => {
    ...
    assert.equal(decision.full, false)
    assert.deepEqual(ids(decision), ['voice-browser', 'component-runtime', 'ui-foundation', 'make-app', 'image-studio-app', 'make-contracts', 'shared', 'chat-app', 'projects-app', 'operations-app', 'admin-app', 'web-reader', 'playwright-reader', 'ui', 'server', 'image-studio', 'make', 'playwright-reader-service', 'runner', 'tts-runner', 'stt-runner', 'automation-runner', 'browser-runner', 'agent', 'web', 'web-recorder', 'browser-contracts', 'playwright-reader-contracts', 'web-reader-contracts', 'web-reader-service'])
  })
  ...
  await t.test('ядро сессий тянет сервер и UI как потребителей', () => {
    ...
    assert.equal(decision.full, false)
    assert.deepEqual(ids(decision), ['voice-browser', 'component-runtime', 'ui-foundation', 'make-app', 'image-studio-app', 'make-contracts', 'shared', 'sessions-core', 'sessions-app', 'profile-app', 'chat-app', 'projects-app', 'operations-app', 'admin-app', 'web-reader', 'playwright-reader', 'ui', 'server', 'image-studio', 'make', 'playwright-reader-service', 'runner', 'tts-runner', 'stt-runner', 'automation-runner', 'browser-runner', 'agent', 'web', 'web-recorder', 'browser-contracts', 'playwright-reader-contracts', 'web-reader-contracts', 'web-reader-service'])
  })
  ...
  for (const file of ['package-lock.json', 'package.json', 'scripts/kb.mjs', '.github/workflows/ci.yml', 'unknown/critical.ts']) {
    await t.test(`${file} включает полный гейт`, () => {
      ...
      assert.deepEqual(ids(decision), ['voice-browser', 'component-runtime', 'ui-foundation', 'make-app', 'image-studio-app', 'make-contracts', 'shared', 'sessions-core', 'ui-kit', 'app-shell', 'sessions-app', 'profile-app', 'chat-app', 'projects-app', 'operations-app', 'admin-app', 'web-reader', 'playwright-reader', 'ui', 'server', 'image-studio', 'make', 'playwright-reader-service', 'runner', 'tts-runner', 'stt-runner', 'automation-runner', 'browser-runner', 'agent', 'web', 'web-recorder', 'browser-contracts', 'playwright-reader-contracts', 'web-reader-contracts', 'web-reader-service'])
