...
import { checkRoutes, compareRoutes } from './route-budgets.mjs'
if (process.platform === 'linux' && !process.env.DISPLAY) {
  ...
  try {
    ...
    const comparison = compareRoutes(JSON.parse(readFileSync('frontend-quality/measurements/CHAT-473/before.json', 'utf8')), report)
    writeFileSync(resolve(output, 'diff.json'), JSON.stringify({ budgetDiff, comparison }, null, 2))
